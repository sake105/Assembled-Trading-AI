...
def extract_universe_from_filename(filename: str) -> str | None:
    ...
    universe_patterns = [
        ...
        ("healthcare_biotech", r"healthcare.*biotech|biotech.*healthcare|healthcare_biotech"),
        ("energy_resources_cyclicals", r"energy.*resource|resource.*energy|energy_resources"),
        ("defense_security_aero", r"defense.*security|security.*defense|defense_security"),
        ("consumer_financial_misc", r"consumer.*financial|financial.*consumer|consumer_financial"),
    ]
...
def group_files_by_universe_and_factor_set(
    ...
    logger.info(f"Grouped files into {len(grouped)} (universe, factor_set) combinations")
...
def create_ranking_by_universe(
    ...
    for (universe, factor_set), files in grouped_files.items():
        logger.info(f"Building ranking for universe={universe}, factor_set={factor_set}")
        ...
        if not ic_paths and not rank_ic_paths:
            logger.warning(f"No IC or Rank-IC files for ({universe}, {factor_set}). Skipping.")
            continue
    ...
    if not all_rankings:
        logger.warning("No rankings generated. Check if summary files exist and have correct naming.")
        return pd.DataFrame()
...
def write_universe_ranking_report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write("This report shows factor rankings grouped by universe and factor set.\n\n")
        f.write("## Summary\n\n")
...
def create_plots(
    ...
    logger.info(f"Created {len(consolidated_df['universe'].unique())} plots in {plots_dir}")
...
def main() -> int:
    ...
    if not grouped_files:
        ...
        logger.info("Expected filename pattern: factor_analysis_{factor_set}_{horizon}d_{freq}_...")
        return 1
