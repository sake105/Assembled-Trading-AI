...
def main():
    ...
    if not settings.local_data_root:
        ...
        print('  $env:ASSEMBLED_LOCAL_DATA_ROOT = "F:\\Python_Projekt\\Aktiengerüst\\datensammlungen\\altdaten\\stand 3-12-2025"')
        return 1
    ...
    print(f"  Factor columns: {[c for c in factors_df.columns if c not in ['timestamp', 'symbol', 'close', 'open', 'high', 'low', 'volume']]}")
    print()
    ...
    print(f"  Computed IC for {len(ic_df.columns)} factors across {len(ic_df)} timestamps")
    print()
    ...
    try:
        ...
        fig, axes = plt.subplots(len(top_factors), 1, figsize=(12, 3 * len(top_factors)))
        ...
        for i, factor in enumerate(top_factors):
            ...
            if ic_col in ic_df.columns:
                axes[i].plot(ic_df.index, ic_df[ic_col], label=f"IC: {factor}", alpha=0.7)
