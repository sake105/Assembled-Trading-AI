...
def run_model_zoo_for_panel(
    ...
    if factor_panel_path.suffix == ".parquet":
        ...
        raise ValueError(f"Unsupported file format: {factor_panel_path.suffix}. Use .parquet or .csv")
    
    logger.info(f"Loaded {len(factor_panel_df)} rows, {len(factor_panel_df.columns)} columns")
    ...
    for i, model_cfg in enumerate(model_configs, 1):
        logger.info(f"[{i}/{len(model_configs)}] Running model: {model_cfg.name} ({model_cfg.model_type})")
        ...
        try:
            ...
            if last_model is not None and feature_names:
                try:
                    feature_imp_df = compute_model_feature_importance(last_model, feature_names)
                    feature_importance_dfs[model_cfg.name] = feature_imp_df
                    logger.info(f"  Feature importance computed for {model_cfg.name}: {len(feature_imp_df)} features")
                except Exception as e:
                    logger.warning(f"  Failed to compute feature importance for {model_cfg.name}: {e}")
                    # Continue without feature importance for this model
            
            if predictions_df.empty:
                logger.warning(f"Model {model_cfg.name} produced empty predictions, skipping...")
                continue
            ...
            r2_str = f"{result_row['test_r2_mean']:.4f}" if result_row['test_r2_mean'] is not None else "N/A"
            ic_ir_str = f"{result_row['ic_ir']:.4f}" if result_row['ic_ir'] is not None else "N/A"
    ...
    if "ic_ir" in summary_df.columns and summary_df["ic_ir"].notna().any():
        summary_df = summary_df.sort_values("ic_ir", ascending=False, na_position="last")
    elif "test_r2_mean" in summary_df.columns:
        summary_df = summary_df.sort_values("test_r2_mean", ascending=False, na_position="last")
    
    logger.info(f"Model zoo completed: {len([r for r in results if r.get('test_r2_mean') is not None])} successful, "
    ...
    if feature_importance_dfs:
        try:
            ...
            global_feature_importance_df = summarize_feature_importance_global(feature_importance_dfs)
            
            # Save global feature importance summary
            global_importance_path = output_dir / "ml_model_zoo_feature_importance_summary.csv"
            global_feature_importance_df.to_csv(global_importance_path, index=False)
            logger.info(f"Saved global feature importance summary to {global_importance_path}")
        except Exception as e:
            logger.warning(f"Failed to compute global feature importance summary: {e}", exc_info=True)
    ...
    if write_markdown:
        ...
        with md_path.open("w", encoding="utf-8") as f:
            ...
            if len(successful_df) > 0:
                ...
                # Best by IC-IR
                if "ic_ir" in successful_df.columns and successful_df["ic_ir"].notna().any():
                    best_ic_ir = successful_df.loc[successful_df["ic_ir"].idxmax()]
                    f.write(f"**Best IC-IR:** {best_ic_ir['model_name']} (IC-IR={best_ic_ir['ic_ir']:.4f})\n\n")
                
                # Best by R²
                if "test_r2_mean" in successful_df.columns and successful_df["test_r2_mean"].notna().any():
            ...
            if len(failed_df) > 0:
                ...
                for _, row in failed_df.iterrows():
                    f.write(f"- **{row['model_name']}**: {row.get('error', 'Unknown error')}\n")
                f.write("\n")
