...
def parse_model_params(param_strings: list[str]) -> dict[str, Any]:
    ...
    for param_str in param_strings:
        if "=" not in param_str:
            logger.warning(
                f"Invalid parameter format: {param_str}. Expected key=value. Skipping."
            )
            continue
...
def export_ml_alpha_factor(
    ...
    if test_start:
        try:
            ...
            raise ValueError(
                f"Invalid test_start date format: {test_start}. Error: {e}"
            )
    ...
    if predictions_df.empty:
        raise ValueError(
            "No predictions generated from CV. Check data quality and configuration."
        )

    # Ensure timestamp and symbol are present in predictions_df
    if (
        "timestamp" not in predictions_df.columns
        or "symbol" not in predictions_df.columns
    ):
        logger.warning(
        ...
        if isinstance(predictions_df.index, pd.MultiIndex):
            if len(predictions_df.index.names) >= 2:
                timestamp_name = (
                    predictions_df.index.names[0]
                    if "timestamp" in str(predictions_df.index.names[0])
                    else None
                )
                symbol_name = (
                    predictions_df.index.names[1]
                    if "symbol" in str(predictions_df.index.names[1])
                    else None
                )
            ...
            if len(predictions_df) == len(factor_panel_df):
                ...
                predictions_df["timestamp"] = factor_panel_df.loc[
                    predictions_df.index, "timestamp"
                ].values
                predictions_df["symbol"] = factor_panel_df.loc[
                    predictions_df.index, "symbol"
                ].values
            else:
                raise ValueError(
    ...
    predictions_for_merge = predictions_for_merge.rename(
        columns={"y_pred": column_name}
    )
    ...
    if "timestamp" in factor_panel_df.columns:
        if factor_panel_df["timestamp"].dtype == "object":
            ...
            not hasattr(factor_panel_df["timestamp"].dtype, "tz")
            or factor_panel_df["timestamp"].dtype.tz is None
        ):
            # Make timezone-aware if not already
            factor_panel_df["timestamp"] = pd.to_datetime(
                factor_panel_df["timestamp"], utc=True
            )

    if "timestamp" in predictions_for_merge.columns:
        if predictions_for_merge["timestamp"].dtype == "object":
            predictions_for_merge["timestamp"] = pd.to_datetime(
                predictions_for_merge["timestamp"], utc=True, errors="coerce"
            )
        elif (
            not hasattr(predictions_for_merge["timestamp"].dtype, "tz")
            or predictions_for_merge["timestamp"].dtype.tz is None
        ):
            predictions_for_merge["timestamp"] = pd.to_datetime(
                predictions_for_merge["timestamp"], utc=True
            )
    ...
    logger.info(
        f"  Rows with ML alpha predictions: {n_with_predictions} ({pct_with_predictions:.1f}%)"
    )
    logger.info(
        f"  Rows without predictions (training samples): {n_total - n_with_predictions}"
    )
    ...
    if global_metrics:
        ...
        logger.info("Model performance (global metrics):")
        if r2 is not None:
    ...
    predictions_output_path = (
        output_dir / f"ml_alpha_predictions_{model_type}_{horizon_str}.parquet"
    )
...
def main() -> int:
    ...
    try:
        ...
        model_params = (
            parse_model_params(args.model_param) if args.model_param else None
        )
