...
def run_factor_panel_export(config: PlaybookConfig) -> Path:
    ...
    from research.factors.export_factor_panel_for_ml import export_factor_panel_for_ml
    ...
    # Load prices
    logger.info(f"Loading prices (freq={config.freq}, start={config.start_date}, end={config.end_date})...")
    ...
    output_filename = (
        f"factor_panel_{universe_name}_{config.factor_set}_{config.horizon_days}d_{config.freq}.parquet"
    )
...
def select_best_model(model_zoo_df: pd.DataFrame) -> dict[str, Any]:
    ...
    if "ic_ir" in df.columns:
        ...
        if df.empty:
            logger.warning("No models with valid IC-IR found. Falling back to test_r2_mean.")
        else:
            # Sort by IC-IR descending
            df = df.sort_values("ic_ir", ascending=False)
            best_row = df.iloc[0]
            logger.info(f"Selected model by IC-IR: {best_row.get('model_name', 'unknown')} (IC-IR={best_row.get('ic_ir', 0):.4f})")
    elif "ic_mean" in df.columns:
        # Fallback to IC mean
        df = df[df["ic_mean"].notna()].copy()
        if df.empty:
            logger.warning("No models with valid IC mean found. Falling back to test_r2_mean.")
        else:
            df = df.sort_values("ic_mean", ascending=False)
            best_row = df.iloc[0]
            logger.info(f"Selected model by IC mean: {best_row.get('model_name', 'unknown')} (IC={best_row.get('ic_mean', 0):.4f})")
    else:
        # Fallback to test_r2_mean
        if "test_r2_mean" not in df.columns:
            raise ValueError("No valid metric columns found in model zoo DataFrame (expected ic_ir, ic_mean, or test_r2_mean)")
        ...
        best_row = df.iloc[0]
        logger.info(f"Selected model by Test-R²: {best_row.get('model_name', 'unknown')} (R²={best_row.get('test_r2_mean', 0):.4f})")
    ...
    result = {
        ...
        "test_r2_mean": float(best_row.get("test_r2_mean", 0)) if "test_r2_mean" in best_row else None,
        "test_mse_mean": float(best_row.get("test_mse_mean", 0)) if "test_mse_mean" in best_row else None,
        "ls_sharpe": float(best_row.get("ls_sharpe", 0)) if "ls_sharpe" in best_row and pd.notna(best_row.get("ls_sharpe")) else None,
    }
...
def run_ml_alpha_export(
    ...
    if not ml_alpha_panel_path.exists():
        raise RuntimeError(f"ML alpha panel file was not created: {ml_alpha_panel_path}")
    ...
    try:
        ...
        if ml_alpha_cols:
            ...
            logger.info(f"  Rows with predictions: {n_with_predictions} / {len(ml_alpha_df)}")
...
def run_backtests_with_bundles(
    ...
    if config.ml_alpha_bundle_path.exists():
        ...
        logger.warning(f"ML alpha bundle not found: {config.ml_alpha_bundle_path}. Skipping ML-only backtest.")
    ...
    for bundle_name, bundle_path, description in bundles_to_test:
        if not bundle_path.exists():
            logger.warning(f"Bundle file not found: {bundle_path}. Skipping {bundle_name} backtest.")
            continue
        ...
        cmd = [
            ...
            "--symbols-file", str(config.universe_file),
            "--data-source", "local",
            "--start-date", config.start_date,
            "--end-date", config.end_date,
            "--rebalance-freq", config.rebalance_freq,
            "--max-gross-exposure", str(config.max_gross_exposure),
            "--start-capital", str(int(config.start_capital)),
        ...
        try:
            ...
            logger.error(f"Unexpected error during backtest for {bundle_name}: {e}", exc_info=True)
            raise RuntimeError(f"Backtest failed for {bundle_name}: {e}") from e
...
def run_risk_reports(
    ...
    for backtest_dir in backtest_dirs:
        if not backtest_dir.exists():
            logger.warning(f"Backtest directory not found: {backtest_dir}. Skipping risk report.")
            continue
        ...
        cmd = [
            ...
            "--output-dir", str(backtest_dir),  # Risk reports go into backtest directory
        ]
        ...
        try:
            ...
            logger.info(f"  Risk report generated successfully")
            ...
            if (backtest_dir / "risk_by_regime.csv").exists():
                logger.info(f"  Regime attribution: risk_by_regime.csv")
            if (backtest_dir / "risk_by_factor_group.csv").exists():
                logger.info(f"  Factor attribution: risk_by_factor_group.csv")
            ...
            logger.warning(f"  Continuing with other backtests...")
        except Exception as e:
            logger.error(f"Unexpected error during risk report generation for {backtest_name}: {e}", exc_info=True)
            logger.warning(f"  Continuing with other backtests...")
...
def write_research_summary(
    ...
    # Header
    lines.append("# Research Summary: AI/Tech Multi-Factor ML Alpha with Regime Analysis")
    lines.append("")
    ...
    if model_zoo_df is not None and not model_zoo_df.empty:
        ...
        for _, row in top_models.iterrows():
            ...
            lines.append(f"| {model_name} | {model_type} | {ic_ir} | {test_r2} | {ls_sharpe} |")
        lines.append("")
    ...
    if best_model:
        ...
        lines.append("")
        lines.append(f"**Selected Model:** {best_model.get('model_name', 'N/A')} ({best_model.get('model_type', 'N/A')})")
        lines.append("")
    ...
    if backtest_dirs:
            ...
            if risk_summary_csv.exists():
                try:
                    ...
                    if not risk_df.empty:
                        ...
                        sharpe = risk_df.get("sharpe", pd.Series([None])).iloc[0] if "sharpe" in risk_df.columns else None
                        sortino = risk_df.get("sortino", pd.Series([None])).iloc[0] if "sortino" in risk_df.columns else None
                        max_dd = risk_df.get("max_drawdown", pd.Series([None])).iloc[0] if "max_drawdown" in risk_df.columns else None
                        ann_return = risk_df.get("mean_return_annualized", pd.Series([None])).iloc[0] if "mean_return_annualized" in risk_df.columns else None
                    ...
                    logger.warning(f"Could not load risk summary from {risk_summary_csv}: {e}")
                    performance_data.append({
                        "Bundle": bundle_name,
                        "Sharpe": "N/A",
                        "Sortino": "N/A",
        ...
        if performance_data:
            ...
            for data in performance_data:
                sharpe_str = f"{data['Sharpe']:.4f}" if isinstance(data['Sharpe'], (int, float)) and pd.notna(data['Sharpe']) else str(data['Sharpe'])
                sortino_str = f"{data['Sortino']:.4f}" if isinstance(data['Sortino'], (int, float)) and pd.notna(data['Sortino']) else str(data['Sortino'])
                max_dd_str = f"{data['Max DD']:.4f}" if isinstance(data['Max DD'], (int, float)) and pd.notna(data['Max DD']) else str(data['Max DD'])
                ann_ret_str = f"{data['Ann Return']:.2%}" if isinstance(data['Ann Return'], (int, float)) and pd.notna(data['Ann Return']) else str(data['Ann Return'])
                lines.append(f"| {data['Bundle']} | {sharpe_str} | {sortino_str} | {max_dd_str} | {ann_ret_str} |")
        lines.append("")
    ...
    lines.append("")
    lines.append("1. **Review Risk Reports:** Examine regime-specific performance and factor attribution")
    lines.append("2. **Compare Bundles:** Identify which bundle performs best in different market conditions")
    lines.append("3. **Model Refinement:** Consider tuning hyperparameters or trying additional models")
    lines.append("4. **Feature Analysis:** Review feature importance to identify most predictive factors")
    lines.append("5. **Regime Analysis:** If regime data is available, analyze performance by market regime")
    lines.append("")
...
def create_default_config() -> PlaybookConfig:
    ...
    return PlaybookConfig(
        # Paths
        data_root=Path("datensammlungen/altdaten/stand 3-12-2025"),  # Adjust to your local data root
        ...
        core_bundle_path=root / "config" / "factor_bundles" / "ai_tech_core_alt_bundle.yaml",
        core_ml_bundle_path=root / "config" / "factor_bundles" / "ai_tech_core_ml_bundle.yaml",
        ml_alpha_bundle_path=root / "config" / "factor_bundles" / "ai_tech_ml_alpha_bundle.yaml",
...
def main() -> None:
    ...
    try:
        ...
        logger.info(f"[SUCCESS] ML model zoo completed: {len(model_zoo_df)} models compared")
        logger.info("")
        
        # Step 3: Select best model
        best_model = select_best_model(model_zoo_df)
        artifacts["best_model"] = best_model
        logger.info(f"[SUCCESS] Best model selected: {best_model.get('model_name', 'unknown')}")
        logger.info("")
        ...
        logger.info(f"[SUCCESS] Risk reports generated: {len(risk_report_paths)} reports")
        logger.info("")
