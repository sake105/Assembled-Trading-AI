...
def setup_logging(
    ...
    file_format = "%(asctime)s | %(levelname)-8s | %(name)-30s | [%(run_id)s] %(message)s"
    ...
    root_logger.filters = [f for f in root_logger.filters if not isinstance(f, RunIDFilter)]
    for handler in root_logger.handlers:
    ...
    # Log initial message
    logger = logging.getLogger(__name__)
    logger.info(f"Logging initialized: Run-ID={run_id}, Level={level}, Log file={log_file}")
