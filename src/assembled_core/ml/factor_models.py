...
def prepare_ml_dataset(
    ...
    missing_features = [col for col in feature_cols if col not in factor_panel_df.columns]
    if missing_features:
        raise ValueError(
    ...
    if timestamp_col in df_filtered.columns:
        if experiment.test_start is not None:
            df_filtered = df_filtered[df_filtered[timestamp_col] >= experiment.test_start]
...
def _split_time_series(
    ...
    test_size_days = total_days // (n_splits + 1)  # Reserve some data for first train set
...
def run_time_series_cv(
    ...
    X, y = prepare_ml_dataset(
        factor_panel_df, experiment, timestamp_col, symbol_col
    )
    ...
    if len(splits) == 0:
        raise ValueError("No valid splits created. Check data range and split configuration.")
    ...
    for split_idx, (train_indices, test_indices) in enumerate(splits):
        ...
        if len(X_train) == 0 or len(X_test) == 0:
            logger.warning(f"Split {split_idx}: Empty train or test set after NaN removal. Skipping.")
            continue
        ...
        if timestamp_col in factor_panel_df.columns:
            pred_df[timestamp_col] = factor_panel_df.loc[y_test.index, timestamp_col].values
    ...
    if len(all_predictions) == 0:
        raise ValueError("No valid splits after processing. Check data quality and configuration.")
...
def evaluate_ml_predictions(
        ...
        for timestamp in predictions_df[timestamp_col].unique():
            ...
            try:
                ...
                try:
                    ...
                    rank_ic = float(ts_df["pred"].corr(ts_df["true"], method="spearman"))
                    if not np.isnan(rank_ic):
    ...
    if timestamp_col in predictions_df.columns and symbol_col in predictions_df.columns:
        ...
        for timestamp in predictions_df[timestamp_col].unique():
            ...
            bottom_quintile = ts_pred_sorted.iloc[: n // 5]  # Bottom 20%
            top_quintile = ts_pred_sorted.iloc[-n // 5 :]  # Top 20%
