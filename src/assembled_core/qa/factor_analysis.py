...
def add_forward_returns(
    ...
    if isinstance(horizon_days, int):
        ...
        if not horizons:
            raise ValueError("horizon_days must be a positive integer or a non-empty list of integers")
    ...
    for horizon in horizons:
        ...
        if return_type == "log":
            ...
            raise ValueError(f"Invalid return_type: {return_type}. Supported: 'log', 'simple'")
    ...
    if len(horizons) == 1:
        logger.info(f"Added forward returns column '{col_name_final}' (horizon: {horizons[0]} days, type: {return_type})")
    else:
        logger.info(f"Added forward returns columns for {len(horizons)} horizons: {horizons} (type: {return_type})")
...
def compute_factor_ic(
    ...
    if symbol_col is None:
        ...
        if group_col == "timestamp":
            ...
            potential_symbol_cols = [col for col in df.columns if col not in [group_col, fwd_return_col] + factor_cols]
            if potential_symbol_cols:
                symbol_col = potential_symbol_cols[0]
                logger.warning(f"Assuming '{symbol_col}' is the symbol column. Please ensure this is correct.")
            else:
                raise KeyError(
    ...
    if group_col == "timestamp" and not pd.api.types.is_datetime64_any_dtype(result_df[group_col]):
    ...
    for group_value in sorted(result_df[group_col].unique()):
        ...
        for factor_col in factor_cols:
            ...
            if method == "pearson":
                ...
                raise ValueError(f"Invalid method: {method}. Supported: 'pearson', 'spearman'")
...
def summarize_factor_ic(
    ...
    for factor in ic_df["factor"].unique():
        ...
        hit_ratio = float((factor_ic > 0).sum() / len(factor_ic)) if len(factor_ic) > 0 else 0.0
    ...
    if not summary_data:
        ...
        return pd.DataFrame(columns=["factor", "mean_ic", "std_ic", "ic_ir", "hit_ratio", "count", "min_ic", "max_ic"])
...
def run_factor_report(
    ...
    if factor_set not in ["core", "vol_liquidity", "all"]:
        raise ValueError(f"Invalid factor_set: {factor_set}. Supported: 'core', 'vol_liquidity', 'all'")
    ...
    result_df = result_df.sort_values([symbol_col, timestamp_col]).reset_index(drop=True)
    
    logger.info(f"Starting factor report: factor_set={factor_set}, fwd_horizon={fwd_horizon_days}d, "
    ...
    logger.info(f"Computed {len(factor_columns)} factors: {factor_columns[:5]}{'...' if len(factor_columns) > 5 else ''}")
    ...
    if ic_df.empty:
        ...
        summary_ic_df = pd.DataFrame(columns=["factor", "mean_ic", "std_ic", "ic_ir", "hit_ratio", "count", "min_ic", "max_ic"])
    else:
    ...
    if rank_ic_df.empty:
        ...
        summary_rank_ic_df = pd.DataFrame(columns=["factor", "mean_ic", "std_ic", "ic_ir", "hit_ratio", "count", "min_ic", "max_ic"])
    else:
...
def compute_ic(
    ...
    logger.info(f"Auto-detected {len(factor_cols)} factor columns: {factor_cols[:5]}{'...' if len(factor_cols) > 5 else ''}")
...
def summarize_ic_series(
    ...
    if isinstance(result_df.index, pd.DatetimeIndex):
        ...
        potential_timestamp_cols = [col for col in result_df.columns if "time" in col.lower() or "date" in col.lower()]
        if potential_timestamp_cols:
            ...
            if len(result_df.columns) > 0:
                ...
                result_df = result_df.set_index(timestamp_col)
                logger.warning(f"Using first column '{timestamp_col}' as timestamp index")
            else:
    ...
    for ic_col in ic_columns:
        ...
        factor_name = ic_col[len(ic_col_prefix):] if ic_col.startswith(ic_col_prefix) else ic_col
        ...
        # Hit ratio: percentage of positive IC values
        hit_ratio = float((ic_values > 0).sum() / len(ic_values)) if len(ic_values) > 0 else 0.0
    ...
    if not summary_data:
        logger.warning("No summary statistics computed. Check input IC DataFrame.")
        return pd.DataFrame(columns=["factor", "mean_ic", "std_ic", "ic_ir", "hit_ratio", "q05", "q95", "min_ic", "max_ic", "count"])
...
def compute_rolling_ic(
    ...
    logger.info(f"Computing rolling IC statistics for {len(ic_columns)} factor(s), window={window}")
    ...
    for ic_col in ic_columns:
        ...
        # Rolling mean
        rolling_mean = result_df[ic_col].rolling(window=window, min_periods=min(5, window // 4)).mean()
        rolling_df[f"rolling_mean_{factor_name}"] = rolling_mean
        
        # Rolling std
        rolling_std = result_df[ic_col].rolling(window=window, min_periods=min(5, window // 4)).std()
        
        # Rolling IR (rolling_mean / rolling_std)
        rolling_ir = rolling_mean / rolling_std
        rolling_ir = rolling_ir.replace([np.inf, -np.inf], np.nan)  # Handle division by zero
...
def example_factor_analysis_workflow(
    ...
    forward_return_cols = [col for col in data_with_returns.columns if col.startswith("fwd_ret") or col.startswith("fwd_return")]
    exclude_cols.update(forward_return_cols)
    
    factor_cols = [col for col in data_with_returns.columns if col not in exclude_cols]
    
    if not factor_cols:
        logger.warning("No factor columns found. Returning data with forward returns only.")
        return {
    ...
    logger.info(f"Found {len(factor_cols)} factor columns: {factor_cols[:5]}{'...' if len(factor_cols) > 5 else ''}")
    ...
    if fwd_return_col not in data_with_returns.columns:
        ...
        potential_cols = [col for col in data_with_returns.columns if str(first_horizon) in col and "fwd" in col.lower()]
        if potential_cols:
            fwd_return_col = potential_cols[0]
        else:
            raise ValueError(f"Could not find forward return column for horizon {first_horizon}")
    
    logger.info(f"Computing IC and Rank-IC using forward return column: {fwd_return_col}")
    ...
    summary_rank_ic_df = summarize_ic_series(rank_ic_df) if not rank_ic_df.empty else pd.DataFrame()
    
    # Step 5: Compute rolling IC statistics
    logger.info("Computing rolling IC statistics...")
    rolling_ic_df = compute_rolling_ic(ic_df, window=60) if not ic_df.empty else pd.DataFrame()
