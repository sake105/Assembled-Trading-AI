...
def build_event_window_prices(
    ...
    if event_id_col is None:
        ...
        if "event_id" in events_work.columns:
            ...
            events_work["event_id"] = (
                events_work[event_type_col].astype(str) + "_" +
                events_work[group_col].astype(str) + "_" +
    ...
    prices_sorted = prices_df.sort_values([group_col, timestamp_col]).reset_index(drop=True)
    ...
    for _, event_row in events_work.iterrows():
        ...
        window_prices["rel_day"] = (
            (window_prices[timestamp_col] - event_day_timestamp).dt.days
        )
    ...
    if not all_windows:
        ...
        return pd.DataFrame(columns=[
            "event_id", group_col, "event_type", "event_timestamp", "rel_day",
            timestamp_col, price_col
        ])
