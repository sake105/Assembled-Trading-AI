        ...
        try:
            ...
            if isinstance(symbol_prices_slice, pd.Series):
                ...
                if not exit_candidates.empty:
                    ...
                    # Try future prices
                    future_prices = symbol_prices_slice[
                        symbol_prices_slice.index > close_time
                    ]
...
def generate_trade_labels(
    ...
    if missing_price_cols:
        raise ValueError(
            f"prices missing required columns: {', '.join(missing_price_cols)}"
        )
    ...
    if label_type == "binary_outperformance" and benchmark_prices is not None:
        ...
        benchmark_prices["timestamp"] = pd.to_datetime(
            benchmark_prices["timestamp"], utc=True
        )
        benchmark_prices = benchmark_prices.sort_values("timestamp").reset_index(
            drop=True
        )
    ...
    for idx, signal in signals.iterrows():
        ...
        forward_mask = (symbol_prices["timestamp"] > entry_time) & (
            symbol_prices["timestamp"] <= exit_time
        )
        ...
        if label_type == "binary_outperformance" and benchmark_prices is not None:
            ...
            if not benchmark_entry_candidates.empty:
                benchmark_entry_price = float(
                    benchmark_entry_candidates.iloc[-1]["close"]
                )

                benchmark_exit_mask = (benchmark_prices["timestamp"] > entry_time) & (
                    benchmark_prices["timestamp"] <= exit_time
                )
                ...
                if not benchmark_exit_candidates.empty:
                    benchmark_exit_price = float(
                        benchmark_exit_candidates.iloc[-1]["close"]
                    )
                    benchmark_return = (
