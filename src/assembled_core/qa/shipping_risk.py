...
def compute_shipping_exposure(
    ...
    merged["shipping_congestion_score"] = merged["shipping_congestion_score"].fillna(0.0)
...
def compute_systemic_risk_flags(
    ...
    high_shipping_risk = (
        avg_congestion > high_congestion_threshold or
        high_exposure_weight > high_exposure_threshold
    )
    
    exposed_to_blockade_routes = len(exposed_symbols) > 0
    
    # Determine risk level
    if avg_congestion > high_congestion_threshold and high_exposure_weight > high_exposure_threshold:
        risk_level = "HIGH"
        risk_reason = f"High average congestion ({avg_congestion:.1f}) and high exposure weight ({high_exposure_weight:.1%})"
    elif avg_congestion > high_congestion_threshold or high_exposure_weight > high_exposure_threshold:
        risk_level = "MEDIUM"
