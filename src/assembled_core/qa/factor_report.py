...
def run_factor_report(
    ...
    for col in result_df.columns:
        ...
        if col not in original_cols and result_df[col].dtype in ["float64", "float32", "int64", "int32"]:
            factor_cols.append(col)
    ...
    if not factor_cols:
        ...
        empty_summary = pd.DataFrame(columns=["factor", "mean_ic", "std_ic", "ic_ir", "hit_ratio", "count", "min_ic", "max_ic"])
        return {
            "factors": result_df,
            "ic": empty_ic,
            "rank_ic": empty_ic,
    ...
    logger.info(f"Identified {len(factor_cols)} factor columns: {factor_cols[:5]}{'...' if len(factor_cols) > 5 else ''}")
