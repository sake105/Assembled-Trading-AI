...
def build_factor_ranking(
    ...
    if not rank_ic_dfs:
        logger.warning(
            "No valid Rank-IC summary files found. Rank-IC metrics will be missing."
        )
        ...
        rank_ic_merged = rank_ic_merged.rename(
            columns={
    ...
    if portfolio_summary_paths:
        ...
        for path in portfolio_summary_paths:
            try:
                ...
                logger.warning(
                    f"Failed to read portfolio summary {path}: {e}. Skipping."
                )
                continue

        if not portfolio_dfs:
            logger.warning(
                "No valid portfolio summary files found. Portfolio metrics will be missing."
            )
            ...
            portfolio_grouped = portfolio_merged.groupby("factor", as_index=False).agg(
                agg_dict
            )
    else:
        portfolio_grouped = pd.DataFrame(columns=["factor"])
    ...
    if not portfolio_grouped.empty and "sharpe" in portfolio_grouped.columns:
        ...
        ranking_df = ranking_df.merge(
            portfolio_grouped[portfolio_cols], on="factor", how="left"
        )
    ...
    if len(ic_ir_values) > 0:
        ...
        if ic_ir_range > 1e-10:
            ranking_df["ic_ir_normalized"] = (
                ranking_df["ic_ir"] - ic_ir_min
            ) / ic_ir_range
        else:
    ...
    if len(dsr_values) > 0:
        ...
        if dsr_range > 1e-10:
            ranking_df["dsr_normalized"] = (
                ranking_df["ls_deflated_sharpe"] - dsr_min
            ) / dsr_range
        else:
    ...
    ranking_df.loc[only_ic_mask, "combined_score"] = ranking_df.loc[
        only_ic_mask, "ic_ir_normalized"
    ]

    # Only dsr
    only_dsr_mask = ~has_ic_ir & has_dsr
    ranking_df.loc[only_dsr_mask, "combined_score"] = ranking_df.loc[
        only_dsr_mask, "dsr_normalized"
    ]

    # Drop normalization columns
    ranking_df = ranking_df.drop(
        columns=["ic_ir_normalized", "dsr_normalized"], errors="ignore"
    )
