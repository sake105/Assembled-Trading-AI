...
def compute_sharpe_ratio(
    returns: pd.Series, freq: str = "1d", risk_free_rate: float = 0.0
) -> float | None:
...
def compute_cagr(
    start_value: float, end_value: float, periods: int, freq: str = "1d"
) -> float | None:
...
def compute_equity_metrics(
    ...
    if (
        start_eq > 0 and abs(start_eq - start_capital) / start_capital < 0.1
    ):  # Within 10% of start_capital
    ...
    # Risk-adjusted returns
    sharpe_ratio = (
        compute_sharpe_ratio(returns, freq, risk_free_rate)
        if len(returns) >= 2
        else None
    )
    sortino_ratio = (
        compute_sortino_ratio(returns, freq, risk_free_rate)
        if len(returns) >= 2
        else None
    )

    # Drawdown metrics
    _, max_drawdown, max_drawdown_pct, current_drawdown = compute_drawdown(
        equity_series
    )
    ...
    if cagr is not None and max_drawdown_pct < 0:
        calmar_ratio = (
            cagr / abs(max_drawdown_pct / 100.0) if max_drawdown_pct != 0 else None
        )
...
def deflated_sharpe_ratio_from_returns(
    ...
    if scale == "daily":
        sharpe_annual = compute_sharpe_ratio(
            returns_clean, freq=freq, risk_free_rate=risk_free_rate
        )
    else:
    ...
    if kurtosis is None:
        kurtosis = (
            float(returns_clean.kurtosis()) + 3.0 if len(returns_clean) >= 4 else 3.0
        )
