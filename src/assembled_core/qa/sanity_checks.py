...
class SanityChecker:
    ...
    def __init__(self, history_dir: Path | str | None = None, lookback_days: int = 30) -> None:
        self.history_dir = Path(history_dir) if history_dir else Path("output/trade_journal")
    ...
    def check_order(self, order: dict, market_state: dict | None = None) -> dict:
        ...
        sym_hist = self.recent_trades[self.recent_trades["symbol"] == sym] \
        ...
        if not sym_hist.empty and "side" in sym_hist.columns:
            last_3 = sym_hist.sort_values("timestamp").tail(3) \
                if "timestamp" in sym_hist.columns else sym_hist.tail(3)
            if len(last_3) == 3:
        ...
        if "last_quote" in market_state and market_state["last_quote"] and limit_price > 0:
            ...
            if "last_quote_time" in market_state:
                try:
                    age_s = (pd.Timestamp.now(tz="UTC") -
                             pd.Timestamp(market_state["last_quote_time"])).total_seconds()
                    if age_s > 60:
                        flags.append({
                            "rule": "quote_stale",
                            "severity": "medium",
        ...
        max_sev = max(flags, key=lambda f: severity_order.get(f["severity"], 0),
                      default={"severity": "none"})["severity"] if flags else "none"

        if flags:
            logger.warning(
                "[sanity] %s %s: %d flag(s), max_severity=%s halt=%s",
                side.upper(), sym, len(flags), max_sev, halt,
            )
