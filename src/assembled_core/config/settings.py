...
class Settings(BaseSettings):
    ...
    environment: Environment = Field(
        ...
        description="Environment mode: BACKTEST, PAPER, or DEV"
    )
    
    # Base directory (auto-detected from file location)
    base_dir: Path = Field(
        default_factory=lambda: Path(__file__).resolve().parents[3],  # config/settings.py -> config -> assembled_core -> src -> repo root
    ...
    sample_events_dir: Path = Field(
        default_factory=lambda: Path(__file__).resolve().parents[3] / "data" / "sample" / "events",
        description="Directory for sample event data (insider, shipping, etc.)"
    )
    
    sample_eod_file: Path = Field(
        default_factory=lambda: Path(__file__).resolve().parents[3] / "data" / "sample" / "eod_sample.parquet",
    ...
    supported_freqs: tuple[str, ...] = Field(
        default=("1d", "5min"),
        description="Supported trading frequencies"
    )
