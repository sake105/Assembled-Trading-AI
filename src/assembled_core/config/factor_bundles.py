...
def load_factor_bundle(path: str | Path) -> FactorBundleConfig:
    ...
    for factor_data in data["factors"]:
        ...
        if "weight" not in factor_data:
            raise ValueError(f"Factor '{factor_data['name']}' missing 'weight' field in {path}")
        if "direction" not in factor_data:
            raise ValueError(f"Factor '{factor_data['name']}' missing 'direction' field in {path}")
...
def list_available_factor_bundles(root_dir: str | Path = "config/factor_bundles") -> list[Path]:
