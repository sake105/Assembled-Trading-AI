...
def _compute_config_hash(config: PaperTrackConfig) -> str:
    ...
    config_dict = {
        ...
        "strategy_params": (
            sorted(config.strategy_params.items())
...
class PaperTrackDayResult:
    ...
    daily_turnover: float = (
        0.0  # Daily turnover (sum(abs(trade_notional)) / equity_prev)
    )
    ...
    for window in windows:
        ...
        for end_idx in range(window - 1, len(df)):
            ...
            if (
                ...
                and "timestamp" in trades.columns
            ):
                start_ts = df.loc[start_idx, "timestamp"]
                end_ts = df.loc[end_idx, "timestamp"]
                trades_window = trades[
                    (trades["timestamp"] >= start_ts) & (trades["timestamp"] <= end_ts)
                ].copy()
                if not trades_window.empty:
                    equity_df = pd.DataFrame(
                        {
                            "timestamp": df.loc[start_idx:end_idx, "timestamp"].values,
...
def _migrate_state_dict(
    state_dict: dict[str, Any], from_version: str
) -> dict[str, Any]:
    ...
    if from_version == "1.0" and current_version == "2.0":
        ...
        logger.debug(
            "State migration v1.0 -> v2.0: Added last_equity, last_positions_value"
        )
...
def save_paper_state(state: PaperTrackState, state_path: Path) -> None:
    ...
    data = {
        ...
        "positions": (
            state.positions.to_dict(orient="records")
...
def _ptd_load_or_init_state(
    state_path: "Path | None", config: PaperTrackConfig, as_of: pd.Timestamp
) -> PaperTrackState:
...
def _ptd_load_and_filter_prices(
    config: PaperTrackConfig, universe_symbols: list[str], as_of: pd.Timestamp
    ...
    if prices_tradeable.empty:
        logger.warning(
            ...
            f"(requested: {n_req}, tradeable: {n_tradeable}, missing: {n_missing})"
        )
    return prices, prices_history, prices_tradeable, n_req, n_tradeable, n_missing


def _ptd_apply_order_filters(
    orders: pd.DataFrame, config: PaperTrackConfig, current_positions: pd.DataFrame
) -> pd.DataFrame:
    ...
    if config.rebalance_filter_enabled and not orders.empty:
        ...
        if stats["orders_dropped"] > 0:
            logger.info(
                ...
                f"(dropped {stats['orders_dropped']}, min_notional={config.rebalance_min_notional})"
            )
    if config.deadzone_enabled and not orders.empty:
        ...
        if stats["orders_dropped"] > 0:
            logger.info(
                f"Dead-zone filter: {stats['orders_before']} -> {stats['orders_after']} orders "
                f"(dropped {stats['orders_dropped']}, deadzone_pct={config.deadzone_pct})"
            )
    return orders


def _ptd_build_state_after(
    config: PaperTrackConfig,
    state_before: PaperTrackState,
...
def _ptd_compute_daily_metrics(
    filled_orders: pd.DataFrame,
    updated_positions: pd.DataFrame,
    prices_tradeable: pd.DataFrame,
    state_before: PaperTrackState,
    new_equity: float,
) -> dict:
...
def run_paper_day(
    ...
    """
    _ptd_validate_config(config, as_of, state_path)
    _set_random_seed(config.random_seed)
    state_before = _ptd_load_or_init_state(state_path, config, as_of)
    try:
        universe_symbols = _ptd_load_universe(config)
        prices, prices_history, prices_tradeable, n_sym_req, n_tradeable, n_missing = (
            _ptd_load_and_filter_prices(config, universe_symbols, as_of)
        )
        ...
        logger.info(
            ...
            f"daily_return={metrics['daily_return_pct']:.2f}%, trades={len(filled_orders)}"
        )
        return result
...
def write_paper_day_outputs(
    result: PaperTrackDayResult,
    output_dir: Path,
    config: PaperTrackConfig | None = None,
    ...
    with open(run_dir / "daily_summary.md", "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- Gross Exposure: ${result.gross_exposure:,.2f} ({result.gross_exposure_pct:.2f}%)\n"
        )
        f.write(
            f"- Net Exposure: ${result.net_exposure:,.2f} ({result.net_exposure_pct:.2f}%)\n\n"
        )
        f.write("## Universe\n")
        ...
        if result.n_missing > 0:
            f.write(
                f"  ⚠️  Warning: {result.n_missing} symbol(s) have missing data or NaN prices\n"
            )
        f.write("\n")
        f.write("## Performance\n")
        ...
        f.write("\n## Metadata\n")
        f.write(
            "- See `manifest.json` for run metadata, config hash, and git commit hash.\n"
        )
...
def _write_run_manifest(
    ...
    manifest = {
        ...
        "strategy_name": (
            state_after.strategy_name
            if state_after
            else (config.strategy_name if config else "unknown")
        ),
        ...
        "state_before": {
            ...
            "positions_count": (
                len(state_before.positions)
                if state_before and not state_before.positions.empty
                else 0
            ),
            "last_run_date": (
                state_before.last_run_date.isoformat()
                if state_before and state_before.last_run_date
                else None
            ),
        },
        "state_after": {
            ...
            "positions_count": (
                len(state_after.positions)
                if state_after and not state_after.positions.empty
                else 0
            ),
            "last_run_date": (
                state_after.last_run_date.isoformat()
                if state_after and state_after.last_run_date
                else None
            ),
        },
        "run_summary": {
