...
def export_metrics_json(
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to create output directory for metrics JSON: {output_path.parent}"
        ) from exc
...
def _metrics_to_dict(
    ...
    result["total_trades"] = (
        metrics.total_trades if metrics.total_trades is not None else None
    )

    # Metadata
    result["start_date"] = (
        metrics.start_date.isoformat() if metrics.start_date is not None else None
    )
    result["end_date"] = (
        metrics.end_date.isoformat() if metrics.end_date is not None else None
    )
    ...
    if "sharpe_ratio" in result:
        result["sharpe"] = result[
            "sharpe_ratio"
        ]  # Alias for batch runner compatibility
