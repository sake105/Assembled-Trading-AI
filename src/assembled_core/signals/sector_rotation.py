...
def compute_sector_scores(
    ...
    if symbol_col in spy_prices.columns:
        ...
        spy_series = (
            spy_prices[price_col].sort_index()
            if price_col in spy_prices.columns
            else spy_prices.squeeze()
        )
    ...
    if not available_etfs:
        _log.warning(
            "No sector ETFs found in price data. Columns: %s", list(sector_wide.columns)
        )
        return pd.DataFrame()
    ...
    _log.info(
        "Sector scores computed: %d dates, %d ETFs", len(result_df), len(available_etfs)
    )
    return result_df
...
def generate_sector_rotation_signals(
    ...
    for etf in etfs:
        key = f"{etf}_score"
        val = (
            scores_row.get(key, np.nan)
            if isinstance(scores_row, dict)
            else scores_row.get(key, np.nan)
        )
        if not np.isnan(val):
            etf_scores[etf] = val
