...
def generate_event_signals(
    ...
    *,
    ...
    if insider_weight < 0 or shipping_weight < 0:
        raise ValueError(f"weights must be non-negative, got insider_weight={insider_weight}, shipping_weight={shipping_weight}")
    ...
    combined_signal = (insider_weight * insider_signal) + (shipping_weight * shipping_signal)
    ...
    logger.debug(f"Generated event signals: {len(result)} rows, "
