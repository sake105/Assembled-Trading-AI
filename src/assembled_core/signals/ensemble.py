...
def apply_meta_filter(
    ...
    if len(result) == len(confidence_scores):
        ...
        if all(k in signals.columns for k in join_keys):
            ...
            raise ValueError(f"Cannot join on keys {join_keys}: not all keys present in signals")
    else:
    ...
    if filtered_count > 0:
        logger.info(f"Filtering {filtered_count} signals with confidence < {min_confidence}")
...
def apply_meta_scaling(
    ...
    if join_keys is not None:
        ...
        if all(k in signals.columns for k in join_keys):
            ...
            confidence_df = pd.DataFrame({
                "meta_confidence": confidence_scores.values,
                **{k: features_reset[k].values for k in join_keys if k in features_reset.columns}
            })
    ...
    if scale_score and "score" in result.columns:
        ...
        logger.info("Created final_score from meta-confidence (no original score column)")
