...
class SignalsResponse(BaseModel):
    ...
    first_timestamp: Optional[datetime] = Field(
        None, description="First signal timestamp"
    )
    last_timestamp: Optional[datetime] = Field(
        None, description="Last signal timestamp"
    )
...
class OrdersResponse(BaseModel):
    ...
    first_timestamp: Optional[datetime] = Field(
        None, description="First order timestamp"
    )
...
class PerformanceSummary(BaseModel):
    ...
    sharpe_ratio: Optional[float] = Field(
        None, description="Sharpe ratio from performance_report"
    )

    model_config = ConfigDict(
        json_schema_extra={"example": {"pf": 1.1250, "sharpe_ratio": 1.2345}}
    )
...
class RiskMetrics(BaseModel):
    ...
    max_drawdown: float = Field(
        ..., description="Maximum drawdown (absolute, negative value)"
    )
    max_drawdown_pct: float = Field(
        ..., description="Maximum drawdown (percent, negative value)"
    )
...
class QaCheck(BaseModel):
    ...
    details: Optional[dict[str, Any]] = Field(
        None, description="Additional check details"
    )
...
class QaStatus(BaseModel):
    ...
    summary: dict[str, int] = Field(
        ..., description="Summary counts: {'ok': N, 'warning': M, 'error': K}"
    )
...
class EquityPoint(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"timestamp": "2023-01-01T00:00:00Z", "equity": 10000.0}
        }
    )
...
class PerformanceMetricsResponse(BaseModel):
    ...
    sortino_ratio: Optional[float] = Field(
        None, description="Sortino Ratio (annualized)"
    )
    ...
    turnover: Optional[float] = Field(
        None, description="Portfolio turnover (annualized)"
    )
...
class QAGateResultModel(BaseModel):
    ...
    gate_name: str = Field(
        ..., description="Name of the gate (e.g., 'sharpe_ratio', 'max_drawdown')"
    )
    ...
    details: Optional[dict[str, Any]] = Field(
        None, description="Additional details (e.g., actual value, threshold)"
    )
...
class QAGatesSummaryResponse(BaseModel):
    ...
    overall_result: str = Field(
        ..., description="Overall result: 'OK', 'WARNING', or 'BLOCK'"
    )
    counts: dict[str, int] = Field(
        ..., description="Gate counts: {'ok': N, 'warning': M, 'block': K}"
    )
    gate_results: list[QAGateResultModel] = Field(
        ..., description="List of individual gate results"
    )
...
class QAStatusSummary(BaseModel):
    ...
    overall_result: str = Field(
        ..., description="Overall QA gate result: 'OK', 'WARNING', or 'BLOCK'"
    )
    ...
    last_updated: Optional[datetime] = Field(
        None, description="Timestamp of last QA evaluation"
    )
...
class RiskStatusSummary(BaseModel):
    ...
    max_drawdown_pct: Optional[float] = Field(
        None, description="Maximum drawdown (percent, negative value)"
    )
    volatility: Optional[float] = Field(None, description="Volatility (annualized)")
    var_95: Optional[float] = Field(None, description="Value at Risk (95% confidence)")
    current_drawdown: Optional[float] = Field(
        None, description="Current drawdown (absolute)"
    )
    last_updated: Optional[datetime] = Field(
        None, description="Timestamp of last risk evaluation"
    )
...
class FeatureDriftItem(BaseModel):
    ...
    drift_flag: str = Field(
        ..., description="Drift severity: 'NONE', 'MODERATE', or 'SEVERE'"
    )

    model_config = ConfigDict(
        json_schema_extra={
            "example": {"feature": "ta_ema_20", "psi": 0.25, "drift_flag": "MODERATE"}
        }
    )
        ...
        json_schema_extra={
            "example": {
                ...
                "features_with_drift": [
                    ...
                    {"feature": "insider_net_buy", "psi": 0.15, "drift_flag": "NONE"},
                ],
...
class PaperOrderRequest(BaseModel):
    ...
    price: Optional[float] = Field(
        None, gt=0, description="Order price (optional, None = market order)"
    )
    client_order_id: Optional[str] = Field(
        None, description="Optional client-provided order ID"
    )
    route: Optional[str] = Field(
        None, description="Order route (e.g., 'PAPER', 'IBKR'). Default: 'PAPER'"
    )
    source: Optional[str] = Field(
        None, description="Order source (e.g., 'CLI', 'API', 'BACKTEST', 'DASHBOARD')"
    )
...
class PaperOrderResponse(BaseModel):
    ...
    client_order_id: Optional[str] = Field(
        None, description="Client-provided order ID (if provided)"
    )
...
class PaperPosition(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"symbol": "AAPL", "quantity": 6.0}}
    )
...
class OmsOrderView(BaseModel):
    ...
    status: str = Field(..., description="Order status: NEW, FILLED, or REJECTED")
    route: Optional[str] = Field(
        None, description="Order route (e.g., 'PAPER', 'IBKR', etc.)"
    )
    source: Optional[str] = Field(
        None, description="Order source (e.g., 'CLI', 'API', 'BACKTEST')"
    )
