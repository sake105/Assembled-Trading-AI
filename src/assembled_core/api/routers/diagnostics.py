...
from src.assembled_core.api.models import FeatureDriftItem, FeatureDriftResponse, ModuleStatus, ModulesResponse
...
_MODULE_REGISTRY: list[dict] = [
    # wired
    {"module": "conviction_engine", "status": "wired", "callers": 4, "path": "src/assembled_core/intel/conviction_engine.py"},
    {"module": "market_stress", "status": "wired", "callers": 6, "path": "src/assembled_core/risk/market_stress.py"},
    {"module": "conformal_position", "status": "wired", "callers": 6, "path": "src/assembled_core/ml/conformal_position.py"},
    {"module": "signal_diagnostics", "status": "wired", "callers": 3, "path": "src/assembled_core/signals/signal_diagnostics.py"},
    {"module": "circuit_breaker", "status": "wired", "callers": 1, "path": "src/assembled_core/risk/circuit_breaker.py"},
    {"module": "behavioral_features", "status": "wired", "callers": 1, "path": "src/assembled_core/features/behavioral_features.py"},
    {"module": "altdata_loader", "status": "wired", "callers": 2, "path": "src/assembled_core/data/altdata_loader.py"},
    # canary
    {"module": "walk_forward", "status": "canary", "callers": 0, "path": "src/assembled_core/qa/walk_forward.py"},
    {"module": "cpcv_validation", "status": "canary", "callers": 0, "path": "src/assembled_core/qa/cpcv.py"},
    {"module": "leakage_analyzer", "status": "canary", "callers": 0, "path": "src/assembled_core/qa/leakage_analyzer.py"},
...
def get_feature_drift() -> FeatureDriftResponse:
    ...
    try:
        ...
        if not panel_files:
            raise HTTPException(status_code=404, detail="No panel parquet found for drift analysis")

        df = pd.read_parquet(panel_files[-1])

        # Select numeric feature columns; if panel is raw OHLCV, derive rolling features
        skip = {"symbol", "timestamp", "open", "high", "low", "close", "volume", "adj_close"}
        feature_cols = [c for c in df.select_dtypes("number").columns if c not in skip][:30]

        if not feature_cols and "close" in df.columns:
            # Derive basic technical features from OHLCV for drift detection
            grp = df.groupby("symbol") if "symbol" in df.columns else df
            close = df["close"] if "symbol" not in df.columns else df.groupby("symbol")["close"].transform("last")
            df["ret_1d"] = df["close"].pct_change(1)
            df["ret_5d"] = df["close"].pct_change(5)
            df["ret_20d"] = df["close"].pct_change(20)
            df["vol_20d"] = df["ret_1d"].rolling(20).std()
            df["vol_ratio"] = df["vol_20d"] / df["ret_1d"].rolling(60).std().replace(0, float("nan"))
            if "volume" in df.columns:
                df["vol_norm"] = df["volume"] / df["volume"].rolling(20).mean().replace(0, float("nan"))
            feature_cols = ["ret_1d", "ret_5d", "ret_20d", "vol_20d", "vol_ratio"] + (
                ["vol_norm"] if "volume" in df.columns else []
            )

        if not feature_cols:
            raise HTTPException(status_code=404, detail="No feature columns found in panel")
        ...
        for col in feature_cols:
            try:
                ...
                flag = "SEVERE" if psi >= 0.2 else ("MODERATE" if psi >= 0.1 else "NONE")
                drift_items.append(FeatureDriftItem(feature=col, psi=round(psi, 4), drift_flag=flag))
            except Exception:
                pass

        drift_items.sort(key=lambda x: x.psi, reverse=True)
        n_drifted = sum(1 for d in drift_items if d.drift_flag != "NONE")
        worst = max((d.drift_flag for d in drift_items), default="NONE",
