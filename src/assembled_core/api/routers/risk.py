...
def get_risk_summary(freq: Frequency) -> RiskMetrics:
    ...
    try:
        ...
        if len(returns) > 1 and returns.std() > 0:
            ...
            periods_per_year = 252 if freq.value == "1d" else 252 * 78  # 78 trading sessions per day (5min)
        ...
        max_drawdown_pct = float((max_drawdown / rolling_max.max()) * 100) if rolling_max.max() > 0 else 0.0
        ...
        raise HTTPException(status_code=500, detail=f"Error computing risk metrics: {e}")
