...
def get_qa_metrics(freq: str) -> PerformanceMetricsResponse:
    ...
    try:
                ...
                if "qa_metrics" in manifest and manifest["qa_metrics"]:
                    ...
                    if "start_date" in metrics_dict:
                        if isinstance(metrics_dict["start_date"], str):
                            metrics_dict["start_date"] = pd.to_datetime(metrics_dict["start_date"], utc=True)
                        elif isinstance(metrics_dict["start_date"], pd.Timestamp):
                            metrics_dict["start_date"] = metrics_dict["start_date"].to_pydatetime()
                    if "end_date" in metrics_dict:
                        if isinstance(metrics_dict["end_date"], str):
                            metrics_dict["end_date"] = pd.to_datetime(metrics_dict["end_date"], utc=True)
                        elif isinstance(metrics_dict["end_date"], pd.Timestamp):
                            metrics_dict["end_date"] = metrics_dict["end_date"].to_pydatetime()
                    
                    return PerformanceMetricsResponse(**metrics_dict)
            except Exception as e:
                logger.warning(f"Failed to load metrics from manifest: {e}, falling back to computation")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error computing QA metrics: {e}"
        )
    ...
    try:
        ...
        if manifest_path.exists():
            try:
                ...
                logger.warning(f"Failed to load gate results from manifest: {e}, falling back to computation")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error computing QA gates: {e}"
        )
