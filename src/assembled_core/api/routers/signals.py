...
def get_signals(freq: Frequency) -> SignalsResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Malformed price data: {e}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error loading prices: {e}")

    if prices.empty:
        raise HTTPException(
            status_code=404, detail=f"Price data for freq {freq.value} is empty."
        )
...
def get_latest_signals(freq: Frequency) -> list[Signal]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error computing latest signals: {e}"
        )
