...
from src.assembled_core.api.models import EquityCurveResponse, EquityPoint, Frequency, PortfolioSnapshot
...
def _parse_portfolio_report(report_path: Path) -> dict[str, float | int]:
    ...
    try:
        ...
        sharpe = float(sharpe_str) if sharpe_str and sharpe_str.lower() != "nan" else None
...
def get_portfolio_current(freq: Frequency) -> PortfolioSnapshot:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error loading portfolio data: {e}"
        )
...
def get_portfolio_equity_curve(freq: Frequency) -> EquityCurveResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error reading portfolio equity curve: {e}"
        )
