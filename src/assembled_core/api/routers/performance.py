...
def get_performance_metrics(freq: Frequency) -> dict:
    ...
    try:
        ...
        return {
            ...
            "rows": metrics["rows"],
            "first": metrics["first"].isoformat() if hasattr(metrics["first"], "isoformat") else str(metrics["first"]),
            "last": metrics["last"].isoformat() if hasattr(metrics["last"], "isoformat") else str(metrics["last"])
        }
