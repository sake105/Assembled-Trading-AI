...
def get_qa_status_summary(
    ...
    try:
        ...
        if manifest_path.exists():
            try:
                ...
                if "qa_gate_result" in manifest and manifest["qa_gate_result"]:
                    ...
                    overall_result = str(gate_dict.get("overall_result", "UNKNOWN")).upper()
                    gate_counts = {
                ...
                if "timestamp" in manifest:
                    try:
                        last_updated = datetime.fromisoformat(str(manifest["timestamp"]).replace("Z", "+00:00"))
                    except Exception:
                        pass
                ...
                logger.warning(f"Failed to load from manifest: {e}, computing on-the-fly")
        ...
        # Compute metrics
        start_capital = equity_df["equity"].iloc[0] if "equity" in equity_df.columns else 10000.0
...
def get_risk_status_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error computing risk status: {e}"
        )
...
def get_drift_status_summary(
    ...
    try:
        ...
        if drift_results_file.exists():
            ...
            if not drift_df.empty and "feature" in drift_df.columns and "psi" in drift_df.columns:
                # Sort by PSI descending and take top N
                drift_df_sorted = drift_df.sort_values("psi", ascending=False).head(top_n)
                ...
                last_updated = datetime.fromtimestamp(drift_results_file.stat().st_mtime)
        ...
        logger.info(f"No drift analysis file found for freq={freq}, returning example data")
        return DriftStatusSummary(
            overall_severity="NONE",
            features_with_drift=[],
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error computing drift status: {e}"
        )
