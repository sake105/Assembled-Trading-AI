...
from src.assembled_core.config.factor_bundles import FactorBundleConfig, load_factor_bundle
...
def _is_rebalance_date(
    ...
    try:
        ...
        logger.warning(f"Unknown rebalance_freq '{rebalance_freq}', defaulting to daily")
        return True
    except Exception:
        logger.warning(f"Could not parse rebalance_freq '{rebalance_freq}', defaulting to daily")
        return True
...
def generate_multifactor_long_short_signals(
    ...
    if config is None:
        ...
        if not bundles:
            raise ValueError("No factor bundles found. Please provide config.bundle_path")
    ...
    if factors is None:
        ...
        logger.info(f"Computing factors from prices (factor_set: {bundle.factor_set})...")
        ...
        logger.info(f"Using provided factors DataFrame with {len(factors_df.columns)} columns")
    ...
    if config.use_regime_overlay:
        ...
        try:
            ...
            macro_cols = ["macro_growth_regime", "macro_inflation_regime", "macro_risk_aversion_proxy"]
            if all(col in factors_df.columns for col in macro_cols):
                macro_factors_df = factors_df[["timestamp", "symbol"] + macro_cols].copy()
            ...
            if any(col in factors_df.columns for col in vol_cols):
                available_vol_cols = [col for col in vol_cols if col in factors_df.columns]
            ...
            try:
                ...
                breadth_df = compute_market_breadth_ma(
                    prices,
                    ma_window=config.regime_config.breadth_ma_window if config.regime_config else 50,
                )
                
                # Also try to get risk_on_off_score
                try:
                    from src.assembled_core.features.market_breadth import compute_risk_on_off_indicators
                    risk_indicators = compute_risk_on_off_indicators(prices)
                    if not risk_indicators.empty and "risk_on_off_score" in risk_indicators.columns:
                ...
                logger.warning(f"Could not compute market breadth for regime detection: {e}")
            ...
            logger.warning(f"Failed to build regime state: {e}. Continuing without regime overlay.")
            regime_state_df = None
...
def compute_multifactor_long_short_positions(
    ...
    if regime_state_df is None and hasattr(signals, "attrs") and "regime_state_df" in signals.attrs:
    ...
    if config.use_regime_overlay and regime_state_df is not None:
        ...
        regime_for_ts = regime_state_df[regime_state_df[timestamp_col] == latest_timestamp]
        if not regime_for_ts.empty:
            ...
            if config.regime_risk_map and current_regime in config.regime_risk_map:
                ...
                max_gross_exp = risk_params.get("max_gross_exposure", config.max_gross_exposure)
    ...
    if not short_signals.empty:
        ...
        for _, row in short_signals.iterrows():
            positions.append({
                ...
                "target_qty": short_weight_per_symbol * capital,  # Negative quantity for short
            })
    ...
    if not positions_df.empty:
        ...
        total_long_weight = positions_df[positions_df["target_weight"] > 0]["target_weight"].sum()
        total_short_weight = abs(positions_df[positions_df["target_weight"] < 0]["target_weight"].sum())
