"""STRATEGY-V2: 34-factor multi-factor strategy with regime-conditional weights.
...
Active factors (34):
  ...
  - Earnings/Insider (19-22): EPS surprise z, insider activity score (zeroed),
    insider cluster score (EDGAR), PEAD/SUE drift
  - News/Macro (23-26): sentiment, volume spike, growth momentum, inflation
  - Intermarket (27-29): bond-equity divergence, credit spread, yield curve
  - Options (30-31): put-call extreme, VIX regime score
  - Congress (32): congressional trading activity (zeroed)
  - Geo (33): geo-political risk composite (GPR index + ACLED intensity)
  - Buyback (34): corporate buyback drift signal

Crash probability is applied as a multiplicative scaler on the composite score,
not as a z-scored additive factor.

All 34 factors are wired to their data sources. Factors gracefully degrade
...
Meta-model confidence is a multiplicative filter, not an additive summand.
It gates signals via a min-confidence threshold and scales scores.
        ...
        for regime, weights in regimes.items():
            ...
            if abs(w_sum - 1.0) > 0.05:
                logger.warning(
                    ...
                    "(composite normalized by live-factor sum at scoring time)",
                    regime,
                    w_sum,
                )
