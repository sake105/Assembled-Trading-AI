    ...
    *,
    ...
    if policy is None:
        try:
            ...
            logger.debug(
                "[_tc_execution] load_policy failed, using empty policy: %s", _exc
            )
            policy = {}
    ...
    try:
        ...
        pwf = (
            prices_with_features if prices_with_features is not None else pd.DataFrame()
        )
        if not orders.empty and not pwf.empty:
            if prices_latest is not None and "close" in prices_latest.columns:
                latest_prices = prices_latest[["symbol", "close"]].rename(
                    columns={"close": "price"}
                )
            ...
            if latest_prices is not None:
                orders = orders.merge(
                    latest_prices, on="symbol", how="left", suffixes=("", "_latest")
                )
    ...
    try:
        impact_cfg = (policy.get("execution", {}) or {}).get(
            "pre_trade_impact", {}
        ) or {}
        if impact_cfg.get("enabled", False) and not orders.empty:
            orders, _impact_meta = _apply_pre_trade_impact(
                orders, prices_filtered, impact_cfg
            )
    except Exception as e:
    ...
    try:
        ...
        if group_cfg.get("enabled", False) and not orders.empty:
            ...
            try:
                ...
                logger.debug(
                    "[_tc_execution] security_master load failed, skipping group caps: %s",
                    _exc,
                )
            if sec_meta is not None:
                orders, _grp_meta = _apply_group_exposure_caps(
                    orders, sec_meta, group_cfg
                )
    except Exception as e:
        ...
        if (
            ...
            for _rl_idx, _rl_row in orders.iterrows():
                ...
                if _rl_qty >= _rl_min_qty and _rl_price > 0:
                    _rl_env_cfg = ExecutionEnvConfig(
                        total_shares=_rl_qty,
                    ...
                    orders.at[_rl_idx, "rl_avg_exec_price"] = _rl_res.get(
                        "avg_execution_price", _rl_price
                    )
                    orders.at[_rl_idx, "rl_est_shortfall_bps"] = _rl_res.get(
                        "shortfall_bps", 0.0
                    )
                    _rl_shortfall_bps.append(float(_rl_res.get("shortfall_bps", 0.0)))
            if _rl_shortfall_bps:
                log.debug(
                    "[RL-EXEC] annotated %d orders; avg shortfall %.1f bps",
                    len(_rl_shortfall_bps),
                    sum(_rl_shortfall_bps) / len(_rl_shortfall_bps),
                )
    except Exception as e:
    ...
    *,
    ...
    if result.orders_filtered is None:
        result.orders_filtered = (
            ...
            else pd.DataFrame(columns=["timestamp", "symbol", "side", "qty", "price"])
        )

    # A8: Apply cost annotation for backtest/paper modes
    if (
        ctx.mode in ("backtest", "paper")
        ...
        try:
            ...
            log.debug(
                "[book_fills] cost annotation skipped (mode=%s): %s",
                ctx.mode,
                _cost_err,
            )
        ...
        if result.orders_filtered is not None and not result.orders_filtered.empty:
            if "total_cost_bps" not in result.orders_filtered.columns:
                if "total_cost_cash" in result.orders_filtered.columns:
                    _notional = (
                        result.orders_filtered["qty"].abs()
                        * result.orders_filtered["price"].abs()
                    )
                    result.orders_filtered["total_cost_bps"] = 0.0
                    _mask = _notional > 0
                    result.orders_filtered.loc[_mask, "total_cost_bps"] = (
                        result.orders_filtered.loc[_mask, "total_cost_cash"].abs()
                        / _notional[_mask]
                        * 10_000.0
                    )
                elif "expected_impact_bps" in result.orders_filtered.columns:
                    result.orders_filtered["total_cost_bps"] = result.orders_filtered[
                        "expected_impact_bps"
                    ]
        ...
        if dm_cfg.get("enabled", False):
            ...
            if ref_path and current_features is not None and not current_features.empty:
                ...
                _ref_df = (
                    ...
                    if str(ref_path).endswith(".parquet")
                ...
                log.info(
                    ...
                    _drift_report.max_psi,
                    _drift_report.action,
                    len(_drift_report.drifted_features),
                )
    ...
    try:
        if ctx.write_outputs and ctx.as_of is not None:
            ...
            append_run_index(
                ...
                config_hash=compute_config_hash(policy) if policy else "",
                manifest_path=ctx.output_dir
                / "manifests"
                / str(ctx.as_of.date())
    ...
    try:
        ...
        if (
            sd_cfg.get("enabled", False)
            and result.prices_with_features is not None
            ...
            if (
                fwd_col in result.prices_with_features.columns
                and "timestamp" in result.prices_with_features.columns
            ):
                factor_cols = [
                    c
                    for c in result.prices_with_features.columns
                    if c
                    not in {
                        "timestamp",
                        "symbol",
                ...
                if factor_cols:
                    ...
                    save_signal_health_artifact(
                        ...
                        alerts,
                        output_dir=(
                            str(ctx.output_dir / "diagnostics")
                            if ctx.write_outputs
                            else sd_cfg.get("output_dir", "output/diagnostics")
                        ),
    ...
    try:
        ...
        if kpi_cfg.get("enabled", False):
            ...
            if "estimated_turnover" in tb_meta and tb_meta[
                "estimated_turnover"
            ] != float("inf"):
                kpi_metrics["assembled_turnover_estimated"] = float(
                    tb_meta["estimated_turnover"]
                )
            ...
            kpi_histograms = None
            if (
                result.orders_filtered is not None
                and "total_cost_bps" in result.orders_filtered.columns
            ):
                _slip_obs = result.orders_filtered["total_cost_bps"].dropna().tolist()
                if _slip_obs:
                    kpi_histograms = {
                        "assembled_slippage_bps": slippage_histogram(_slip_obs)
                    }
            ...
            try:
                ...
                kpi_metrics["assembled_kill_switch_engaged"] = (
                    1.0 if is_kill_switch_engaged() else 0.0
                )
            except Exception as _exc:
