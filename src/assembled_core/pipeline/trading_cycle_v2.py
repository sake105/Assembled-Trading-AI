    ...
    *,
    ...
    Kept steps (3-criteria rule — changes a downstream-read value):
      - Step 4: all sizing method dispatch (kelly/risk_parity/vol_scaled/
        black_litterman/cost_aware/erc/bl_blend/hrp/mvo/default)
      - Phase 9.5: liquidity-aware position scaling
      - Exposure overlay: GeoRisk × profit_lock × vol_targeting × market_stress ×
        crisis_alpha → apply_exposure_multiplier_to_targets
      - D2: factor risk model vol check (rescales target_weight)
      - Phase 11.5: trailing stops (ATR-based, modifies weights)
      - Turnover budget gate (INT-6): applies turnover cap
      - Correlation guard (M6-T07): scales correlated clusters
      - [D3] Crash prediction equity cap
      - [D4] Inverse-ETF tail hedge (when shadow_only=False)
      - Quantile asymmetry sizing (reduces high-downside-skew positions)
      - Crowding detector (HHI cap)
      - T4.1: Crisis Alpha weight cap (when shadow_only=False)
      - Step 4.5: Rebalance trigger check → sets do_rebal
      - Step 4.85: Cost-aware weight shrinkage

    Dropped (meta-only, 3-criteria rule fails criterion 1 or 3):
      - kelly_uncertainty_shadow, Step 4.9 ML snap, Step 4.86 regime costs,
        Step 4.87 stress scenario, Step 4.88 regime portfolio, Step 4.93 robust,
        Step 4.94 reverse stress, Step 4.9 long-short balance exposure metrics
    """
    if log is None:
        log = logger
    ...
    try:
                ...
                if prices_for_sizing is not None and not prices_for_sizing.empty and not signals.empty and "close" in prices_for_sizing.columns and "symbol" in prices_for_sizing.columns:
                    ...
                    if ctx.current_positions is not None and isinstance(ctx.current_positions, pd.DataFrame) and "symbol" in ctx.current_positions.columns:
                        for _, r in ctx.current_positions.iterrows():
                            _cur_w[r["symbol"]] = float(r.get("weight", r.get("target_weight", 0.0)))
        ...
        meta["sizing_method"] = sizing_method
    ...
    # ------------------------------------------------------------------
    # Exposure overlay: geo × profit_lock × vol_targeting × stress × crisis_alpha
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # D2: Factor risk model vol check
    # ------------------------------------------------------------------
    try:
    ...
    try:
        ...
        log.debug("correlation_guard skipped: %s", e)
    ...
    try:
        ...
        if cp_cfg.get("equity_cap_enabled", False) and not target_positions.empty and "target_weight" in target_positions.columns:
            ...
            if crash_prob > threshold:
                ...
                record_shadow("crash_prediction_cap", {"cap": cap_val, "scale": scale}, as_of=str(ctx.as_of) if ctx.as_of else None, meta={"applied": (not cp_shadow) and scale < 1.0})
            ...
            if _feature_cols and _target_col in prices_with_features.columns:
                ...
                if _valid_fcols:
                    ...
                    for idx, row in target_positions.iterrows():
                        if _asym_map.get(row.get("symbol", ""), 0.0) > _asym_thresh:
                            target_positions.at[idx, "target_weight"] *= _asym_red
                            if "target_qty" in target_positions.columns:
                                target_positions.at[idx, "target_qty"] *= _asym_red
    except Exception as e:
            ...
            if _hhi > 0.25 and len(_tw_dict_crowd) >= 5:
                ...
                for idx, row in target_positions.iterrows():
                    if abs(float(row.get("target_weight", 0.0))) > _max_w:
                        target_positions.at[idx, "target_weight"] = _max_w
                        if "target_qty" in target_positions.columns:
                            target_positions.at[idx, "target_qty"] = _max_w * ctx.capital
    except Exception as e:
        ...
        try:
            ...
            if not shadow_only and ca_result.get("target_weights") and not target_positions.empty and "target_weight" in target_positions.columns:
                ...
                for idx, row in target_positions.iterrows():
                    ...
                    if sym in ca_tw:
                        ca_cap = float(ca_tw[sym])
                        if float(row["target_weight"]) > ca_cap:
                            target_positions.at[idx, "target_weight"] = ca_cap
                            n_adjusted += 1
                if n_adjusted:
