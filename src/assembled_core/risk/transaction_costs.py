...
def estimate_per_trade_cost(
    ...
    **kwargs,
    ...
    if "commission" in trades.columns or "fee" in trades.columns:
        ...
        commission_cost = trades.get("commission", trades.get("fee", pd.Series(0.0, index=trades.index)))
...
def compute_tca_for_trades(
    ...
    *,
    ...
    if isinstance(cost_per_trade, float):
        ...
        for idx in trades.index:
            ...
            if (symbol, side) in cost_per_trade:
                ...
                raise ValueError(f"No cost mapping found for symbol={symbol}, side={side}")
        ...
        raise ValueError(f"Invalid cost_per_trade type: {type(cost_per_trade)}. Expected pd.Series, float, or dict.")
    ...
    if compute_pnl:
        ...
        logger.info("PnL computation requested but simplified - full round-trip matching not yet implemented")
...
def summarize_tca(
    ...
    if not pd.api.types.is_datetime64_any_dtype(tca_trades["timestamp"]):
        tca_trades["timestamp"] = pd.to_datetime(tca_trades["timestamp"], utc=True, errors="coerce")
    ...
    summary["avg_cost_per_trade"] = summary["cost_total"] / summary["n_trades"].replace(0, np.nan)
    ...
    if equity_curve is not None:
        if "timestamp" not in equity_curve.columns or "equity" not in equity_curve.columns:
            logger.warning("equity_curve must have 'timestamp' and 'equity' columns. Skipping cost ratio calculation.")
        else:
            # Compute returns from equity curve
            equity_curve = equity_curve.copy()
            if not pd.api.types.is_datetime64_any_dtype(equity_curve["timestamp"]):
                equity_curve["timestamp"] = pd.to_datetime(equity_curve["timestamp"], utc=True, errors="coerce")
            ...
            if "cost_ratio" not in summary.columns:
                ...
                summary["cost_ratio"] = summary["cost_ratio"].fillna(cost_ratio_from_returns)
...
def compute_cost_adjusted_risk_metrics(
    ...
    if not returns.index.equals(costs.index):
        ...
        logger.warning(f"Aligned returns and costs to {len(common_index)} common periods")
    ...
    if len(returns) == 1:
        ...
        return {
            "net_mean_return_annualized": float(net_return) if not np.isnan(net_return) else None,
            ...
            "cost_ratio": float(costs.sum() / returns.iloc[0]) if returns.iloc[0] != 0 else None,
            "n_periods": 1,
        }
    ...
    gross_metrics = compute_basic_risk_metrics(returns, freq=freq, risk_free_rate=risk_free_rate)
    
    # Compute net metrics
    net_metrics = compute_basic_risk_metrics(net_returns, freq=freq, risk_free_rate=risk_free_rate)
    ...
    # Compute cost impact
    cost_impact_sharpe = (gross_sharpe - net_sharpe) if (gross_sharpe is not None and net_sharpe is not None) else None
    cost_impact_cagr = (gross_cagr - net_cagr) if (gross_cagr is not None and net_cagr is not None) else None
    
    # Total cost and cost ratio
    total_cost = float(costs.sum())
    total_gross_return = float(returns.sum())
    cost_ratio = total_cost / abs(total_gross_return) if total_gross_return != 0 else None
