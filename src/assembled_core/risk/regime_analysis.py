...
def classify_regimes_from_index(
    ...
    vol_window = config.lookback_days_vol if config.lookback_days_vol is not None else config.vol_window
    trend_window = config.lookback_days_return if config.lookback_days_return is not None else config.trend_ma_window
    ...
    vol = index_returns.rolling(window=vol_window, min_periods=config.min_periods).std() * np.sqrt(252)
    ...
    regimes = pd.Series(["neutral"] * len(index_returns), index=index_returns.index, dtype=object)
    ...
    for i in range(len(index_returns)):
        ...
        # Rule 3: Bull
            ...
            regimes.iloc[i] = "bull"
        # Rule 4: Sideways
        elif abs(trend_val) <= config.trend_threshold and config.vol_threshold_low <= vol_val <= config.vol_threshold_high:
...
def summarize_metrics_by_regime(
    ...
    """
    from src.assembled_core.risk.risk_metrics import compute_risk_by_regime
    ...
    if not equity.index.equals(regimes.index):
        ...
        if len(common_index) == 0:
            logger.warning("No overlapping indices between equity and regimes. Returning empty DataFrame.")
            return pd.DataFrame(columns=["regime_label", "n_periods"])
    ...
    for regime in regimes_aligned.unique():
        ...
        # Total return
        total_return = float((1.0 + regime_returns).prod() - 1.0) if len(regime_returns) > 0 else 0.0
        ...
        if trades is not None and not trades.empty:
            if timestamp_col in trades.columns:
                ...
                regime_trades = trades[
                    trades[timestamp_col].isin(regime_equity.index)
                ]
                ...
                if not regime_trades.empty:
                    ...
                    result_row["win_rate"] = None  # TODO: Implement with position tracking
...
def summarize_factor_ic_by_regime(
    ...
    if ic_series.empty or regimes.empty:
        ...
        return pd.DataFrame(columns=["regime_label", "ic_mean", "ic_std", "ic_count", "ic_ir"])
    
    # Align indices
    common_index = ic_series.index.intersection(regimes.index)
    if len(common_index) == 0:
        logger.warning("No overlapping indices between ic_series and regimes. Returning empty DataFrame.")
        return pd.DataFrame(columns=["regime_label", "ic_mean", "ic_std", "ic_count", "ic_ir"])
