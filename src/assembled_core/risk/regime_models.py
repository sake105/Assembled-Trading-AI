        ...
        if macro_factors is not None:
            ...
            if not macro_data.empty:
                ...
                if not pd.isna(growth_regime) or not pd.isna(inflation_regime):
                    ...
                    inflation_val = inflation_regime if not pd.isna(inflation_regime) else 0.0
                ...
                if rv_col in vol_data.columns:
                    ...
                    if not rv_values.empty:
                        ...
                        if median_rv > 0.5:
                            ...
                            risk_score_from_vol = 1.0 - 2.0 * (median_rv - 0.1) / (0.5 - 0.1)
...
def compute_regime_transition_stats(
    ...
    for i, row in df.iterrows():
        ...
        if current_regime is None:
            ...
            if regime_start_idx is not None and i > regime_start_idx:
                duration = (df.iloc[i][timestamp_col] - df.iloc[regime_start_idx][timestamp_col]).days
                if duration > 0:
    ...
    if regime_start_idx is not None:
        ...
        if last_idx > regime_start_idx:
            duration = (df.iloc[last_idx][timestamp_col] - df.iloc[regime_start_idx][timestamp_col]).days
            if duration > 0:
                regime_durations[current_regime].append(duration)
    ...
    # Count transitions
    transition_counts = transitions_df.groupby(["from_regime", "to_regime"]).size().reset_index(name="count")
    ...
    for _, row in transition_counts.iterrows():
        ...
        total_from = transition_counts[transition_counts["from_regime"] == from_regime]["count"].sum()
    ...
    if not result_df.empty:
        result_df = result_df.sort_values(["from_regime", "transition_probability"], ascending=[True, False])
...
def evaluate_factor_ic_by_regime(
    ...
    if timestamp_col not in ic_df.columns:
        if ic_df.index.name == timestamp_col or isinstance(ic_df.index, pd.DatetimeIndex):
    ...
    if not ic_cols:
        logger.warning(f"No IC columns found with suffix '{ic_col_suffix}'. Available columns: {list(merged.columns)}")
        return pd.DataFrame(columns=[
            "factor",
            "regime",
            "mean_ic",
