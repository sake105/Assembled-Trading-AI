...
def detect_volume_anomalies(
    ...
    for ticker, group in df.groupby(ticker_col, sort=False):
        ...
        spike_mask = ((med20 > 0) & (group["volume"] > spike_multiple * med20)).fillna(False)

        is_zero = (group["volume"] == 0).astype(int)
        zero_run = is_zero.rolling(zero_volume_tolerance, min_periods=zero_volume_tolerance).sum()
        ...
        flagged.loc[spike_mask[spike_mask].index.intersection(flagged.index), "reason"] = "volume_spike"
