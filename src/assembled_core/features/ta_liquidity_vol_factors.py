...
def add_vol_of_vol(
    ...
    if not rv_cols:
        logger.warning("No realized volatility columns found. Skipping Vol-of-Vol calculation.")
        return result
    ...
    logger.info(f"Added Vol-of-Vol factors for {len(rv_cols)} RV columns with window {vol_window}")
...
def add_turnover_and_liquidity_proxies(
    ...
    if freefloat_col is not None and freefloat_col in result.columns and volume_col in result.columns:
        ...
        if freefloat_col is not None:
            logger.warning(f"Freefloat column {freefloat_col} not found. Skipping turnover calculation.")
    ...
    if volume_col in result.columns:
        ...
        logger.warning(f"Volume column {volume_col} not found. Skipping volume_zscore calculation.")
    ...
    if all(col in result.columns for col in ["high", "low", "close"]):
        ...
        logger.warning("High/low/close columns not available. Skipping spread_proxy calculation.")
