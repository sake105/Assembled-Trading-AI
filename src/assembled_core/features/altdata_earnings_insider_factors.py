...
def build_earnings_surprise_factors(
    ...
    missing_event_cols = [
        col for col in required_event_cols if col not in events_earnings.columns
    ]
    ...
    # EPS Surprise
    if (
        "eps_actual" in events_with_surprise.columns
        and "eps_estimate" in events_with_surprise.columns
    ):
        eps_actual = pd.to_numeric(events_with_surprise["eps_actual"], errors="coerce")
        eps_estimate = pd.to_numeric(
            events_with_surprise["eps_estimate"], errors="coerce"
        )
    ...
    # Revenue Surprise
    if (
        "revenue_actual" in events_with_surprise.columns
        and "revenue_estimate" in events_with_surprise.columns
    ):
        revenue_actual = pd.to_numeric(
            events_with_surprise["revenue_actual"], errors="coerce"
        )
        revenue_estimate = pd.to_numeric(
            events_with_surprise["revenue_estimate"], errors="coerce"
        )
    ...
    events_with_surprise = events_with_surprise.sort_values(
...
def build_insider_activity_factors(
    ...
    required_event_cols = [timestamp_col, group_col, "event_type"]
    missing_event_cols = [
        col for col in required_event_cols if col not in events_insider.columns
    ]
    if missing_event_cols:
        raise KeyError(
    ...
    if not events_insider.empty:
        if not pd.api.types.is_datetime64_any_dtype(events_insider[timestamp_col]):
            ...
            events_insider[timestamp_col] = pd.to_datetime(
                events_insider[timestamp_col], utc=True
            )
    ...
    if "direction" not in events_filtered.columns:
        ...
        events_filtered["direction"] = events_filtered["event_type"].str.replace(
            "insider_", ""
        )
    ...
    for symbol in result[group_col].unique():
        ...
        sell_events = symbol_events_indexed[
            symbol_events_indexed["direction"] == "sell"
        ]

        buy_notional_series = pd.to_numeric(
            buy_events["usd_notional"], errors="coerce"
        ).fillna(0.0)
        sell_notional_series = pd.to_numeric(
            sell_events["usd_notional"], errors="coerce"
        ).fillna(0.0)
        ...
        buy_notional_aligned = buy_notional_agg.reindex(
            symbol_prices_indexed.index, fill_value=0.0
        )
        sell_notional_aligned = sell_notional_agg.reindex(
            symbol_prices_indexed.index, fill_value=0.0
        )
        buy_count_aligned = buy_count_agg.reindex(
            symbol_prices_indexed.index, fill_value=0.0
        )
        sell_count_aligned = sell_count_agg.reindex(
            symbol_prices_indexed.index, fill_value=0.0
        )

        # Compute rolling sums over lookback_days window (time-based)
        net_notional_rolling = (
            buy_notional_aligned.rolling(
                window=f"{lookback_days}D", min_periods=0
            ).sum()
            - sell_notional_aligned.rolling(
                window=f"{lookback_days}D", min_periods=0
            ).sum()
        )

        buy_count_rolling = buy_count_aligned.rolling(
            window=f"{lookback_days}D", min_periods=0
        ).sum()
        sell_count_rolling = sell_count_aligned.rolling(
            window=f"{lookback_days}D", min_periods=0
        ).sum()
        ...
        # Assign factors
        symbol_prices_indexed[f"insider_net_notional_{lookback_days}d"] = (
            net_notional_rolling.values
        )
        symbol_prices_indexed[f"insider_buy_count_{lookback_days}d"] = (
            buy_count_rolling.values.astype(float)
        )
        symbol_prices_indexed[f"insider_sell_count_{lookback_days}d"] = (
            sell_count_rolling.values.astype(float)
        )
        symbol_prices_indexed[f"insider_buy_sell_ratio_{lookback_days}d"] = (
            buy_sell_ratio
        )
        ...
        if "volume" in symbol_prices_indexed.columns:
            ...
            symbol_prices_indexed[
                f"insider_net_notional_normalized_{lookback_days}d"
            ] = normalized_notional
        else:
            symbol_prices_indexed[
                f"insider_net_notional_normalized_{lookback_days}d"
