...
def test_drawdown_below_threshold_unchanged() -> None:
    ...
    config = PreTradeConfig(
        drawdown_threshold=0.2, de_risk_scale=0.25
    )  # 20% threshold, 25% scale
    ...
    assert (
        abs(filtered["qty"].iloc[0] - 100.0) < 1e-10
    ), "Qty should be unchanged (100.0)"
...
def test_drawdown_above_threshold_scaled() -> None:
    ...
    assert (
        abs(result.reduced_orders[0]["explain"]["threshold"] - 0.2) < 1e-10
    ), "Threshold should be 0.2"
    assert (
        abs(result.reduced_orders[0]["explain"]["de_risk_scale"] - 0.25) < 1e-10
    ), "De-risk scale should be 0.25"
...
def test_drawdown_full_block() -> None:
    ...
    peak_equity = 10000.0
    config = PreTradeConfig(
        drawdown_threshold=0.2, de_risk_scale=0.0
    )  # 20% threshold, full block
...
def test_missing_equity_skips_check() -> None:
    ...
    assert (
        "drawdown_derisk_check" in result.summary
    ), "Should have skip reason in summary"
    assert (
        result.summary["drawdown_derisk_check"] == "skipped_no_current_equity"
    ), "Should skip due to missing current_equity"
    ...
    assert (
        result2.summary["drawdown_derisk_check"] == "skipped_no_peak_equity"
    ), "Should skip due to missing peak_equity"
