...
def normalize_signals(
    ...
    if value_col not in signals.columns:
        raise ValueError(
            f"Signal column '{value_col}' not found. Available columns: {list(signals.columns)}"
        )

    if method not in ("zscore", "rank", "none"):
        raise ValueError(
            f"Unknown normalization method: {method}. Must be one of: zscore, rank, none"
        )
...
def make_signal_frame(
    ...
    if not isinstance(raw_scores.index, pd.DatetimeIndex):
        raise ValueError(
            f"raw_scores must have DatetimeIndex, got {type(raw_scores.index)}"
        )
...
def validate_signal_frame(
    ...
    if not isinstance(signal_df.index, pd.DatetimeIndex):
        error_msg = (
            f"SignalFrame index must be DatetimeIndex, got {type(signal_df.index)}"
        )
        if strict:
    ...
    if duplicate_mask.any():
        ...
        error_msg = (
            f"SignalFrame contains {n_duplicates} duplicate (timestamp, symbol) pairs"
        )
        if strict:
            raise ValueError(error_msg)
        logger.warning("VALIDATE SIGNAL FRAME: %s", error_msg)
