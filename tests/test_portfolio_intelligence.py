...
class TestRiskBudgeting:
    ...
    def test_risk_parity_with_views(self):
        ...
        result = risk_parity_with_views(
            cov, views_confidence=views, confidence_scale=0.5
        )
        assert len(result.weights) == 4
        # Higher confidence asset should have higher risk budget
        assert (
            result.risk_contributions[symbols[0]]
            > result.risk_contributions[symbols[1]]
        )
...
class TestMultiPeriod:
    ...
    def test_multi_period_optimize_basic(self):
        ...
        mu_path = [
            pd.Series(
                np.random.default_rng(42 + i).normal(0.05, 0.02, 4), index=symbols
            )
            for i in range(3)
        ]
