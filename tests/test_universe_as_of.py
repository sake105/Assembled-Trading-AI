...
def test_universe_change_day_membership_switches() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        history = pd.DataFrame(
            {
                ...
                "end_date": [
                    pd.Timestamp(
                        "2024-06-30", tz="UTC"
                    ),  # Removed on 2024-06-30 (exclusive)
                    None,  # Still active
                    None,  # Still active
                ],
            }
        ...
        store_universe_history(
            history, universe_name="test", root=root, format="parquet"
        )

        # Test: as_of = 2024-06-29 (before AAPL removal)
        as_of_before = pd.Timestamp("2024-06-29", tz="UTC")
        members_before = get_universe_members(
            as_of_before, universe_name="test", root=root
        )
        assert set(members_before) == {
            "AAPL",
            "MSFT",
...
def test_active_symbols_end_date_none() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        members_start = get_universe_members(
            as_of_start, universe_name="test", root=root
        )
...
def test_get_universe_members_utc_normalized() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        members_naive = get_universe_members(
            as_of_naive, universe_name="test", root=root
        )
...
def test_load_universe_history_formats() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert set(loaded_parquet["symbol"]) == {
            "AAPL",
            "MSFT",
        }, "Should load correct symbols"

        # Test: CSV format
        store_universe_history(
            history, universe_name="test_csv", root=root, format="csv"
        )
        loaded_csv = load_universe_history(universe_name="test_csv", root=root)
        assert len(loaded_csv) == 2, "Should load 2 records from CSV"
        assert set(loaded_csv["symbol"]) == {
            "AAPL",
            "MSFT",
        }, "Should load correct symbols"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for symbol in history["symbol"]:
            ...
            if orig["end_date"] is None:
                assert (
                    pd.isna(loaded_row["end_date"]) or loaded_row["end_date"] is None
                ), "Should preserve None end_date"
            else:
