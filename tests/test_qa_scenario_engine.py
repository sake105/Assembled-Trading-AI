...
def test_scenario_baseline_no_shock(synthetic_prices):
    ...
    original_sorted = (
        synthetic_prices[["timestamp", "symbol", "close"]]
        .sort_values(["timestamp", "symbol"])
        .reset_index(drop=True)
    )
    shocked_sorted = (
        shocked_prices[["timestamp", "symbol", "close"]]
        .sort_values(["timestamp", "symbol"])
        .reset_index(drop=True)
    )

    pd.testing.assert_frame_equal(
        original_sorted, shocked_sorted, check_exact=False, rtol=1e-5
    )
...
def test_scenario_equity_crash_basic(synthetic_prices):
    ...
    before_shock = (
        synthetic_prices[synthetic_prices["timestamp"] < shock_start]
        .sort_values(["timestamp", "symbol"])
        .reset_index(drop=True)
    )
    shocked_before = (
        shocked_prices[shocked_prices["timestamp"] < shock_start]
        .sort_values(["timestamp", "symbol"])
        .reset_index(drop=True)
    )
    pd.testing.assert_frame_equal(
...
def test_scenario_equity_crash_with_end(synthetic_prices):
    ...
    synthetic_prices[synthetic_prices["timestamp"] > shock_end]
    shocked_prices[shocked_prices["timestamp"] > shock_end]
...
def test_scenario_vol_spike(synthetic_prices):
    ...
    for symbol in synthetic_prices["symbol"].unique():
        original = (
            synthetic_prices[synthetic_prices["symbol"] == symbol]
            .sort_values("timestamp")
            .reset_index(drop=True)
        )
        shocked = (
            shocked_prices[shocked_prices["symbol"] == symbol]
            .sort_values("timestamp")
            .reset_index(drop=True)
        )
...
def test_scenario_shipping_blockade(synthetic_prices):
    ...
    shipping_before = all_prices[
        (all_prices["symbol"] == "SHIPPING") & (all_prices["timestamp"] >= shock_start)
    ]
    shipping_after = shocked_prices[
        (shocked_prices["symbol"] == "SHIPPING")
    ...
    if len(shipping_before) > 0 and len(shipping_after) > 0:
        ...
        shipping_ratio = (
            shipping_after["close"].iloc[0] / shipping_before["close"].iloc[0]
        )
        ...
        if "SHIP" not in str(non_shipping_symbol).upper():
            non_shipping_before = all_prices[
                (all_prices["symbol"] == non_shipping_symbol)
                & (all_prices["timestamp"] >= shock_start)
            ]
            non_shipping_after = shocked_prices[
                (shocked_prices["symbol"] == non_shipping_symbol)
                & (shocked_prices["timestamp"] >= shock_start)
            ]

            if len(non_shipping_before) > 0 and len(non_shipping_after) > 0:
                non_shipping_ratio = (
                    non_shipping_after["close"].iloc[0]
                    / non_shipping_before["close"].iloc[0]
                )
...
def test_scenario_empty_prices():
    ...
    scenario = Scenario(name="Test", shock_type="equity_crash", shock_magnitude=-0.20)
...
def test_scenario_no_shock_start_end(synthetic_prices):
    ...
    for symbol in synthetic_prices["symbol"].unique():
        original = synthetic_prices[synthetic_prices["symbol"] == symbol]["close"].iloc[
            0
        ]
...
def test_apply_scenario_to_equity_basic(synthetic_equity):
    ...
    scenario = Scenario(
        name="Equity Crash", shock_type="equity_crash", shock_magnitude=-0.20
    )
...
def test_apply_scenario_to_equity_with_window(synthetic_equity):
    ...
    shock_mask = (synthetic_equity.index >= shock_start) & (
        synthetic_equity.index <= shock_end
    )
    if shock_mask.any():
        expected_ratio = 1 + scenario.shock_magnitude
...
def test_apply_scenario_to_equity_vol_spike(synthetic_equity):
    ...
    scenario = Scenario(name="Vol Spike", shock_type="vol_spike", shock_magnitude=2.0)
...
def test_run_scenario_on_equity(synthetic_equity):
    ...
    scenario = Scenario(
        name="Test Crash", shock_type="equity_crash", shock_magnitude=-0.20
    )
