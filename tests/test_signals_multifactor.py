...
def sample_bundle_mixed_directions() -> FactorBundleConfig:
    ...
    return FactorBundleConfig(
        ...
        factors=[
            ...
            FactorConfig(name="rv_20", weight=0.4, direction="negative"),  # Lower volatility is better
...
class TestBuildMultifactorSignal:
    ...
    def test_direction_positive_higher_values_better(
        ...
        for timestamp in sample_factors_df["timestamp"].unique():
            ...
            if len(aapl_score) > 0 and len(googl_score) > 0 and not (np.isnan(aapl_score[0]) or np.isnan(googl_score[0])):
                assert aapl_score[0] > googl_score[0], \
                    f"At {timestamp}, AAPL mf_score ({aapl_score[0]}) should be > GOOGL mf_score ({googl_score[0]})"
    
    def test_direction_negative_inverts_factor(self, sample_factors_df, sample_bundle_mixed_directions):
    ...
    def test_zscore_crosssectional_mean_zero_std_one(self, sample_factors_df, sample_bundle_positive):
        ...
        for factor_name in ["returns_12m", "momentum_12m_excl_1m"]:
            ...
            for timestamp in result.df["timestamp"].unique():
                ...
                if len(ts_data) >= 2:  # Need at least 2 values for std
                    ...
                    std_val = ts_data.std(ddof=0)  # Population std (as used in function)
    ...
    def test_missing_factors_are_logged(self, sample_factors_df, sample_bundle_positive):
    ...
    def test_no_factors_available_raises_error(self, sample_factors_df):
        ...
        bundle = FactorBundleConfig(
            ...
            horizon_days=20,
            factors=[
                FactorConfig(name="nonexistent_factor", weight=1.0, direction="positive"),
            ],
            options=FactorBundleOptions(),
        )
...
class TestSelectTopBottom:
    ...
    def test_basic_functionality(self, sample_factors_df, sample_bundle_positive):
        ...
        mf_result = build_multifactor_signal(sample_factors_df, bundle=sample_bundle_positive)
    ...
    def test_quantile_per_timestamp(self, sample_factors_df, sample_bundle_positive):
        ...
        assert len(timestamps_with_long) > 0, "Should have long flags for at least one timestamp"
        
        # Check that we have short flags for multiple timestamps
        timestamps_with_short = result[result["mf_short_flag"] == 1]["timestamp"].unique()
        assert len(timestamps_with_short) > 0, "Should have short flags for at least one timestamp"
    
    def test_invalid_quantiles_raise_error(self, sample_factors_df, sample_bundle_positive):
