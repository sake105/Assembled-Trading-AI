...
def test_deflated_sharpe_from_returns_edge_cases():
    ...
    # All NaN returns
    returns_nan = pd.Series(
        [np.nan] * 10, index=pd.date_range("2020-01-01", periods=10, freq="D", tz="UTC")
    )
    ...
    returns_constant = pd.Series(
        [0.01] * 100, index=pd.date_range("2020-01-01", periods=100, freq="D", tz="UTC")
    )
