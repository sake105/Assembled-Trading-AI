...
def test_shipping_features_basic(sample_prices):
    ...
    assert pd.api.types.is_numeric_dtype(result["shipping_congestion_score"]) or result["shipping_congestion_score"].isna().any()
...
def test_insider_features_no_events(sample_prices):
    ...
    empty_events = pd.DataFrame(columns=["timestamp", "symbol", "net_shares", "trades_count"])
