...
pytest.importorskip("src.assembled_core.pipeline.precomputed_index")
...
def test_snapshot_as_of_equivalence_with_groupby():
    ...
    for as_of in test_dates:
        ...
        snapshot_index = snapshot_as_of(
            df, index, as_of, use_monotonic_optimization=False
        )
        ...
        pd.testing.assert_frame_equal(
            snapshot_index[["symbol", "timestamp", "close"]]
            .sort_values("symbol")
            .reset_index(drop=True),
            snapshot_groupby[["symbol", "timestamp", "close"]]
            .sort_values("symbol")
            .reset_index(drop=True),
    ...
    df = pd.DataFrame(
        {
            ...
            "timestamp": pd.to_datetime(
                ["2020-01-01", "2020-01-02", "2020-01-01"], utc=True
            ),
    ...
    df = pd.DataFrame(
        {
            "symbol": ["AAPL", "AAPL", "MSFT"],
            "timestamp": pd.to_datetime(
                ["2020-01-02", "2020-01-03", "2020-01-02"], utc=True
            ),
            "close": [100.0, 101.0, 200.0],
        }
    )
...
def test_snapshot_timezone_aware_handling():
    ...
    assert snapshot.iloc[0]["timestamp"] == pd.Timestamp(
        "2020-01-02 09:00", tz="America/New_York"
    )
...
def test_snapshot_naive_timestamps_normalized_to_utc():
    ...
    assert pd.Timestamp(result_timestamp, tz="UTC") == pd.Timestamp(
        "2020-01-02", tz="UTC"
    )
    assert snapshot.iloc[0]["close"] == 101.0
