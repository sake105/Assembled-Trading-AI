...
def test_factor_store_roundtrip() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        factors_compare = factors_compare.sort_values(
            ["timestamp", "symbol"]
        ).reset_index(drop=True)
        factors_orig = factors.sort_values(["timestamp", "symbol"]).reset_index(
            drop=True
        )
...
def test_factor_store_partitioning() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        partition_2023 = factor_partition_path(
            "core_ta", "1d", universe_key, year=2023, root=root
        )
        partition_2024 = factor_partition_path(
            "core_ta", "1d", universe_key, year=2024, root=root
        )
...
def test_factor_store_contracts() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        pd.testing.assert_frame_equal(
            factors_loaded,
            sorted_check,
            "Factors should be sorted by (timestamp, symbol)",
        )
...
def test_factor_store_error_handling() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert (
                "missing required columns" in str(e).lower()
                or "symbol" in str(e).lower()
            )
...
def test_factor_store_append_mode() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        timestamps_2 = pd.date_range(
            "2024-01-03", periods=5, freq="1d", tz="UTC"
        )  # Overlaps with factors_1
        ...
        duplicate_timestamp = timestamps_2[
            0
        ]  # First timestamp in factors_2 (overlaps with factors_1)
