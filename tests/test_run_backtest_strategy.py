...
def test_run_backtest_strategy_smoke(
    ...
    try:
        ...
        report_files = list(
            (tmp_path / "reports").glob("qa_report_trend_baseline_1d_*.md")
        )
...
def test_run_backtest_strategy_invalid_freq(tmp_path: Path, monkeypatch):
    ...
    try:
        script_path = ROOT / "scripts" / "run_backtest_strategy.py"
        result = subprocess.run(
            [sys.executable, str(script_path), "--freq", "invalid"],
            cwd=str(ROOT),
            capture_output=True,
            text=True,
            timeout=30,
...
def test_run_backtest_strategy_with_universe(
    tmp_path: Path, sample_price_file: Path, sample_universe_file: Path, monkeypatch
):
...
def test_run_backtest_strategy_no_costs(
    tmp_path: Path, sample_price_file: Path, monkeypatch
):
...
def test_run_backtest_strategy_custom_costs(
    tmp_path: Path, sample_price_file: Path, monkeypatch
):
...
def test_run_backtest_event_insider_shipping(
    ...
    try:
        ...
        assert (
            "event_insider_shipping" in result.stdout.lower()
            or "Event Strategy" in result.stdout
        )
        assert "Backtest completed" in result.stdout or "Final PF" in result.stdout

        # Verify that event features were loaded/used (should appear in logs)
        assert (
            "insider" in result.stdout.lower()
            or "shipping" in result.stdout.lower()
...
def test_event_strategy_generates_trades_with_sample_events(
    ...
    try:
        ...
        assert (
            "Using sample insider events" in result.stdout
            or "Using sample shipping events" in result.stdout
        )
