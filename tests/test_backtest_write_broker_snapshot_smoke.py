...
def test_backtest_write_broker_snapshot_smoke(tmp_path: Path):
    ...
    assert "broker_snapshot_path" in result.meta, "Meta should contain broker_snapshot_path"
    ...
    assert snapshot_dir.exists(), f"Broker snapshot directory should exist: {snapshot_dir}"
    
    # Verify snapshot JSON file exists (find by pattern)
    snapshot_files = list(snapshot_dir.glob("snapshot_*.json"))
    assert len(snapshot_files) > 0, f"At least one snapshot JSON file should exist in {snapshot_dir}"
