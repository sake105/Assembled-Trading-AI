...
class TestPreTradeChecks:
    ...
    def test_pre_trade_checks_block_position_too_large(self, sample_orders):
        ...
        assert len(result.blocked_reasons) > 0
        assert any("max_notional_per_symbol" in reason for reason in result.blocked_reasons)
    ...
    def test_pre_trade_checks_missing_required_columns(self):
        ...
        assert result.is_ok is False
        assert len(result.blocked_reasons) > 0
        assert any("missing required columns" in reason.lower() for reason in result.blocked_reasons)
