...
def sample_ml_dataset():
    ...
    feature_data = {
        "timestamp": pd.date_range(
            "2024-01-01", periods=n_samples, freq="1D", tz="UTC"
        ),
...
def reference_ml_dataset():
    ...
    feature_data = {
        "timestamp": pd.date_range(
            "2023-01-01", periods=n_samples, freq="1D", tz="UTC"
        ),
        "symbol": ["AAPL"] * (n_samples // 2) + ["MSFT"] * (n_samples // 2),
        "label": np.random.choice([0, 1], size=n_samples),
...
class TestValidationChecks:
    ...
    def test_run_validation_checks_with_metrics(self, sample_ml_dataset: pd.DataFrame):
        ...
        metrics = {"sharpe_ratio": 1.5, "max_drawdown_pct": -15.0, "total_trades": 100}
    ...
    def test_run_validation_checks_without_metrics(
        self, sample_ml_dataset: pd.DataFrame
    ):
...
class TestDriftChecks:
    ...
    def test_run_drift_checks_with_reference(
        self, sample_ml_dataset: pd.DataFrame, reference_ml_dataset: pd.DataFrame
    ):
    ...
    def test_run_drift_checks_without_reference(self, sample_ml_dataset: pd.DataFrame):
        ...
        results = run_drift_checks(
            current_dataset=sample_ml_dataset, reference_dataset=None, psi_threshold=0.2
        )

        assert "feature_drift" in results
        assert (
            results["feature_drift"] is None
            or "overall_severity" in results["feature_drift"]
        )
...
class TestScriptIntegration:
    ...
    def test_script_runs_with_sample_data(
        self,
        tmp_path: Path,
        sample_ml_dataset: pd.DataFrame,
        reference_ml_dataset: pd.DataFrame,
    ):
        ...
        result = subprocess.run(
            [
                ...
                str(script_path),
                "--current-dataset",
                str(current_dataset_file),
                "--reference-dataset",
                str(reference_dataset_file),
                "--output",
                str(output_path),
                "--model-name",
                "test_model",
    ...
    def test_script_without_reference_dataset(
        self, tmp_path: Path, sample_ml_dataset: pd.DataFrame
    ):
