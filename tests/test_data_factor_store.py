...
def test_factor_store_roundtrip(tmp_path: Path) -> None:
    ...
    loaded_sorted = loaded_df[["timestamp", "symbol", "factor_mom", "factor_value"]].sort_values(
        ["timestamp", "symbol"]
    ...
    assert original_keys == loaded_keys, "Loaded DataFrame should have same (timestamp, symbol) pairs"
...
def test_factor_store_list_available(tmp_path: Path) -> None:
    ...
    insider_rows = []
    for date in dates_2022.tolist() + dates_2023.tolist():
        insider_rows.append({"timestamp": date, "symbol": "AAPL", "factor_insider": 0.8})
...
def test_factor_store_point_in_time(tmp_path: Path) -> None:
    ...
    dates = [pd.Timestamp("2022-01-01"), pd.Timestamp("2022-01-02"), pd.Timestamp("2022-01-03")]
    ...
    assert max_timestamp <= pd.Timestamp("2022-01-02"), f"Max timestamp ({max_timestamp}) should be <= 2022-01-02"
    ...
    assert excluded_date not in dates_in_result, "Should not contain data for 2022-01-03 (beyond end date)"
...
def test_factor_store_multiple_groups(tmp_path: Path) -> None:
    ...
    assert "factor_mom" in loaded_df.columns, "Should contain factor_mom from 'ta' group"
    assert "factor_insider" in loaded_df.columns, "Should contain factor_insider from 'alt_insider' group"

    # Verify merged correctly (should have same number of rows as input)
    assert len(loaded_df) == len(dates), "Merged DataFrame should have correct number of rows"
...
def test_factor_store_universe_filtering(tmp_path: Path) -> None:
    ...
    assert symbols_in_result == {"AAPL", "MSFT"}, f"Should only contain AAPL and MSFT, got {symbols_in_result}"
...
def test_factor_store_nonexistent_files(tmp_path: Path) -> None:
    ...
    assert list(loaded_df.columns) == ["timestamp", "symbol"], "Empty DataFrame should have timestamp and symbol columns"
