...
class TestComputeTargetPositions:
    ...
    def test_compute_target_positions_total_capital(self, sample_signals):
        ...
        assert result2["target_qty"].iloc[0] == pytest.approx(result1["target_qty"].iloc[0] * 2.0)
...
class TestComputeTargetPositionsFromTrendSignals:
    ...
    def test_compute_target_positions_from_trend_signals_happy_path(self, sample_trend_signals):
    ...
    def test_compute_target_positions_from_trend_signals_min_score(self, sample_trend_signals):
        """Test filtering by minimum score."""
        result = compute_target_positions_from_trend_signals(sample_trend_signals, min_score=0.6)
        
        # Should only include signals with score >= 0.6: AAPL (0.9), MSFT (0.7)
        assert len(result) == 2
        assert set(result["symbol"]) == {"AAPL", "MSFT"}

    def test_compute_target_positions_from_trend_signals_top_n(self, sample_trend_signals):
        """Test top-N selection."""
        result = compute_target_positions_from_trend_signals(sample_trend_signals, top_n=2)
        
        assert len(result) == 2
        # Should select top 2 by score: AAPL (0.9) and MSFT (0.7)
        assert set(result["symbol"]) == {"AAPL", "MSFT"}

    def test_compute_target_positions_from_trend_signals_score_based(self, sample_trend_signals):
    ...
    def test_compute_target_positions_from_trend_signals_total_capital(self, sample_trend_signals):
        """Test that total_capital is respected."""
        result = compute_target_positions_from_trend_signals(
            sample_trend_signals,
