...
def test_portfolio_report_prefers_freq_specific(tmp_path: Path, monkeypatch, client: TestClient):
    ...
    try:
        ...
        equity_data = {
            "timestamp": pd.to_datetime([datetime(2025, 11, 28, 14, 0, 0) + Timedelta(days=i) for i in range(5)], utc=True),
...
def test_portfolio_report_fallback_to_legacy(tmp_path: Path, monkeypatch, client: TestClient):
