...
def test_append_day_equals_recompute_last_day() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _prices_last_day_only = prices_full[
            prices_full["timestamp"] == timestamps[-1]
        ].copy()  # Only D10 for append
        ...
        factors_last_day = factors_with_history[
            factors_with_history["timestamp"] == timestamps[-1]
        ].copy()
        ...
        factors_loaded_compare = (
            factors_loaded.drop(columns=["date"])
            .sort_values(["timestamp", "symbol"])
            .reset_index(drop=True)
        )
        factors_full_compare = factors_full.sort_values(
            ["timestamp", "symbol"]
        ).reset_index(drop=True)
        ...
        feature_cols = [
            col
            for col in factors_full_compare.columns
...
def test_append_overlaps_overwrite_deterministic() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        prices_overlap = pd.DataFrame(
            {
                "timestamp": [
                    timestamps[3] for _ in symbols
                ],  # D4 (overlaps with base)
                "symbol": symbols,
...
def test_append_mode_is_deterministic() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        factors_first_compare = (
            factors_after_first.drop(columns=["date"])
            .sort_values(["timestamp", "symbol"])
            .reset_index(drop=True)
        )
        factors_second_compare = (
            factors_after_second.drop(columns=["date"])
            .sort_values(["timestamp", "symbol"])
            .reset_index(drop=True)
        )
...
def test_incremental_compute_only_last_session() -> None:
    ...
    factors_last_compare = (
        factors_last.drop(columns=["date"] if "date" in factors_last.columns else [])
        .sort_values(["timestamp", "symbol"])
        .reset_index(drop=True)
    )
    factors_manual_compare = factors_last_manual.sort_values(
        ["timestamp", "symbol"]
    ).reset_index(drop=True)
