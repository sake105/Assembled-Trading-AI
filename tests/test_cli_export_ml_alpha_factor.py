            ...
            all_data.append(
                {
                    ...
                    "returns_12m": np.random.normal(
                        0, 0.1
                    ),  # Additional factor-like column
...
def test_export_ml_alpha_factor_basic(sample_factor_panel_file, tmp_path: Path):
    ...
    ml_alpha_cols = [
        col for col in ml_alpha_panel_df.columns if col.startswith("ml_alpha_")
    ]
    ...
    if len(ml_alpha_valid) > 10:  # Need enough samples for correlation
        correlation = ml_alpha_valid[ml_alpha_col].corr(
...
@pytest.mark.advanced
def test_export_ml_alpha_factor_custom_column_name(
    sample_factor_panel_file, tmp_path: Path
):
...
@pytest.mark.advanced
def test_export_ml_alpha_factor_cli_subcommand(
    sample_factor_panel_file, tmp_path: Path
):
...
@pytest.mark.advanced
def test_export_ml_alpha_factor_preserves_original_columns(
    sample_factor_panel_file, tmp_path: Path
):
...
@pytest.mark.advanced
def test_export_ml_alpha_factor_only_test_samples_have_predictions(
    sample_factor_panel_file, tmp_path: Path
):
