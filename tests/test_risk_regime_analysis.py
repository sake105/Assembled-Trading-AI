...
def synthetic_index_returns_with_phases() -> pd.Series:
    ...
    returns.iloc[0:200] = np.random.normal(
        0.001, 0.01, 200
    )  # ~0.1% daily return, 1% daily vol

    # Phase 2: Bear market (Days 200-400) - negative trend, elevated vol
    returns.iloc[200:400] = np.random.normal(
        -0.0005, 0.015, 200
    )  # ~-0.05% daily return, 1.5% daily vol

    # Phase 3: Crisis (Days 400-450) - crash, high vol
    returns.iloc[400:450] = np.random.normal(
        -0.002, 0.025, 50
    )  # ~-0.2% daily return, 2.5% daily vol

    # Phase 4: Reflation (Days 450-550) - strong recovery
    returns.iloc[450:550] = np.random.normal(
        0.002, 0.012, 100
    )  # ~0.2% daily return, 1.2% daily vol

    # Phase 5: Sideways (Days 550-700) - low trend, low vol
    returns.iloc[550:700] = np.random.normal(
        0.0001, 0.008, 150
    )  # ~0.01% daily return, 0.8% daily vol
...
def test_summarize_factor_ic_by_regime_basic():
    ...
    ic_series.iloc[: len(dates) // 2] = np.random.normal(
        0.1, 0.05, len(dates) // 2
    )  # Bull: positive IC
    ic_series.iloc[len(dates) // 2 :] = np.random.normal(
        -0.05, 0.05, len(dates) - len(dates) // 2
    )  # Bear: negative IC
    ...
    bear_count = ic_metrics_df[ic_metrics_df["regime_label"] == "bear"][
        "ic_count"
    ].iloc[0]
