...
def test_backtest_engine_numba_vs_pandas_numerical_compatibility(
    ...
    assert metrics["final_pf"] == pytest.approx(result_numba_2.metrics["final_pf"], abs=1e-9)
    assert metrics["sharpe"] == pytest.approx(result_numba_2.metrics["sharpe"], abs=1e-9, nan_ok=True)
