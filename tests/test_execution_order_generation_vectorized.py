...
from src.assembled_core.qa.backtest_engine import (
    _process_rebalancing_timestamp,
...
def test_rebalancing_timestamp_vectorized() -> None:
    ...
    signal_group_1 = signals[
        signals["timestamp"] == pd.Timestamp("2024-01-01", tz="UTC")
    ]
    orders_1, pos_1, targets_1 = _process_rebalancing_timestamp(
        timestamp=pd.Timestamp("2024-01-01", tz="UTC"),
        signal_group=signal_group_1,
        current_positions=current_positions,
        position_sizing_fn=lambda sig, cap: compute_target_positions(
            sig, total_capital=cap, equal_weight=True
        ),
    ...
    orders_2, pos_2, targets_2 = _process_rebalancing_timestamp(
        ...
        signal_group=signals[
            signals["timestamp"] == pd.Timestamp("2024-01-02", tz="UTC")
        ],
...
def test_position_update_empty_orders() -> None:
    ...
    pd.testing.assert_frame_equal(
        updated.sort_values("symbol").reset_index(drop=True),
