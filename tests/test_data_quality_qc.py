...
def test_negative_price_fails() -> None:
    ...
    negative_issues = [
        issue for issue in report.issues if issue.check == "negative_price"
    ]
...
def test_duplicate_symbol_timestamp_fails() -> None:
    ...
    duplicate_issues = [
        issue for issue in report.issues if issue.check == "duplicate_rows"
    ]
...
def test_missing_sessions_warns_or_fails() -> None:
    ...
    missing_issues = [
        issue for issue in report.issues if issue.check == "missing_sessions"
    ]
    ...
    for issue in missing_issues:
        assert issue.severity in [
            "WARN",
            "FAIL",
        ], "Missing sessions should be WARN or FAIL"
...
def test_zero_volume_anomalies() -> None:
    ...
    zero_volume_issues = [
        issue for issue in report.issues if issue.check == "zero_volume"
    ]
