...
def test_commission_bps_plus_fixed() -> None:
    ...
    model = CommissionModel(
        mode="bps_plus_fixed", commission_bps=5.0, fixed_per_trade=1.0
    )
...
def test_commission_deterministic_no_nans() -> None:
    ...
    commission_cash_finite = compute_commission_cash(
        notional_finite, n_trades_finite, model
    )
...
def test_add_cost_columns_to_trades() -> None:
    ...
    np.testing.assert_array_almost_equal(
        trades_with_costs["commission_cash"].values, expected_commission, decimal=10
    )
