...
def test_run_daily_eod_no_symbols_after_filtering(tmp_path: Path, monkeypatch, capsys):
    ...
    assert (
        "No valid symbols" in output
        or "no price data" in output.lower()
        or "no valid symbols" in output.lower()
    )
...
def test_run_daily_eod_empty_price_data(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(SystemExit) as exc_info:
        run_daily_eod(date_str="2025-01-15", price_file=price_file, output_dir=tmp_path)
