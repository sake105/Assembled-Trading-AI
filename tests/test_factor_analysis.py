...
class TestAddForwardReturns:
    ...
    def test_forward_returns_are_forward_looking(self, sample_price_panel):
        ...
        for symbol in sample_price_panel["symbol"].unique():
            ...
            assert pd.isna(last_row["fwd_return_1d"]), "Last row should have NaN for forward returns"
    ...
    def test_custom_horizon(self, sample_price_panel):
        ...
        for symbol in sample_price_panel["symbol"].unique():
            ...
            assert last_5["fwd_return_5d"].isna().all(), "Last 5 rows should have NaN for 5d forward returns"
    
    def test_custom_column_name(self, sample_price_panel):
        """Test with custom column name."""
        result = add_forward_returns(sample_price_panel, horizon_days=1, col_name="future_return")
    ...
    def test_log_vs_simple_returns(self, sample_price_panel):
        ...
        result_log = add_forward_returns(sample_price_panel, horizon_days=1, return_type="log")
        result_simple = add_forward_returns(sample_price_panel, horizon_days=1, return_type="simple")
...
class TestComputeFactorIc:
    ...
    def test_perfect_correlation_ic(self, factor_panel_with_forward_returns):
        ...
        if len(result) > 0:
            ...
            if len(ic_values) > 0:
                ...
                assert (ic_values > 0.95).all(), f"Perfect factor should have IC ≈ 1, got {ic_values.mean()}"
    ...
    def test_inverse_correlation_ic(self, factor_panel_with_forward_returns):
        ...
        if len(result) > 0:
            ...
            if len(ic_values) > 0:
                ...
                assert (ic_values < -0.95).all(), f"Inverse factor should have IC ≈ -1, got {ic_values.mean()}"
    ...
    def test_ic_per_timestamp(self, factor_panel_with_forward_returns):
        ...
        assert len(result) <= unique_timestamps - 1  # First timestamp has no forward return
    ...
    def test_rank_ic_vs_regular_ic(self, factor_panel_with_forward_returns):
        ...
        if len(result_rank) > 0 and len(result_regular) > 0:
            ...
            if len(rank_ic) > 0 and len(regular_ic) > 0:
                assert (rank_ic > 0.9).all(), "Rank-IC should be high for perfect factor"
...
class TestSummarizeFactorIc:
    ...
    def test_summary_statistics_correct(self, factor_panel_with_forward_returns):
        ...
        if len(summary) > 0:
            ...
            if len(perfect_row) > 0:
                ...
                assert perfect_row["mean_ic"].iloc[0] > 0.8, "Perfect factor should have high mean IC"
                
                # Hit ratio should be high (most days with positive IC)
                assert perfect_row["hit_ratio"].iloc[0] > 0.8, "Perfect factor should have high hit ratio"
    ...
    def test_ic_ir_calculation(self, factor_panel_with_forward_returns):
        ...
        if len(summary) > 0:
            row = summary.iloc[0]
            expected_ir = row["mean_ic"] / row["std_ic"] if row["std_ic"] > 1e-10 else 0.0
            assert abs(row["ic_ir"] - expected_ir) < 1e-6, "IC-IR should equal mean_ic / std_ic"
    ...
    def test_sorting_by_ic_ir(self, factor_panel_with_forward_returns):
        ...
        assert (ic_ir_values == sorted(ic_ir_values, reverse=True)).all(), "Summary should be sorted by IC-IR (descending)"
...
class TestIntegrationWithPhaseAFactors:
    ...
    def test_ic_with_core_ta_factors(self, sample_price_panel):
        ...
        factors_df = add_forward_returns(factors_df, horizon_days=21, col_name="fwd_return_21d")
        ...
        assert len(ic_df) > 0
        assert "returns_12m" in ic_df["factor"].values or "trend_strength_20" in ic_df["factor"].values
...
class TestEdgeCases:
    ...
    def test_single_symbol(self):
        ...
        ic_df = compute_factor_ic(df, factor_cols=["factor1"], fwd_return_col="fwd_return_1d")
