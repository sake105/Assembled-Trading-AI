...
def test_strategy_benchmark_produces_report_and_scoreboard(tmp_path: Path) -> None:
    ...
    assert (bench / "regime_metrics.json").exists() or (bench / "regime_metrics.csv").exists(), "regime_metrics not produced"
    assert (bench / "attribution_summary.json").exists() or (bench / "attribution_summary.csv").exists(), "attribution_summary not produced"
    assert (bench / "anomalies.json").exists(), "anomalies.json not produced"
    assert (bench / "data_quality_summary.json").exists(), "data_quality_summary.json not produced"
    assert (bench / "indicator_exposure_summary.json").exists() or (bench / "indicator_exposure_summary.csv").exists(), "indicator_exposure_summary not produced"
    ...
    for col in ("var_95", "es_95", "pct_days_in_market"):
        assert col in all_keys, f"scoreboard missing extended column {col} (columns: {sorted(all_keys)})"
    for col in ("gross_total_return_est", "net_total_return"):
        assert col in all_keys, f"scoreboard missing profit diagnostic column {col} (columns: {sorted(all_keys)})"
    ...
    assert "columns_synthesized" in run_inputs, "run_inputs.json missing columns_synthesized"
    ...
    if run_inputs.get("synthetic_ohlcv") is True:
        assert "synthetic_ohlcv" in anomaly_types, "run synthesized OHLCV but anomalies.json missing synthetic_ohlcv"
    else:
        assert "data_qc_fail" in anomaly_types or "synthetic_ohlcv" in anomaly_types or len(anomaly_types) > 0, (
...
def test_benchmark_outputs_deterministic_json(tmp_path: Path) -> None:
    ...
    out_root = tmp_path / "system_run"
    subprocess.run(
        [sys.executable, str(SCRIPT), "--output-root", str(out_root), "--synthetic-only", "--quick", "--max-variants", "2"],
        cwd=str(ROOT), timeout=120, capture_output=True, text=True,
    )
    bench = out_root / "benchmark"
    for name in ("anomalies.json", "data_quality_summary.json", "indicator_exposure_summary.json"):
        path = bench / name
        if not path.exists():
            continue
    ...
    if (bench / "indicator_exposure_summary.json").exists():
        ...
        assert raw == reencoded, "indicator_exposure_summary.json must be deterministic (sort_keys, newline)"
