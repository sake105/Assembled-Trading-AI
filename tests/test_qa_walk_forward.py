...
def test_generate_walk_forward_splits_expanding_basic():
    ...
    for i, split in enumerate(splits):
        ...
        assert split.train_end <= split.test_start, "Train should end before test starts"
...
def test_generate_walk_forward_splits_no_splits_error():
    ...
    with pytest.raises(ValueError):  # Accept either "Insufficient data" or "No valid splits"
        config = WalkForwardConfig(
            start_date=pd.Timestamp("2024-01-01", tz="UTC"),
