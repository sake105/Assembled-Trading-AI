...
def test_playbook_main_dry_run(monkeypatch, tmp_path: Path):
    ...
    def mock_run_factor_panel_export(config):
        ...
        pd.DataFrame({"timestamp": [], "symbol": []}).to_parquet(
    ...
    def mock_run_ml_alpha_export(config, factor_panel_path, best_model):
        output_path = (
            config.ml_output_dir / "ml_alpha_factors" / "ml_alpha_panel_test.parquet"
        )
        output_path.parent.mkdir(parents=True, exist_ok=True)
        pd.DataFrame({"timestamp": [], "symbol": []}).to_parquet(
            output_path, index=False
    ...
    summary_files = list(
        (tmp_path / "risk_reports" / "research_summaries").glob("*.md")
    )
