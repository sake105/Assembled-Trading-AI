...
def test_risk_report_contains_regime_section_when_enabled(tmp_path: Path):
    ...
    equity_values = 10000.0 * (1.0 + np.random.RandomState(42).normal(0.001, 0.02, len(dates)).cumsum())
    ...
    if report_file.exists():
        ...
        has_regime_section = (
            "Performance by Regime" in report_content or 
            "Risk by Regime" in report_content or
            "regime" in report_content.lower()
        )
        ...
        if has_regime_section:
            ...
            assert "Performance by Regime" in report_content or "Risk by Regime" in report_content, \
...
def test_risk_report_cli_via_cli_py(tmp_path: Path):
    ...
    assert "error" not in result.stderr.lower() or "argument" not in result.stderr.lower(), \
