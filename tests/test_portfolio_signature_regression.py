...
def test_simulate_with_costs_signature():
    ...
    result = simulate_with_costs(
        orders, start_capital, 0.0, 0.0, 0.0, "1d"
    )

    # Verify return type is tuple with 3 elements
    assert isinstance(result, tuple), "simulate_with_costs should return a tuple"
    assert len(result) == 3, f"simulate_with_costs should return 3 values, got {len(result)}"

    equity, metrics, trades_df = result

    # Verify equity
    assert isinstance(equity, pd.DataFrame), "First return value should be DataFrame (equity)"
    ...
    # Verify trades_df
    assert isinstance(trades_df, pd.DataFrame), "Third return value should be DataFrame (trades_df)"
...
def test_run_portfolio_step_signature(tmp_path: Path):
    ...
    try:
        ...
        assert len(result) == 3, f"run_portfolio_step should return 3 values, got {len(result)}"

        eq_path, rep_path, trades_df = result

        # Verify paths
        assert isinstance(eq_path, Path), "First return value should be Path (eq_path)"
        assert isinstance(rep_path, Path), "Second return value should be Path (rep_path)"
...
def test_simulate_with_costs_trades_df_structure():
    ...
    if not orders.empty:
        ...
        if not trades_df.empty:
            ...
            assert len(trades_df) > 0, "trades_df should have at least one row if orders are provided"
