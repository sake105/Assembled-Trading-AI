...
def test_generate_trend_signals_volume_filter():
    ...
    long_high = len(signals_high_threshold[signals_high_threshold["direction"] == "LONG"])
...
def test_generate_trend_signals_ma_auto_compute():
    ...
    df = df.drop(columns=[col for col in df.columns if col.startswith("ma_")], errors="ignore")
