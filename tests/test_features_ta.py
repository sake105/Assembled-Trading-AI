...
def test_add_all_features():
    ...
    df = create_sample_price_data()
    df = add_all_features(df, ma_windows=(20, 50), atr_window=14, rsi_window=14, include_rsi=True)
    ...
    for col in feature_cols:
        assert df[col].notna().any(), f"Feature column '{col}' should have non-NaN values"
