...
def test_build_ml_dataset_from_backtest(
    sample_prices_with_features: pd.DataFrame, sample_trades: pd.DataFrame
):
    ...
    ta_features = [
        ...
        if col.startswith(("ma_", "rsi_", "log_return", "atr_"))
    ]
    ...
    event_features = [
        col
        for col in dataset.columns
        if col.startswith(("insider_", "shipping_", "congress_", "news_"))
    ]
...
def test_build_ml_dataset_feature_filtering(
    sample_prices_with_features: pd.DataFrame, sample_trades: pd.DataFrame
):
    ...
    feature_cols = [
        col
        for col in dataset_insider.columns
        if col
        not in [
            "label",
            "timestamp",
...
def test_save_ml_dataset(
    sample_prices_with_features: pd.DataFrame,
    ...
    trades = pd.DataFrame(
        {
            ...
            "pnl_pct": [
                ...
                -0.01,
                0.03,
            ],  # 2 successful (>= 0.02), 2 unsuccessful
        }
    )
