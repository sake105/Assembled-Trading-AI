...
class TestBuildCoreTaFactors:
    ...
    def test_multiple_symbols_grouped_correctly(self, sample_price_panel):
        ...
        for symbol in sample_price_panel["symbol"].unique():
            ...
            factor_cols = [c for c in result.columns if c not in sample_price_panel.columns]
            non_null_counts = {col: symbol_data[col].notna().sum() for col in factor_cols}
            
            # At least some factors should have non-null values (except forward returns at the end)
            assert sum(non_null_counts.values()) > 0, f"No non-null factors for {symbol}"
...
class TestMultiHorizonReturns:
    ...
    def test_forward_returns_computation(self, monotonic_price_data):
        ...
        if len(returns_1m) > 0:
            ...
            assert (returns_1m > 0).all(), "Forward returns should be positive for increasing prices"
        
        # Check returns_12m (252 days forward)
        returns_12m = result["returns_12m"].dropna()
        if len(returns_12m) > 0:
            assert (returns_12m > 0).all(), "12-month returns should be positive for increasing prices"
    ...
    def test_forward_returns_nan_at_end(self, sample_price_panel):
        ...
        for symbol in sample_price_panel["symbol"].unique():
            ...
            assert pd.isna(last_row["returns_12m"]), "Last row should have NaN for 12m returns"
            assert pd.isna(last_row["returns_6m"]), "Last row should have NaN for 6m returns"
    ...
    def test_momentum_12m_excl_1m(self, monotonic_price_data):
        ...
        if len(momentum) > 0:
            ...
            assert (momentum > 0).all(), "Momentum should be positive for increasing prices"
...
class TestTrendStrengthFactors:
    ...
    def test_trend_strength_positive_for_uptrend(self, monotonic_price_data):
        ...
        if len(trend_strength_20) > 0:
            ...
            assert positive_ratio > 0.7, "Most trend strength values should be positive for uptrend"
...
class TestShortTermReversal:
    ...
    def test_reversal_zscore_range(self, sample_price_panel):
        ...
        if len(reversal_1d) > 0:
            ...
            assert ratio > 0.9, f"Too many extreme z-scores: {within_range}/{len(reversal_1d)} within [-5, 5]"
    ...
    def test_reversal_symmetry(self, sample_price_panel):
        ...
        if len(symbols) > 1:
            ...
            assert len(set(reversal_values.values())) > 1, "Reversal values should differ between symbols"
...
class TestFactorRelationships:
    ...
    def test_monotonic_price_relationships(self, monotonic_price_data):
        ...
        if len(returns_1m) > 10:
            ...
            assert positive_ratio > 0.8, "Most forward returns should be positive for increasing prices"
        ...
        if len(trend_strength) > 10:
            ...
            assert positive_ratio > 0.7, "Most trend strength should be positive for uptrend"
    ...
    def test_timestamp_sorting_preserved(self, sample_price_panel):
        ...
        for symbol in sample_price_panel["symbol"].unique():
            ...
            assert timestamps.is_monotonic_increasing, f"Timestamps not sorted for {symbol}"
