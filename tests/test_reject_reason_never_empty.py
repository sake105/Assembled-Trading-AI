...
def test_ensure_fill_schema_sets_unknown_when_rejected_and_reason_empty():
    ...
    trades = pd.DataFrame([
        {"timestamp": ts, "symbol": "AAPL", "side": "BUY", "qty": 10.0, "price": 100.0},
    ])
    trades["fill_qty"] = 0.0
...
def test_ensure_fill_schema_rejected_rows_have_non_empty_ascii_reason():
    ...
    ts = pd.Timestamp("2025-01-15 16:00", tz="UTC")
    trades = pd.DataFrame([
        {"timestamp": ts, "symbol": "A", "side": "BUY", "qty": 10.0, "price": 100.0},
...
def test_pipeline_output_rejected_rows_have_non_empty_reason():
    ...
    prices = pd.DataFrame([
        {"timestamp": d, "symbol": "AAPL", "open": 100.0, "high": 101.0, "low": 99.0, "close": 100.0, "volume": 1e6}
        for d in dates
    ])
    orders = pd.DataFrame([
        {"timestamp": dates[0], "symbol": "AAPL", "side": "BUY", "qty": 1000.0, "price": 100.0},
    ])
    ...
    for _, row in rejected.iterrows():
        ...
        assert rr is not None and str(rr).strip() != "", "rejected row must have non-empty reject_reason"
