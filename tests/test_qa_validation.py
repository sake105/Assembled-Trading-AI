...
class TestValidatePerformance:
    ...
    def test_validate_performance_ok(self, good_metrics):
        ...
        result = validate_performance(
            good_metrics, min_sharpe=1.0, max_drawdown=0.20, min_trades=30
        )
    ...
    def test_validate_performance_bad_sharpe(self, bad_metrics):
        ...
        result = validate_performance(
            bad_metrics, min_sharpe=1.0, max_drawdown=0.20, min_trades=30
        )
    ...
    def test_validate_performance_missing_sharpe(self, partial_metrics):
        ...
        result = validate_performance(
            partial_metrics, min_sharpe=1.0, max_drawdown=0.20, min_trades=30
        )
    ...
    def test_validate_performance_missing_all(self):
        ...
        result = validate_performance(
            minimal_metrics, min_sharpe=1.0, max_drawdown=0.20, min_trades=30
        )
        ...
        assert len(result.warnings) > 0
        assert all(
            "not available" in warn.lower() or "missing" in warn.lower()
    ...
    def test_validate_performance_abs_drawdown_without_start_capital(self):
        ...
        result = validate_performance(
            metrics, min_sharpe=1.0, max_drawdown=0.20, min_trades=30
        )
        ...
        assert len(result.warnings) > 0
        assert any(
            "start_capital" in warn.lower() and "missing" in warn.lower()
            for warn in result.warnings
        )
    ...
    def test_validate_performance_abs_drawdown_with_zero_start_capital(self):
        ...
        assert len(result.warnings) > 0
        assert any(
            "start_capital" in warn.lower() and "zero" in warn.lower()
            for warn in result.warnings
        )
        assert any("cannot validate" in warn.lower() for warn in result.warnings)
...
class TestValidateOverfitting:
    ...
    def test_validate_overfitting_fail(self):
        ...
        assert len(result.errors) > 0
        assert any(
            "Sharpe" in err or "overfitting" in err.lower() for err in result.errors
        )
        assert any("0.3" in err for err in result.errors)
    ...
    def test_validate_overfitting_none(self):
        ...
        assert len(result.warnings) > 0
        assert any(
            "not available" in warn.lower() or "cannot validate" in warn.lower()
            for warn in result.warnings
        )
        assert len(result.errors) == 0
...
class TestRunFullModelValidation:
    ...
    def test_run_full_model_validation_multiple_failures(
        self, bad_metrics, dirty_feature_df
    ):
    ...
    def test_run_full_model_validation_no_feature_df(self, good_metrics):
        ...
        assert len(result.warnings) > 0
        assert any(
            "not provided" in warn.lower() or "skipping" in warn.lower()
            for warn in result.warnings
        )
        assert result.metadata["validation_details"]["data_quality"]["is_ok"] is None

    def test_run_full_model_validation_custom_config(
        self, good_metrics, clean_feature_df
    ):
