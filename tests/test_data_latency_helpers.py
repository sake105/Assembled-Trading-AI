...
def test_ensure_event_schema_required_columns() -> None:
    ...
    with pytest.raises(ValueError, match="Missing required columns"):
        ensure_event_schema(
            df_missing, required_cols=["timestamp", "symbol"], strict=True
        )
...
def test_ensure_event_schema_non_strict() -> None:
    ...
    result = ensure_event_schema(
        df, required_cols=["timestamp", "symbol"], strict=False
    )
    ...
    events = pd.DataFrame(
        {
            ...
            "disclosure_date": pd.date_range(
                "2025-01-01", periods=10, freq="D", tz="UTC"
            ),
            "value": range(10),
        }
    )
...
def test_apply_source_latency_basic() -> None:
    ...
    assert (
        result["disclosure_date"] == result["event_date"] + pd.Timedelta(days=2)
    ).all()
...
def test_apply_source_latency_fallback_to_timestamp() -> None:
    ...
    assert "disclosure_date" in result.columns
    assert (
        result["disclosure_date"] == result["timestamp"] + pd.Timedelta(days=3)
    ).all()
...
def test_apply_source_latency_shift_mode() -> None:
    ...
    assert (
        result["disclosure_date"] == events["disclosure_date"] + pd.Timedelta(days=5)
    ).all()
...
def test_apply_source_latency_negative_latency() -> None:
    ...
    assert (
        result["disclosure_date"] == result["event_date"] - pd.Timedelta(days=1)
    ).all()
...
def test_ensure_event_schema_empty_dataframe() -> None:
    ...
    result = ensure_event_schema(
        df_empty, required_cols=["timestamp", "symbol"], strict=False
    )
    assert result.empty
...
def test_filter_events_as_of_timezone_handling() -> None:
    ...
    filtered = filter_events_as_of(
        events, as_of_naive, disclosure_col="disclosure_date"
    )
    ...
    events = pd.DataFrame(
        {
            "event_date": pd.to_datetime(
                ["2025-01-01 10:30:00", "2025-01-02 15:45:00"], utc=True
            ),
            "symbol": ["AAPL", "MSFT"],
        }
    )
