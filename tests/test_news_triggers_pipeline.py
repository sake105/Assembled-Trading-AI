...
from src.assembled_core.pipeline.trading_cycle_v2 import _compute_news_triggers
...
def test_news_triggers_cluster_assigns_same_id_for_similar():
    ...
    df = _events(
        [
            ...
            {
                "symbol": "AAPL",
                "title": "Apple earnings beat expectations this quarter",
                "source_tier": "T1",
            },
            {
                "symbol": "GOOG",
                "title": "Google search revenue decline unrelated topic",
                "source_tier": "T1",
            },
        ]
    )
...
def test_news_triggers_t1_higher_score_than_t3():
    ...
    if "symbol" in result.columns and not result.empty:
        ...
        if not t1_row.empty and not t3_row.empty:
            assert float(t1_row["trigger_score"].iloc[0]) > float(
                t3_row["trigger_score"].iloc[0]
            )
...
def test_news_triggers_burst_bonus_recent_events():
    ...
    df = _events(
        [
            {
                "symbol": "AAPL",
                "title": "Breaking news A",
                "source_tier": "T2",
                "published_utc": now,
            },
            {
                "symbol": "AAPL",
                "title": "Breaking news B",
                "source_tier": "T2",
                "published_utc": now,
            },
        ]
