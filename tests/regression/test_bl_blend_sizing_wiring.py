"""Tier-2 wiring — verify portfolio.bl_sizing.apply_bl_sizing is consumed
as an alternative sizing method in the trading cycle.

Dispatch: ``policy.position_sizing.method == "bl_blend"``. Populates
``result.meta['bl_blend_sizing']`` when active.
"""
...
def test_bl_blend_sizing_dispatch_fires(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.meta.get("sizing_method") == "bl_blend"
    assert "bl_blend_sizing" in result.meta
    assert result.meta["bl_blend_sizing"]["n_symbols"] >= 2
