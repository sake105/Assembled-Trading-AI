...
def test_trading_cycle_vs_legacy_orders_identical() -> None:
    ...
    legacy_sorted = legacy_orders.sort_values(["symbol", "side", "qty"]).reset_index(
        drop=True
    )
    cycle_sorted = cycle_orders.sort_values(["symbol", "side", "qty"]).reset_index(
        drop=True
    )

    # Check structure
    assert set(legacy_sorted.columns) == set(cycle_sorted.columns)
    assert {"timestamp", "symbol", "side", "qty", "price"}.issubset(
        set(legacy_sorted.columns)
    )
...
def test_trading_cycle_orders_structure() -> None:
    ...
    if not orders.empty:
        ...
        assert len(invalid_sides) == 0, f"Invalid side values: {invalid_sides}"
...
def test_trading_cycle_with_three_symbols() -> None:
    ...
    assert len(result.prices_with_features.columns) > len(
        result.prices_filtered.columns
    )
...
def test_trading_cycle_empty_signals() -> None:
    ...
    prices = _create_synthetic_prices(
        symbols=["AAPL"], n_days=2
    )  # Too few days for MA signals
