...
def test_holiday_rejected() -> None:
    ...
    try:
        ...
        holiday_orders = pd.DataFrame(
            {
                "timestamp": [
                    pd.Timestamp("2024-01-01 21:00", tz="UTC")
                ],  # New Year's Day
                "symbol": ["AAPL"],
                "side": ["BUY"],
                "qty": [100.0],
...
def test_5min_within_session_accepted() -> None:
    ...
    try:
        ...
        orders_before_open = pd.DataFrame(
            {
                "timestamp": [
                    session_open_utc - pd.Timedelta(hours=1)
                ],  # 1 hour before open
                "symbol": ["AAPL"],
                "side": ["BUY"],
                "qty": [100.0],
        ...
        orders_after_close = pd.DataFrame(
            {
                "timestamp": [
                    session_close_utc + pd.Timedelta(hours=1)
                ],  # 1 hour after close
                "symbol": ["AAPL"],
                "side": ["BUY"],
                "qty": [100.0],
...
def test_session_gate_deterministic() -> None:
    ...
    try:
        orders = pd.DataFrame(
            {
                "timestamp": [
                    ...
                    pd.Timestamp(
                        "2024-01-02 21:00", tz="UTC"
                    ),  # Tuesday (may be accepted/rejected depending on freq)
                ],
                "symbol": ["AAPL", "MSFT"],
                "side": ["BUY", "SELL"],
                "qty": [100.0, 50.0],
        ...
        pd.testing.assert_frame_equal(
            fills1, fills2, "Session gate should be deterministic"
        )
    except ImportError:
        # Skip if exchange_calendars not available
        pass
