...
class TestTrainMetaModel:
    ...
    def test_train_meta_model_unsupported_model_type(
...
class TestSaveLoadMetaModel:
    ...
    def test_save_and_load_meta_model(
        self, trained_model: MetaModel, tmp_path: pathlib.Path
    ):
    ...
    def test_save_and_load_predictions_match(
        ...
        pd.testing.assert_series_equal(
            y_prob_before, y_prob_after, rtol=1e-5, atol=1e-5
        )
...
class TestPlotCalibrationCurve:
    ...
    def test_plot_calibration_curve_creates_file(self, tmp_path: pathlib.Path):
        ...
        y_true = pd.Series(
            [0, 1, 1, 0, 1, 0, 1, 1, 0, 1] * 10
        )  # More samples for better plot
