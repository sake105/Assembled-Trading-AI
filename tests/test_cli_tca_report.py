...
def test_tca_report_basic(sample_backtest_dir: Path, tmp_path: Path):
    ...
    assert tca_risk_summary_file.exists(), f"TCA risk summary file not found: {tca_risk_summary_file}"
...
@pytest.mark.advanced
def test_tca_report_without_equity_curve(sample_backtest_dir_no_equity: Path, tmp_path: Path):
    ...
    tca_risk_summary_file = output_dir / "tca_risk_summary.csv"
    assert not tca_risk_summary_file.exists(), "Risk summary should not exist without equity curve"
...
def test_tca_report_with_custom_cost_params(sample_backtest_dir: Path, tmp_path: Path):
    ...
    assert total_cost_custom > total_cost_default, "Higher cost parameters should result in higher total costs"
