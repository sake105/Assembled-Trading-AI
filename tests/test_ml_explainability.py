...
def test_compute_model_feature_importance_linear(sample_X_y_linear):
    ...
    factor_value_importance = importance_df[importance_df["feature"] == "factor_value"][
        "importance"
    ].iloc[0]
    factor_quality_importance = importance_df[
    ...
    factor_mom_direction = importance_df[importance_df["feature"] == "factor_mom"][
        "direction"
    ].iloc[0]
    factor_value_direction = importance_df[importance_df["feature"] == "factor_value"][
        "direction"
    ].iloc[0]
    ...
    factor_mom_raw = importance_df[importance_df["feature"] == "factor_mom"][
        "raw_value"
    ].iloc[0]
    assert (
        abs(factor_mom_raw - true_coefs["factor_mom"]) < 0.5
    )  # Allow some variation due to regularization/noise
...
def test_compute_permutation_importance_basic(sample_X_y_linear):
    ...
    factor_mom_importance = perm_importance_df[
        perm_importance_df["feature"] == "factor_mom"
    ]["importance_mean"].iloc[0]
    factor_quality_importance = perm_importance_df[
        perm_importance_df["feature"] == "factor_quality"
    ]["importance_mean"].iloc[0]
    assert factor_mom_importance > factor_quality_importance
...
def test_summarize_feature_importance_global_basic():
    ...
    factor_mom_mean = summary_df[summary_df["feature"] == "factor_mom"][
        "importance_mean"
    ].iloc[0]
    factor_value_mean = summary_df[summary_df["feature"] == "factor_value"][
        "importance_mean"
    ].iloc[0]
    factor_vol_mean = summary_df[summary_df["feature"] == "factor_vol"][
        "importance_mean"
    ].iloc[0]
    ...
    # Check max values
    factor_mom_max = summary_df[summary_df["feature"] == "factor_mom"][
        "importance_max"
    ].iloc[0]
...
def test_summarize_feature_importance_global_missing_columns_raises():
    ...
    with pytest.raises(
        ValueError, match="missing 'importance' or 'importance_mean' column"
    ):
