...
def synthetic_shipping_features() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        "symbol": ["AAPL", "MSFT", "GOOGL", "TSLA"],
        "shipping_congestion_score": [80.0, 45.0, 30.0, 75.0],  # AAPL and TSLA have high congestion
...
def test_compute_shipping_exposure_basic(synthetic_portfolio, synthetic_shipping_features):
    """Test basic shipping exposure computation."""
    exposure = compute_shipping_exposure(synthetic_portfolio, synthetic_shipping_features)
...
def test_compute_shipping_exposure_high_congestion(synthetic_portfolio, synthetic_shipping_features):
...
def test_compute_shipping_exposure_top_routes(synthetic_portfolio, synthetic_shipping_features):
...
def test_compute_shipping_exposure_empty_portfolio():
    ...
    shipping_features = pd.DataFrame({
        "symbol": ["AAPL"],
        "shipping_congestion_score": [80.0]
    })
...
def test_compute_systemic_risk_flags_custom_thresholds():
    ...
    flags = compute_systemic_risk_flags(
        exposure,
        high_congestion_threshold=50.0,
        high_exposure_threshold=0.2
    )
