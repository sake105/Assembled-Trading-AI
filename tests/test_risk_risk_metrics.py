...
def test_compute_basic_risk_metrics_edge_cases():
    ...
    # Single return
    single_return = pd.Series(
        [0.01], index=pd.date_range("2020-01-01", periods=1, freq="D", tz="UTC")
    )
...
def test_compute_risk_by_regime_basic(sample_returns, sample_regime_state_df):
    ...
    bull_periods = len(
        sample_regime_state_df[sample_regime_state_df["regime_label"] == "bull"]
    )
    bear_periods = len(
        sample_regime_state_df[sample_regime_state_df["regime_label"] == "bear"]
    )
    neutral_periods = len(
        sample_regime_state_df[sample_regime_state_df["regime_label"] == "neutral"]
    )
...
def test_compute_risk_by_regime_empty_regime():
    ...
    result = compute_risk_by_regime(
        returns=returns, regime_state_df=empty_regime, freq="1d"
    )

    assert len(result) == 0
    assert all(
        col in result.columns
        for col in ["regime", "n_periods", "mean_return_annualized"]
    )


def test_compute_risk_by_factor_group_basic(
    sample_returns, sample_factor_panel_df, sample_positions
):
...
def test_compute_risk_by_factor_group_no_overlap():
    ...
    returns = pd.Series(
        [0.01, -0.01], index=pd.date_range("2020-01-01", periods=2, freq="D", tz="UTC")
    )
    ...
    assert (
        result.iloc[0]["n_periods"] == 0
        or result.iloc[0]["correlation_with_returns"] is None
    )


def test_compute_risk_by_factor_group_default_groups(
    sample_returns, sample_factor_panel_df, sample_positions
):
