...
from src.assembled_core.portfolio.position_sizing import apply_news_sentiment_weight_adjustment
...
class TestNewsWeightAdjustment:
    ...
    def test_no_news_returns_unchanged(self):
        ...
        result = apply_news_sentiment_weight_adjustment(pos, None, entity_linker=_MockLinker())
        assert list(result["target_weight"]) == list(pos["target_weight"])
    ...
    def test_live_mode_adjusts_up(self):
        ...
        result = apply_news_sentiment_weight_adjustment(
            pos, news, entity_linker=_MockLinker(), max_adjustment=0.10, shadow_only=False
        )
    ...
    def test_unknown_symbol_not_adjusted(self):
        ...
        assert pytest.approx(result.iloc[0]["target_weight"], abs=1e-6) == result.iloc[1]["target_weight"]
