...
def test_build_or_load_factors_cache_hit(tmp_path: Path) -> None:
    ...
    with patch(
        "src.assembled_core.features.factor_store_integration.load_factors"
    ) as mock_load:
        mock_load.return_value = cached_factors

        with patch(
            "src.assembled_core.features.factor_store_integration.store_factors"
        ) as mock_store:
...
def test_build_or_load_factors_cache_miss(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                "src.assembled_core.features.factor_store_integration.add_all_features"
            ) as mock_builder:
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                ...
                assert (
                    store_call_kwargs["universe_key"]
                    == load_call_kwargs["universe_key"]
                )
