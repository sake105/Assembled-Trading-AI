...
def test_paper_orders_submit_single(client: TestClient):
    ...
    order_data = {"symbol": "MSFT", "side": "BUY", "quantity": 20.0, "price": 300.0}
...
def test_paper_positions_aggregate_correctly(client: TestClient):
    ...
    response = client.post(
        "/api/v1/paper/orders",
        json=[{"symbol": "AAPL", "side": "BUY", "quantity": 10.0, "price": 150.0}],
    )
    assert response.status_code == 200
    ...
    response = client.post(
        "/api/v1/paper/orders",
        json=[{"symbol": "AAPL", "side": "SELL", "quantity": 4.0, "price": 151.0}],
    )
    assert response.status_code == 200
...
def test_paper_reset_clears_state(client: TestClient):
    ...
    response = client.post(
        ...
        json=[
            {"symbol": "AAPL", "side": "BUY", "quantity": 10.0, "price": 150.0},
            {"symbol": "GOOGL", "side": "BUY", "quantity": 5.0, "price": 2500.0},
        ],
    )
    assert response.status_code == 200
...
def test_paper_order_invalid_quantity(client: TestClient):
    ...
    )  # Unprocessable Entity (Pydantic validation failed)
...
def test_paper_positions_not_updated_for_rejected_orders(
