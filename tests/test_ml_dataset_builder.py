        ...
        prices = pd.DataFrame(
            {
                ...
                "symbol": ["AAPL"] * 30,
                "close": [
                    100.0 + i * 0.01 for i in range(30)
                ],  # Small trend: 100 -> 100.29
            }
        )
            ...
            pd.DataFrame(
                {
                    ...
                    "symbol": ["MSFT"] * 30,
                    "close": [
                        100.0 + i * 0.02 for i in range(30)
                    ],  # Small upward trend
                }
            )
        )
...
class TestBuildMlDatasetForStrategy:
    ...
    def test_build_ml_dataset_for_strategy_trend_baseline_mini_setup(
        self, tmp_path: Path
    ):
        ...
        feature_cols = [
            ...
            if col.startswith("ma_") or col.startswith("atr_") or col.startswith("rsi_")
        ]
        ...
        assert len(dataset) == len(
            dataset.drop_duplicates(subset=["timestamp", "symbol"])
