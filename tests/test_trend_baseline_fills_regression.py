...
def test_trend_baseline_produces_fills_and_equity_time_series():
    ...
    if result.trades is not None and not result.trades.empty:
        ...
        not_rejected = (result.trades.get("status", pd.Series(dtype=str)) != "rejected").any()
        ...
        rejected = result.trades[result.trades.get("status", pd.Series(dtype=str)) == "rejected"]
        if not rejected.empty:
            assert "reject_reason" in result.trades.columns
            for val in result.trades["reject_reason"].dropna().astype(str):
                assert val == "" or val.isascii(), f"reject_reason must be ASCII-only, got {val!r}"
    else:
...
def test_strict_qty_guard_raises_when_notional_exceeds_2x_capital():
    ...
    bad_orders = pd.DataFrame([
        {"timestamp": pd.Timestamp("2021-01-04", tz="UTC"), "symbol": "A", "side": "BUY", "qty": 10_000.0, "price": 140.0},
    ])
    start_capital = 10000.0
...
def test_reject_reason_present_when_rejected():
    ...
    orders = pd.DataFrame([
        {"timestamp": pd.Timestamp("2021-01-04 21:00", tz="UTC"), "symbol": "A", "side": "BUY", "qty": 10.0, "price": 100.0},
        {"timestamp": pd.Timestamp("2021-01-04 21:00", tz="UTC"), "symbol": "B", "side": "BUY", "qty": 0.0, "price": 50.0},
    ])
