...
def test_run_daily_eod_with_factor_store_flag(tmp_path: Path) -> None:
    ...
    with patch("src.assembled_core.config.settings.get_settings") as mock_get_settings:
        ...
        ) as mock_load_universe:
            mock_load_universe.return_value = prices

            with patch(
                "src.assembled_core.features.factor_store_integration.build_or_load_factors"
            ) as mock_build_or_load:
                mock_build_or_load.return_value = prices.copy()

                with patch(
                    "scripts.run_daily.generate_trend_signals_from_prices"
                ) as mock_signals:
                    ...
                    with patch(
                        ...
                        with patch(
                            "scripts.run_daily.generate_orders_from_signals"
                        ) as mock_orders:
                            ...
                            with patch(
                                ...
                                with patch(
                                    "src.assembled_core.execution.risk_controls.filter_orders_with_risk_controls"
                                ) as mock_risk:
                        ...
                        with patch(
                            ...
                            with patch(
                                ...
                                if "build_factors" in timings_data.get("steps", {}):
                                    build_factors_step = timings_data["steps"][
                                        "build_factors"
                                    ]
