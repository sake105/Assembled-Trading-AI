...
def sample_prices_bull_bear() -> pd.DataFrame:
    ...
    for symbol in symbols:
        ...
        recovery_prices = recovery_base + np.arange(59) * 0.2 + np.random.randn(59) * 0.3
...
def sample_macro_factors() -> pd.DataFrame:
    ...
    for symbol in symbols:
        ...
        all_data.append(pd.DataFrame({
            "timestamp": dates,
            "symbol": symbol,
            "macro_growth_regime": np.concatenate([growth_regime_1, growth_regime_2, growth_regime_3]),
            "macro_inflation_regime": np.concatenate([inflation_regime_1, inflation_regime_2, inflation_regime_3]),
            "macro_risk_aversion_proxy": np.concatenate([risk_aversion_1, risk_aversion_2, risk_aversion_3]),
        }))
...
class TestComputeRegimeTransitionStats:
    ...
    def test_basic_functionality(self, sample_prices_bull_bear, sample_macro_factors, sample_breadth_df, sample_vol_df):
    ...
    def test_transition_probabilities_sum_to_one(self, sample_prices_bull_bear, sample_macro_factors, sample_breadth_df, sample_vol_df):
        ...
        if not transition_stats.empty:
            ...
            for from_regime in transition_stats["from_regime"].unique():
                from_transitions = transition_stats[transition_stats["from_regime"] == from_regime]
                prob_sum = from_transitions["transition_probability"].sum()
                
                # Allow small floating point errors
                assert abs(prob_sum - 1.0) < 0.01, f"Probabilities for {from_regime} sum to {prob_sum}, not 1.0"
    ...
    def test_ic_differs_by_regime(
        ...
        if not ic_by_regime.empty:
            ...
            factor1_bull = ic_by_regime[
                (ic_by_regime["factor"] == "factor1") & (ic_by_regime["regime"] == "bull")
            ]
            factor1_bear = ic_by_regime[
                (ic_by_regime["factor"] == "factor1") & (ic_by_regime["regime"] == "bear")
            ]
            ...
            if not factor1_bull.empty and not factor1_bear.empty:
                ...
                assert mean_ic_bull > mean_ic_bear, f"Expected factor1 IC higher in bull ({mean_ic_bull}) than bear ({mean_ic_bear})"
