        ...
        [
            ...
            },
            {
                "timestamp": pd.Timestamp(
                    "2020-02-29", tz="UTC"
                ),  # Day 60 (15 + 45, but Feb has 29 days in 2020)
                "symbol": "GOOGL",
...
class TestBuildEventWindowPrices:
        ...
        for _, event_row in sample_events.iterrows():
            ...
            if not event_data.empty:
                ...
                if not event_day_row.empty:
                    assert (
                        event_day_row["timestamp"].iloc[0] == event_timestamp
                        or abs(
                            (event_day_row["timestamp"].iloc[0] - event_timestamp).days
                        )
                        <= 1
    ...
    def test_edge_case_events_at_boundary(self, sample_price_panel):
        ...
        last_event = result[
            result["event_timestamp"] == boundary_events.iloc[1]["timestamp"]
        ]
    ...
    def test_custom_column_names(self, sample_price_panel, sample_events):
        ...
        # Rename columns
        prices_renamed = sample_price_panel.rename(
            columns={
                "symbol": "ticker",
                "timestamp": "date",
                "close": "price",
        ...
        events_renamed = sample_events.rename(
            columns={
                "symbol": "ticker",
                "timestamp": "date",
            }
    ...
    def test_log_returns_calculation(self, sample_price_panel, sample_events):
        ...
        for event_id in result["event_id"].unique():
            ...
            for i in range(1, len(returns)):
                if (
                    not pd.isna(returns[i])
                    and not pd.isna(prices[i])
    ...
    def test_abnormal_returns_with_benchmark(self, sample_price_panel, sample_events):
        ...
        for event_id in result["event_id"].unique():
            ...
            for i in range(1, len(event_data)):
                ...
                if not pd.isna(row["event_return"]) and not pd.isna(
                    row["abnormal_return"]
                ):
    ...
    def test_abnormal_returns_with_varying_benchmark(
        ...
        for event_id in windows["event_id"].unique():
            ...
            for i in range(len(event_data)):
                windows.loc[event_data.index[i], "benchmark_price"] = 100.0 * (
                    1.0005**i
                )
...
class TestAggregateEventStudy:
    ...
    def test_average_return_calculation(self, sample_price_panel, sample_events):
        ...
        for rel_day in result["rel_day"].values:
            day_returns = returns[returns["rel_day"] == rel_day][
                "event_return"
            ].dropna()
    ...
    def test_cumulative_return_calculation(self, sample_price_panel, sample_events):
        ...
        assert np.allclose(
            result["cum_ret"].values, cum_ret_manual.values, equal_nan=True
    ...
    def test_confidence_intervals(self, sample_price_panel, sample_events):
        ...
        result = aggregate_event_study(
            returns, use_abnormal=False, confidence_level=0.95
        )
    ...
    def test_edge_case_no_events(self):
        ...
        windows = build_event_window_prices(
            prices, empty_events, window_before=5, window_after=5
        )
