...
def _synthetic_panel(n: int = 300, seed: int = 42) -> pd.DataFrame:
    ...
    for sym in ["AAPL", "MSFT"]:
        for i, d in enumerate(dates):
            ...
            rows.append({"timestamp": d, "symbol": sym, "f0": f0, "f1": f1,
...
class TestTuneModelOptuna:
    def test_basic_ridge(self):
        ...
        exp = MLExperimentConfig(
            label_col="fwd_return_5d", feature_cols=["f0", "f1"],
            n_splits=3, min_train_samples=30,
        )
        best_cfg = tune_model_optuna(
            panel, exp, model_type="ridge", n_trials=5,
        )
    ...
    def test_basic_lasso(self):
        ...
        best_cfg = tune_model_optuna(
            panel, exp, model_type="lasso", n_trials=3,
        )
