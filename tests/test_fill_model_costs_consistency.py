...
def test_total_cost_equals_sum_of_components() -> None:
    ...
    commission_model = CommissionModel(
        mode="bps", commission_bps=1.0, fixed_per_trade=0.0
    )
    spread_model = SpreadModel(adv_window=20, buckets=None, fallback_spread_bps=5.0)
    slippage_model = SlippageModel(
        vol_window=20, k=1.0, min_bps=0.0, max_bps=50.0, fallback_slippage_bps=10.0
    )
...
def test_partial_fills_costs_scale_with_filled_notional() -> None:
    ...
    partial_model = PartialFillModel(
        adv_window=20, participation_cap=0.05, min_fill_qty=0.0
    )
    ...
    filled_notional = (
        fills_with_costs["fill_qty"].iloc[0] * fills_with_costs["fill_price"].iloc[0]
    )
    original_notional = (
        fills_with_costs["qty"].iloc[0] * fills_with_costs["price"].iloc[0]
    )
