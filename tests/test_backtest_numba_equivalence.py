...
def create_synthetic_prices(
    ...
    for date in dates:
        for symbol in symbols:
            ...
            base_price = {"AAPL": 150.0, "MSFT": 300.0, "GOOGL": 2500.0}.get(
                symbol, 100.0
            )
...
def create_simple_signal_fn(ma_fast: int = 5, ma_slow: int = 10):
    ...
    def signal_fn(prices_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol in prices_df["symbol"].unique():
            symbol_prices = prices_df[prices_df["symbol"] == symbol].sort_values(
                "timestamp"
            )
...
def create_simple_position_sizing_fn():
    ...
    def position_sizing_fn(
        signals_df: pd.DataFrame, total_capital: float
    ) -> pd.DataFrame:
...
def test_numba_equivalence_mini_backtest():
    ...
    prices = create_synthetic_prices(
        symbols=symbols, start_date="2020-01-01", num_days=20
    )
    ...
    if result_no_numba.trades is not None and result_with_numba.trades is not None:
        trades_df_no_numba = result_no_numba.trades.sort_values(
            ["timestamp", "symbol"]
        ).reset_index(drop=True)
        trades_df_with_numba = result_with_numba.trades.sort_values(
            ["timestamp", "symbol"]
        ).reset_index(drop=True)
