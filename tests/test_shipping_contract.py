"""Tests for shipping release contract (Sprint 11.E3)."""
...
def test_required_columns_validation() -> None:
    ...
    df = pd.DataFrame({"series_id": ["SHIPPING_INDEX"]})
...
def test_metric_alternative_to_series_id() -> None:
    ...
    assert result["series_id"].iloc[0] == "SHIPPING_INDEX"
...
def test_string_trimming() -> None:
    ...
    assert result["series_id"].iloc[0] == "SHIPPING_INDEX"
    assert result["region"].iloc[0] == "Global"
