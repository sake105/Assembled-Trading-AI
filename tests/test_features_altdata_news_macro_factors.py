...
def sample_news_sentiment_daily() -> pd.DataFrame:
    ...
    return pd.DataFrame(sentiment_data).sort_values(["symbol", "timestamp"]).reset_index(drop=True)
...
def sample_macro_series() -> pd.DataFrame:
    ...
    return pd.DataFrame(macro_data).sort_values(["timestamp", "macro_code"]).reset_index(drop=True)
...
class TestBuildNewsSentimentFactors:
    ...
    def test_sentiment_mean_aggregation(self, sample_price_panel, sample_news_sentiment_daily):
    ...
    def test_sentiment_trend_calculation(self, sample_price_panel, sample_news_sentiment_daily):
    ...
    def test_sentiment_shock_flag(self, sample_price_panel, sample_news_sentiment_daily):
    ...
    def test_empty_sentiment_data(self, sample_price_panel):
        ...
        empty_sentiment = pd.DataFrame(columns=["timestamp", "symbol", "sentiment_score", "sentiment_volume"])
    ...
    def test_custom_lookback_days(self, sample_price_panel, sample_news_sentiment_daily):
...
class TestIntegration:
    ...
    def test_combined_news_and_macro(self, sample_price_panel, sample_news_sentiment_daily, sample_macro_series):
        ...
        combined = news_factors.merge(
            macro_factors[["timestamp", "symbol", "macro_growth_regime", "macro_inflation_regime", "macro_risk_aversion_proxy"]],
            on=["timestamp", "symbol"],
            how="left",
        )
