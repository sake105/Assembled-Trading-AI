...
def test_compute_cost_adjusted_risk_metrics_cost_impact():
    ...
    if (
        metrics_low["cost_impact_sharpe"] is not None
        and metrics_high["cost_impact_sharpe"] is not None
    ):
        assert (
            metrics_high["cost_impact_sharpe"]
            >= metrics_low["cost_impact_sharpe"] - 1e-6
        )
