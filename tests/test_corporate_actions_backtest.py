...
def test_split_adjustment_removes_price_drop(tmp_path):
    ...
    pre_split_close = adjusted.loc[adjusted["timestamp"] == pd.Timestamp("2024-01-31"), "close"].iloc[0]
    post_split_close = adjusted.loc[adjusted["timestamp"] == pd.Timestamp("2024-02-01"), "close"].iloc[0]
