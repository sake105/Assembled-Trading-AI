        ...
        df = pd.DataFrame(
            {
                "timestamp": pd.Series(
                    [None]
                    + list(pd.date_range("2024-01-02", periods=4, freq="D", tz="UTC"))
                ),
                "symbol": ["AAPL"] * 5,
                "open": [100.0] * 5,
                "high": [105.0] * 5,
...
class TestLoadEodPrices:
    ...
    def test_load_eod_prices_with_sample_data(self, tmp_path: Path):
        ...
        sample_data = pd.DataFrame(
            {
                "timestamp": pd.date_range(
                    "2024-01-01", periods=20, freq="D", tz="UTC"
                ),
...
class TestLoadEodPricesForUniverse:
    ...
    def test_load_eod_prices_for_universe_file_not_found(self, tmp_path: Path):
        ...
        with pytest.raises(FileNotFoundError):
            load_eod_prices_for_universe(
                universe_file=non_existent_universe, price_file=sample_file
            )
    ...
    def test_load_eod_prices_for_universe_with_comments(self, tmp_path: Path):
        ...
        result = load_eod_prices_for_universe(
            universe_file=universe_file, price_file=sample_file
        )
