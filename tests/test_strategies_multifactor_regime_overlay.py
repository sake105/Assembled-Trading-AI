...
def regime_factors_df(regime_price_panel) -> pd.DataFrame:
    ...
    all_data = []
    for (symbol, timestamp), group in regime_price_panel.groupby(
        ["symbol", "timestamp"]
    ):
        symbol_idx = {
            "STOCK1": 4,
...
def regime_vol_df(regime_price_panel) -> pd.DataFrame:
    ...
    for symbol in regime_price_panel["symbol"].unique():
        symbol_df = regime_price_panel[
            regime_price_panel["symbol"] == symbol
...
class TestExposureChangesByRegime:
    ...
    def test_bull_regime_high_exposure(
        ...
        regime_state_df = build_regime_state(
            ...
            macro_factors=regime_factors_df[
                [
                    "timestamp",
                    "symbol",
                    "macro_growth_regime",
        ...
        first_rebalance_signals = signals[
            signals["timestamp"] == first_rebalance_date
        ].copy()
        ...
        if not first_rebalance_signals.empty:
            ...
            if not positions.empty:
                ...
                total_long = positions[positions["target_weight"] > 0][
                    "target_weight"
                ].sum()
                total_short = abs(
                    positions[positions["target_weight"] < 0]["target_weight"].sum()
                )
                net_exposure = positions["target_weight"].sum()

                # Get regime for first rebalance
                regime_for_date = regime_state_df[
                    regime_state_df["timestamp"] == first_rebalance_date
                ]
        ...
        if len(signals["timestamp"].unique()) > 1:
            ...
            if not later_signals.empty:
                ...
                if not positions.empty:
                    ...
                    regime_for_date = regime_state_df[
                        regime_state_df["timestamp"] == later_dates
                    ]
                    if not regime_for_date.empty:
                        regime_label = regime_for_date["regime_label"].iloc[0]
                        ...
                        if regime_label in ["crisis", "bear"]:
                            expected_gross = default_regime_risk_map.get(
                                regime_label, {}
