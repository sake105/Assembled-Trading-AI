...
class TestExperimentTracker:
    ...
    def test_log_metrics_creates_and_appends_csv(self, tmp_path: Path):
        ...
        assert df[df["metric_name"] == "sharpe"]["metric_value"].iloc[
            0
        ] == pytest.approx(1.23)
        assert df[df["metric_name"] == "max_drawdown"]["metric_value"].iloc[
            0
        ] == pytest.approx(-0.15)
        ...
        # Step is read as string from CSV (empty string for None)
        assert str(final_pf_rows["step"].iloc[0]) == "" or pd.isna(
            final_pf_rows["step"].iloc[0]
        )
    ...
    def test_list_runs_returns_all_runs(self, tmp_path: Path):
        ...
        tracker.start_run(name="experiment1", tags=["tag1"])
        tracker.start_run(name="experiment2", tags=["tag2"])
        tracker.start_run(name="experiment3", tags=["tag1", "tag2"])
