...
class TestOperatorOverviewExample:
    ...
    def test_operator_overview_runs_successfully(self, tmp_path: Path, monkeypatch):
        ...
        # Mock settings
        from src.assembled_core.config.settings import get_settings as original_get_settings
        ...
        monkeypatch.setattr("src.assembled_core.config.settings.get_settings", mock_get_settings)
        monkeypatch.setattr("notebooks.operator_overview_example.get_settings", mock_get_settings)
        ...
        try:
            ...
            assert "OPERATOR OVERVIEW" in output or "SYSTEM HEALTH CHECK" in output, "Missing header"
            ...
            assert "FINAL SUMMARY" in output or "System Health" in output or "SUMMARY" in output, "Missing final summary"
            
        finally:
            monkeypatch.setattr(config_module, "OUTPUT_DIR", original_output_dir)

    def test_operator_overview_contains_required_patterns(self, tmp_path: Path, monkeypatch):
        ...
        try:
            ...
            found_patterns = [pattern for pattern in required_patterns if pattern in output]
            
            # At least some patterns should be present (depending on whether data is available)
            assert len(found_patterns) >= 2, f"Too few required patterns found. Found: {found_patterns}. Output: {output[:500]}"
            
        finally:
            monkeypatch.setattr(config_module, "OUTPUT_DIR", original_output_dir)

    def test_operator_overview_no_crash_on_missing_data(self, tmp_path: Path, monkeypatch):
        ...
        try:
            ...
            assert result.returncode in [0, 1], f"Script crashed or exited unexpectedly: {result.returncode}"
            
            # Should produce some output
            assert len(result.stdout) > 0 or len(result.stderr) > 0, "Script produced no output"
