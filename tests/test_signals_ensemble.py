...
class FakeMetaModel:
    ...
    def __init__(
        self, feature_names: list[str], confidence_scores: pd.Series | None = None
    ):
        ...
        if self._confidence_scores is not None:
            if len(self._confidence_scores) == len(X):
                ...
                if len(self._confidence_scores) >= n:
                    ...
                    # Repeat last value
                    repeated = list(self._confidence_scores.values) + [
                        self._confidence_scores.iloc[-1]
                    ] * (n - len(self._confidence_scores))
                    return pd.Series(
                        repeated[:n], index=X.index, name="confidence_score"
                    )
        else:
...
class TestApplyMetaFilter:
    ...
    def test_apply_meta_filter_filters_low_confidence(
        self, sample_signals: pd.DataFrame, sample_features: pd.DataFrame
    ):
        ...
        confidence_scores = pd.Series(
            [0.9, 0.3, 0.5, 0.7, 0.2, 0.6, 0.4, 0.8, 0.1, 0.5]
        )
        meta_model = FakeMetaModel(
            feature_names=["feature_1", "feature_2", "ma_20"],
    ...
    def test_apply_meta_filter_preserves_index(
        self, sample_signals: pd.DataFrame, sample_features: pd.DataFrame
    ):
    ...
    def test_apply_meta_filter_with_join_keys(
        self, sample_signals: pd.DataFrame, sample_features: pd.DataFrame
    ):
...
class TestApplyMetaScaling:
    ...
    def test_apply_meta_scaling_scales_scores(
        self, sample_signals: pd.DataFrame, sample_features: pd.DataFrame
    ):
    ...
    def test_apply_meta_scaling_respects_max_scaling(
        self, sample_signals: pd.DataFrame, sample_features: pd.DataFrame
    ):
    ...
    def test_apply_meta_scaling_filters_low_confidence(
        self, sample_signals: pd.DataFrame, sample_features: pd.DataFrame
    ):
    ...
    def test_apply_meta_scaling_creates_score_if_missing(
