...
class TestRunFactorReport:
    ...
    def test_factors_dataframe_structure(self, sample_price_panel):
        ...
        factor_cols = [col for col in factors_df.columns if any(
            pattern in col for pattern in ["returns_", "momentum_", "trend_strength_", "reversal_"]
        )]
    ...
    def test_vol_liquidity_factor_set(self, sample_price_panel):
        ...
        vov_cols = [col for col in factors_df.columns if col.startswith("vov_")]
    ...
    def test_all_factor_set(self, sample_price_panel):
        ...
        core_factor_cols = [col for col in factors_df.columns if any(
            pattern in col for pattern in ["returns_", "momentum_", "trend_strength_"]
        )]
        assert len(core_factor_cols) > 0, "Should have core factor columns"
        
        # Should have vol factors (if volume data available)
        vol_factor_cols = [col for col in factors_df.columns if col.startswith("rv_")]
...
class TestFactorReportIntegration:
    ...
    def test_core_factors_included(self, sample_price_panel):
        ...
        assert len(found_patterns) > 0, f"Should have factors matching patterns: {expected_patterns}"
