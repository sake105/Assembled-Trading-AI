...
def factor_panel_with_known_ic() -> pd.DataFrame:
    ...
    df["moderate_factor"] = (
        df["fwd_return_5d"].fillna(0.0) * 0.7 + np.random.randn(len(df)) * 0.3
    )
...
class TestAddForwardReturns:
    ...
    def test_multiple_horizons(self, sample_panel_3x40):
        ...
        for horizon in [20, 60]:
            ...
            for symbol in sample_panel_3x40["symbol"].unique():
                symbol_data = result[result["symbol"] == symbol].sort_values(
                    "timestamp"
                )
    ...
    def test_log_vs_simple_returns(self, sample_panel_3x40):
        ...
        result_log = add_forward_returns(
            sample_panel_3x40, horizon_days=5, return_type="log"
        )
        result_simple = add_forward_returns(
            sample_panel_3x40, horizon_days=5, return_type="simple"
        )
        ...
        valid_mask = (
            result_log["fwd_return_5d"].notna() & result_simple["fwd_return_5d"].notna()
        )
        if valid_mask.sum() > 0:
...
class TestSummarizeIcSeries:
    ...
    def test_basic_statistics(self, factor_panel_with_known_ic):
        ...
        if len(perfect_row) > 0:
            ...
            assert (
                perfect_row["hit_ratio"].iloc[0] > 0.7
            )  # Most days should have positive IC
        ...
        for factor_col in ic_df.columns:
            if factor_col.startswith("ic_"):
                ...
                if mean_col in rolling_df.columns and ir_col in rolling_df.columns:
                    ...
                    valid_mask = (
                        rolling_df[mean_col].notna() & rolling_df[ir_col].notna()
                    )
                    if valid_mask.sum() > 0:
                        ...
                        assert not np.isinf(
                            ir_values
                        ).any()  # Should not have inf values
...
class TestExampleFactorAnalysisWorkflow:
    ...
    def test_basic_workflow(self, sample_panel_3x40):
        ...
        assert (
            "fwd_return_20d" in results["data_with_returns"].columns
