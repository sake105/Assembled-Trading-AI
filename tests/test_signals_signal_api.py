...
def test_validate_signal_frame_with_as_of_and_pit_guard():
    ...
    dates = pd.date_range(
        "2024-01-05", periods=3, freq="D", tz="UTC"
    )  # Start after as_of
    ...
    # Should log warning in non-strict mode
    validate_signal_frame(
        signal_df, as_of=as_of, strict=False, feature_source="test_signals"
    )
...
@pytest.mark.advanced
def test_make_signal_frame_with_custom_method(
    sample_raw_scores, sample_signal_metadata
):
