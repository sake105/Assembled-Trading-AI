...
def test_orders_endpoint_with_sample_file(
...
def test_performance_backtest_curve_endpoint_with_sample_file(
    ...
    data = {
        "timestamp": pd.to_datetime(
            [datetime(2025, 11, 28, 14, 0, 0) + Timedelta(days=i) for i in range(10)],
            utc=True,
        ),
...
def test_get_signals_1d_ok(client: TestClient, tmp_path: Path, monkeypatch):
    ...
    data = {
        "timestamp": pd.to_datetime(
            [datetime(2025, 11, 28, 14, 0, 0) + Timedelta(days=i) for i in range(100)],
            utc=True,
        ),
        "symbol": ["AAPL"] * 50 + ["MSFT"] * 50,
        "close": [100.0 + i * 0.1 for i in range(50)]
        + [200.0 + i * 0.1 for i in range(50)],
    }
    df = pd.DataFrame(data)
...
def test_get_qa_status_ok(client: TestClient, tmp_path: Path, monkeypatch):
    ...
    portfolio_df["timestamp"] = portfolio_df["timestamp"].dt.strftime(
        "%Y-%m-%d %H:%M:%S%z"
    )
