...
def test_run_daily_complete_pipeline(sample_price_file, sample_universe_file, tmp_path: Path, monkeypatch):
    ...
    test_args = [
        ...
        "--universe", str(sample_universe_file),
...
def test_run_daily_creates_order_file_structure(sample_price_file, sample_universe_file, tmp_path: Path, monkeypatch):
    ...
    latest_prices = prices_with_features[prices_with_features["timestamp"] == latest_date]
    ...
    if not signals[signals["direction"] == "LONG"].empty:
        ...
        if not targets.empty:
            ...
            if not orders.empty:
                ...
                order_file_path = output_dir / f"orders_{latest_date.date().strftime('%Y%m%d')}.csv"
                ...
                # Verify file exists
                assert order_file.exists(), f"Order file should be created: {order_file}"
                ...
                for col in required_cols:
                    assert col in order_df.columns, f"Order file should have '{col}' column"
                
                # Verify data types
                assert order_df["Ticker"].dtype == object, "Ticker should be string"
                assert order_df["Side"].isin(["BUY", "SELL"]).all(), "Side should be BUY or SELL"
                assert order_df["Quantity"].dtype in [float, "float64"], "Quantity should be numeric"


def test_run_daily_handles_empty_signals(sample_price_file, sample_universe_file, tmp_path: Path):
