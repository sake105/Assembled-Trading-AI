...
class TestBuildEventWindowPrices:
    def test_basic_window(self):
        ...
        result = build_event_window_prices(
            prices, events, window_before=5, window_after=5
        )
    ...
    def test_event_day_is_zero(self):
        ...
        events = _make_events("AAPL", "2024-01-15")
        result = build_event_window_prices(
            prices, events, window_before=3, window_after=3
        )
    ...
    def test_multiple_events(self):
        ...
        events = pd.DataFrame(
            [
                ...
                {
                    "timestamp": pd.Timestamp("2024-01-20", tz="UTC"),
                    "symbol": "AAPL",
                    "event_type": "news",
                },
            ]
    ...
    def test_multiple_symbols(self):
        ...
        result = build_event_window_prices(
            prices, events, window_before=2, window_after=2
        )
    ...
    def test_window_clipped_at_data_boundary(self):
        ...
        result = build_event_window_prices(
            prices, events, window_before=20, window_after=5
        )
        assert not result.empty
...
class TestComputeEventReturns:
    ...
    def test_abnormal_return_requires_benchmark(self):
        ...
        result = compute_event_returns(windows)
        assert (
            "abnormal_return" not in result.columns
            or result["abnormal_return"].isna().all()
        )
