...
def test_risk_report_basic(sample_backtest_dir: Path, tmp_path: Path):
    ...
    assert exposure_csv.exists(), "exposure_timeseries.csv should be created when positions are available"
...
def test_risk_report_with_regime_and_factors(
    ...
    assert regime_csv.exists(), "risk_by_regime.csv should be created when regime file is provided"
    assert factor_csv.exists(), "risk_by_factor_group.csv should be created when factor panel is provided"
...
def test_risk_report_missing_positions(sample_backtest_dir: Path, tmp_path: Path):
    ...
    exposure_csv = output_dir / "exposure_timeseries.csv"
    assert not exposure_csv.exists(), "exposure_timeseries.csv should not be created without positions"
...
def test_risk_report_missing_regime_file(sample_backtest_dir: Path, tmp_path: Path):
    ...
    regime_csv = output_dir / "risk_by_regime.csv"
    assert not regime_csv.exists(), "risk_by_regime.csv should not be created without valid regime file"
...
def test_risk_report_cli_via_subcommand(sample_backtest_dir: Path, tmp_path: Path, monkeypatch):
    ...
    cmd = [
        ...
        "--backtest-dir", str(sample_backtest_dir),
...
def test_risk_report_parquet_files(tmp_path: Path):
    ...
    equity_df = pd.DataFrame({
        "timestamp": dates,
        "equity": 10000.0 * (1 + np.cumsum(np.random.normal(0.001, 0.01, len(dates)))),
    })
