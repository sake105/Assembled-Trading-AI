...
def test_store_and_load_roundtrip(temp_factor_store, sample_factors_df):
    ...
    expected_df = sample_factors_df.sort_values(["timestamp", "symbol"]).reset_index(
        drop=True
    )
...
def test_store_partitions_by_year(temp_factor_store, sample_factors_df):
    ...
    multi_year_df = multi_year_df.sort_values(["timestamp", "symbol"]).reset_index(
        drop=True
    )
...
def test_store_append_mode(temp_factor_store, sample_factors_df):
    ...
    additional_df = additional_df.sort_values(["timestamp", "symbol"]).reset_index(
        drop=True
    )
...
def test_list_available_panels(temp_factor_store, sample_factors_df):
    ...
    # Find our panel
    our_panel = next((p for p in panels if p["universe_key"] == universe_key), None)
    ...
    assert "factor_columns" in our_panel or "factor_columns" in our_panel.get(
        "schema", {}
    )
