        ...
        df = pd.DataFrame(
            {
                ...
                "returns_1m": np.random.normal(
                    0, 0.02, len(dates)
                ),  # Example of other factor-like columns
...
@pytest.mark.advanced
def test_run_model_zoo_for_panel_basic(
    sample_factor_panel_tmpfile: Path, tmp_path: Path
):
...
@pytest.mark.advanced
def test_model_zoo_prefers_regularized_model(
    sample_factor_panel_tmpfile: Path, tmp_path: Path
):
    """
    ...
    if "test_r2_mean" in summary_df.columns:
        ridge_models = summary_df[
            (summary_df["model_type"] == "ridge") & (summary_df["test_r2_mean"].notna())
        ]
    else:
...
@pytest.mark.advanced
def test_ml_model_zoo_cli_subcommand(
    sample_factor_panel_tmpfile: Path, tmp_path: Path, monkeypatch
):
    ...
    cmd = [
        ...
        "ml_model_zoo",
        "--factor-panel-file",
        str(sample_factor_panel_tmpfile),
        "--label-col",
        "fwd_return_20d",
        "--n-splits",
        "3",
