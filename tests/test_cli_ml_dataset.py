...
def test_cli_build_ml_dataset_basic(tmp_path: Path):
    ...
    assert df["label"].dtype in [int, "int64", "int32"], "Label column should be integer"
    assert set(df["label"].unique()).issubset({0, 1}), "Label should only contain 0 or 1"
    
    # Check for required metadata columns
    assert "symbol" in df.columns or "open_time" in df.columns, "Missing metadata columns"
    
    # Check for at least some feature columns
    feature_cols = [c for c in df.columns if c not in ["label", "symbol", "open_time", "open_price", "close_time", "pnl_pct", "horizon_days"]]
...
def test_cli_build_ml_dataset_help():
    ...
    assert "build_ml_dataset" in result.stdout or "Build ML-ready dataset" in result.stdout
