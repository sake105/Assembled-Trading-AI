...
def test_online_hpo_from_param_grid(tmp_path):
    ...
    adapter = OnlineHyperparamAdapter.from_param_grid(grid, state_path=tmp_path / "hpo.json")
...
def test_online_hpo_no_discount_keeps_integer_count(tmp_path):
    ...
    adapter = OnlineHyperparamAdapter(
        arms=[{"x": 1}], state_path=tmp_path / "hpo.json",
    )
    for _ in range(5):
...
def test_news_trade_attribution_tickers_list_match():
    ...
    trade = {"symbol": "MSFT", "opened_at": "2025-06-01T12:00:00Z", "closed_return": 0.01}
    news = pd.DataFrame({
        "event_id": ["E1", "E2"],
...
def test_news_trade_attribution_outside_window_excluded():
    ...
    trade = {"symbol": "AAPL", "opened_at": "2025-06-01T12:00:00Z", "closed_return": 0.01}
    news = pd.DataFrame({
        "event_id": ["OLD"],
        "symbol": ["AAPL"],
    ...
    links = NewsTradeAttributor(pre_window_hours=24, post_window_hours=24).link_trade_to_events(trade, news)
    assert links == []
...
def test_trade_tca_is_buy_formula():
    ...
    tca = compute_trade_tca("T", "X", "buy", 100, execution_price=101.0, arrival_price=100.0)
...
def test_trade_tca_is_sell_formula():
    ...
    tca = compute_trade_tca("T", "X", "sell", 100, execution_price=99.0, arrival_price=100.0)
...
def test_trade_tca_zero_arrival_graceful():
    ...
    tca = compute_trade_tca("T", "X", "buy", 100, execution_price=100.0, arrival_price=0.0)
...
def test_scenario_vol_spike_std_scales_with_multiplier():
    ...
    high = simulate_vol_spike_scenario(baseline, vol_multiplier=5.0, duration=20, seed=1)
...
def test_scenario_crash_mean_reflects_injection():
    ...
    result = simulate_crash_scenario(
        baseline, crash_magnitude=-0.15, recovery_days=10, n_simulations=200,
    )
...
def test_scenario_crash_var_picks_up_shock():
    ...
    result = simulate_crash_scenario(
        baseline, crash_magnitude=-0.20, recovery_days=10, n_simulations=500,
    )
