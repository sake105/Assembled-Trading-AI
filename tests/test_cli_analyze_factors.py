...
def test_analyze_factors_cli_basic(tmp_path: Path, sample_price_data: Path, sample_universe_file: Path, monkeypatch):
    ...
    cmd = [
        ...
        "analyze_factors",
        "--freq", "1d",
        "--symbols-file", str(sample_universe_file),
        "--data-source", "local",
    ...
    if portfolio_file.exists():
        ...
        assert not df.empty, f"Portfolio summary CSV {portfolio_file} should not be empty if it exists"


def test_analyze_factors_direct_call(tmp_path: Path, sample_price_data: Path, sample_universe_file: Path, monkeypatch):
...
def test_analyze_factors_with_vol_liquidity(tmp_path: Path, sample_price_data: Path, sample_universe_file: Path, monkeypatch):
