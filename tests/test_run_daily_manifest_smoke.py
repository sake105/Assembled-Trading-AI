...
def test_daily_manifest_written_with_relative_posix_paths(tmp_path: Path):
    ...
    assert "/" in safe_path_str or safe_path_str == "orders_20250115.csv", "Path should use POSIX slashes"
...
def test_daily_manifest_deterministic_byte_identical(tmp_path: Path):
    ...
    assert content1 == content2, "Manifest should be byte-identical when written twice with same inputs"
...
def test_daily_manifest_all_keys_present(tmp_path: Path):
    ...
    path_fields = ["safe_orders_path", "broker_snapshot_path", "ledger_pack_path", "reconcile_report_path"]
...
def test_daily_manifest_broker_snapshot_import_fields(tmp_path: Path):
    ...
    assert manifest_data["broker_snapshot_file"] is not None, "broker_snapshot_file should be set"
    assert manifest_data["broker_snapshot_import_ok"] is True, "broker_snapshot_import_ok should be True"
    assert manifest_data["broker_snapshot_date"] == "2025-01-15", "broker_snapshot_date should be set"
    assert manifest_data["broker_snapshot_run_id"] == "snapshot_run", "broker_snapshot_run_id should be set"
    ...
    assert not Path(file_str).is_absolute() or file_str == snapshot_file.name, "broker_snapshot_file should be relative or basename"
