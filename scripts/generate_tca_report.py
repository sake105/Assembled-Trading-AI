...
def generate_tca_report(
    ...
    trades_df = load_dataframe(
        trades_file, expected_columns=["timestamp", "symbol", "side", "qty", "price"]
    )
    ...
    logger.info(
        f"Estimated costs: mean={cost_per_trade.mean():.4f}, total={cost_per_trade.sum():.4f}"
    )
    ...
    if equity_file:
        ...
        equity_df = load_dataframe(
            equity_file, expected_columns=["timestamp", "equity"]
        )
    ...
    if equity_df is not None and not equity_df.empty:
        ...
        if len(common_index) > 0:
            ...
            logger.info(
                f"Cost-adjusted metrics computed: net_sharpe={risk_metrics_dict.get('net_sharpe'):.4f}"
            )
...
def write_tca_report_markdown(
    ...
    if not tca_summary_df.empty:
        ...
        if len(tca_summary_df) > 20:
            lines.append(
                f"| ... | ... | ... | ... | ({len(tca_summary_df) - 20} more rows) |"
            )
        lines.append("")
    ...
    if risk_metrics_dict:
        ...
        if gross_sharpe is not None and net_sharpe is not None:
            ...
            lines.append(
                f"| Sharpe Ratio | {gross_sharpe:.4f} | {net_sharpe:.4f} | {diff_sharpe:.4f} |"
            )
        ...
        if gross_sortino is not None and net_sortino is not None:
            ...
            lines.append(
                f"| Sortino Ratio | {gross_sortino:.4f} | {net_sortino:.4f} | {diff_sortino:.4f} |"
            )
        ...
        lines.append("")
        lines.append(
            f"- **Net Annualized Return:** {risk_metrics_dict.get('net_mean_return_annualized', 'N/A')}"
        )
        lines.append(
            f"- **Net Volatility:** {risk_metrics_dict.get('net_vol_annualized', 'N/A')}"
        )
        lines.append(
            f"- **Net Sharpe Ratio:** {risk_metrics_dict.get('net_sharpe', 'N/A')}"
        )
        lines.append(
            f"- **Net Sortino Ratio:** {risk_metrics_dict.get('net_sortino', 'N/A')}"
        )
        lines.append(
            f"- **Net Max Drawdown:** {risk_metrics_dict.get('net_max_drawdown', 'N/A')}"
        )
        lines.append("")
    ...
    lines.append("")
    lines.append(
        f"- `tca_trades.csv`: Detailed TCA for each trade ({len(tca_trades_df)} trades)"
    )
    lines.append(
        f"- `tca_summary.csv`: Daily aggregated TCA metrics ({len(tca_summary_df)} days)"
    )
    if risk_metrics_dict:
