...
def write_ranking_report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(
            "This report consolidates factor analysis results from multiple runs.\n\n"
        )

        f.write("## Combined Score Explanation\n\n")
        f.write("The `combined_score` is a weighted combination of:\n\n")
        f.write(
            "- **60% IC-IR (Information Ratio)**: Measures the consistency of a factor's "
        )
        f.write(
            "predictive power. Higher values indicate more stable factor performance.\n"
        )
        f.write(
            "- **40% Deflated Sharpe Ratio**: Measures the risk-adjusted return of the "
        )
