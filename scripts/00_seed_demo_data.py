...
for s in SYMS:
    ...
    high = np.maximum.reduce(
        [open_, price, open_ * (1 + np.abs(rng.normal(0, 0.0007, len(idx))))]
    )
...
for sym, g in rawu.groupby("symbol"):
    ...
    out = pd.DataFrame(
        {
            "timestamp": o.index.tz_convert(None),
            "symbol": sym,
            "open": o.values,
            "high": h.values,
            "low": l.values,
...
print(
    "[DONE] Seed complete. You can now run sprint8_feature_engineering.py --freq 5min"
)
