...
def cmd_evaluate() -> int:
    ...
    criteria_results = {
        ...
        "total_orders": {"value": total_orders, "pass": total_orders >= criteria["min_trades"]},
    }
        ...
        try:
            ...
            if len(equity_curve) > 1:
                ...
                if daily_returns:
                    ...
                    ) * 100 if len(equity_curve) > 1 else 0.0
    ...
    report_path = PILOT_DIR / f"pilot_verdict_{datetime.now(timezone.utc).strftime('%Y%m%d')}.json"
...
def cmd_status() -> int:
    ...
    print(f"[pilot] Crashes: {crashes} | Verdict: {m.get('verdict', {}).get('decision', 'pending')}")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    group.add_argument("--run-day", action="store_true", help="Run one daily pilot cycle")
    group.add_argument("--evaluate-only", action="store_true", help="Evaluate pilot and emit GO/NO-GO")
    group.add_argument("--status", action="store_true", help="Print current pilot status")
