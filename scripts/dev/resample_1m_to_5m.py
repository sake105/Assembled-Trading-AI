...
def resample_dir(indir: pl.Path) -> pd.DataFrame:
    ...
    for p in files:
        ...
        if not need.issubset({c.lower() for c in df.columns}):
            print(
                f"[RESAMPLE] skip {p.name}: missing cols -> {df.columns.tolist()}",
                file=sys.stderr,
            )
            continue
        ...
        if {"symbol", "timestamp", "close"} - set(d5.columns):
            print(
                f"[RESAMPLE] skip {p.name}: bad shape after resample -> {d5.columns.tolist()}",
                file=sys.stderr,
            )
            continue

        print(
            f"[RESAMPLE] OK {p.name} -> rows={len(d5)} first={d5['timestamp'].min()} last={d5['timestamp'].max()}"
        )
...
def main():
    ...
    print(
        f"[RESAMPLE] DONE rows={len(d5)} symbols={d5['symbol'].nunique()} "
