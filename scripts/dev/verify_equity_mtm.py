...
if not price_path.exists():
    price_path = ROOT / "output/system_run/benchmark/trend_baseline/1y/price_slice.parquet"
...
fills = trades[(trades["fill_qty"].fillna(0) > 0)].copy() if "fill_qty" in trades.columns else trades.copy()
fills["side_sign"] = fills["side"].map({"BUY": 1, "SELL": -1}).fillna(0)
fills["signed_qty"] = fills["fill_qty"] * fills["side_sign"]

qty_by_ts = fills.pivot_table(index="timestamp", columns="symbol", values="signed_qty", aggfunc="sum").fillna(0).sort_index()
holdings = qty_by_ts.cumsum()

px = prices.pivot_table(index="timestamp", columns="symbol", values="close", aggfunc="last").sort_index()
holdings = holdings.reindex(px.index).ffill().fillna(0)

if "cash_delta" in trades.columns:
    trades["cash_delta"] = pd.to_numeric(trades["cash_delta"], errors="coerce").fillna(0)
    cash_delta = trades.groupby("timestamp")["cash_delta"].sum().reindex(px.index).fillna(0)
else:
    fills["notional"] = fills["fill_qty"] * fills["fill_price"]
    fills["cash_delta_est"] = fills["notional"] * (-fills["side_sign"])
    cash_delta = fills.groupby("timestamp")["cash_delta_est"].sum().reindex(px.index).fillna(0)
...
print("equity_mtm_min", float(equity_mtM.min()), "equity_mtm_max", float(equity_mtM.max()))
