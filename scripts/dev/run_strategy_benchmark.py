...
def _schema_ok(path: Path) -> bool:
    try:
        ...
        return "timestamp" in df.columns and "symbol" in df.columns and "close" in df.columns
    except Exception:
        return False


def _pick_dataset(output_root: Path, dataset_path: str | None) -> tuple[str | None, bool]:
    if dataset_path:
        p = Path(dataset_path)
...
def _generate_synthetic(out_path: Path, start: str, end: str) -> None:
    ...
    for sym in symbols:
        for i, d in enumerate(dates):
            ...
            rows.append({"timestamp": d, "symbol": sym, "open": close * 0.99, "high": close * 1.01, "low": close * 0.98, "close": close, "volume": 1_000_000.0})
...
def _ensure_ohlcv(df):
    ...
    if "close" not in df.columns:
        return df, {"columns_synthesized": [], "synthetic_ohlcv": False, "synthetic_volume_value": None}
...
def _validate_variant_wiring(variant: dict) -> list[dict]:
    ...
    if strategy not in SUPPORTED_STRATEGIES:
        issues.append({"variant_id": vid, "horizon": None, "type": "unsupported_strategy", "message": f"Strategy '{strategy}' not in {list(SUPPORTED_STRATEGIES)}"})
    if strategy == "multifactor_long_short":
        bundle = (variant.get("params") or {}).get("bundle_path")
        if not bundle:
            issues.append({"variant_id": vid, "horizon": None, "type": "missing_bundle_path", "message": "multifactor_long_short requires params.bundle_path"})
        else:
            p = Path(bundle)
            if not p.is_absolute():
                p = ROOT / bundle
            if not p.exists():
                issues.append({"variant_id": vid, "horizon": None, "type": "bundle_not_found", "message": f"Bundle not found: {bundle}"})
    return issues
...
def _run_backtest(
    ...
    cmd = [
        sys.executable, str(ROOT / "scripts" / "run_backtest_strategy.py"),
        "--freq", freq, "--price-file", str(price_file), "--strategy", strategy,
        "--start-capital", "10000", "--out", str(run_dir), "--no-ledger",
    ]
    ...
    if strategy == "trend_baseline_regime_gate":
        cmd.extend(["--risk-on-threshold", str(float(params.get("risk_on_threshold", 0)))])
...
def _run_analysis(run_dir: Path, freq: str) -> int:
    r = subprocess.run([
        sys.executable, str(ROOT / "scripts" / "dev" / "analyze_backtest_results.py"),
        "--out", str(run_dir), "--summary-dir", str(run_dir), "--freq", freq,
...
def _enhance_metrics_from_equity(run_dir: Path, freq: str, row: dict) -> dict:
    ...
    try:
        ...
        if len(monthly) > 0:
            ...
            row["pct_profitable_months"] = round(float((monthly > 0).sum() / len(monthly) * 100), 4)
...
def _extend_metrics_from_equity_and_trades(run_dir: Path, freq: str, row: dict) -> dict:
    ...
    import pandas as pd
    for k in ("var_95", "es_95", "pct_days_in_market", "median_holding_period", "avg_holding_period", "win_streak_max", "loss_streak_max", "total_cost_pct", "cost_per_turnover"):
        if k not in row:
            row[k] = None
    eq_path = run_dir / f"equity_curve_{freq}.csv"
    if not eq_path.exists():
    ...
    try:
        ...
        if trades_df is not None and not trades_df.empty and "_date" in trades_df.columns:
            active_days = trades_df["_date"].nunique()
            row["pct_days_in_market"] = round(min(1.0, active_days / total_days) * 100, 4) if total_days else None
            ...
            row["median_holding_period"] = round(float(sym_dates["days"].median()), 2) if len(sym_dates) else None
            row["avg_holding_period"] = round(float(sym_dates["days"].mean()), 2) if len(sym_dates) else None
            # Cost diagnostics
            start_equity = 10000.0
            if "total_cost_cash" in trades_df.columns:
                total_cost = float(trades_df["total_cost_cash"].sum())
                row["total_cost_pct"] = round(total_cost / start_equity * 100, 4) if start_equity else None
                to = row.get("turnover")
                row["cost_per_turnover"] = round(total_cost / to, 6) if (to and to != 0) else None
            ...
            row["pct_days_in_market"] = round((ret != 0).sum() / total_days * 100, 4) if total_days else None
    ...
    for v in to_run:
        ...
        if vid in skipped_variants:
            for horizon_id, _, _ in horizons:
                all_rows.append({"variant_id": vid, "horizon": horizon_id, "total_return": None, "error": "variant_wiring_invalid"})
            continue
        ...
        for horizon_id, start_d, end_d in horizons:
            ...
            try:
                ...
                with (run_dir / "run_inputs.json").open("w", encoding="utf-8", newline="\n") as f:
                ...
                if quick and horizon_id == "1y" and "timestamp" in prices.columns:
                    ...
                    if n_dates < min_bars:
                        ...
                        bar_anomalies.append({"variant_id": vid, "horizon": horizon_id, "type": "too_few_bars_for_warmup", "message": msg})
            ...
            code = _run_backtest(slice_path, run_dir, strategy, freq, commission_bps, bundle_path, params, no_strict_session_gate=no_strict_session_gate)
            if code != 0:
                all_rows.append({"variant_id": vid, "horizon": horizon_id, "total_return": None, "error": "backtest_failed"})
                continue
            _run_analysis(run_dir, freq)
            metrics_path = run_dir / "metrics_summary.json"
            if not metrics_path.exists():
                all_rows.append({"variant_id": vid, "horizon": horizon_id, "total_return": None})
                continue
            ...
            if m.get("cagr") and m.get("max_drawdown_pct") and m.get("max_drawdown_pct") != 0:
                row["calmar_ratio"] = m["cagr"] / abs(m["max_drawdown_pct"]) if m["max_drawdown_pct"] else None
...
def write_scoreboard(bench_root: Path, all_rows: list[dict]) -> None:
    ...
    with (bench_root / "scoreboard.json").open("w", encoding="utf-8", newline="\n") as f:
...
def write_indicator_exposure_summary(bench_root: Path, all_rows: list[dict], variants: list[dict], baseline_id: str = "trend_baseline") -> None:
    ...
    vid_to_components = {v.get("id"): v.get("enabled_components") or [] for v in variants}
    ...
    for r in all_rows:
        ...
        if base_trades is not None and var_trades is not None and base_trades > 0 and indicator_type != "baseline":
        ...
        if base_pct_days is not None and base_pct_days > 0 and var_pct_days is not None and indicator_type != "baseline":
            pct_days_filtered_out = round(100.0 - (float(var_pct_days) / base_pct_days * 100), 2)
        else:
        ...
        if run_dir and (run_dir / "metrics_summary.json").exists():
            try:
                m = json.loads((run_dir / "metrics_summary.json").read_text(encoding="utf-8"))
                mean_on_entry = m.get("mean_indicator_on_entry") or m.get("mean_rsi_on_entry") or m.get("mean_atr_pct_on_entry")
                median_on_entry = m.get("median_indicator_on_entry") or m.get("median_rsi_on_entry") or m.get("median_atr_pct_on_entry")
    ...
    with (bench_root / "indicator_exposure_summary.json").open("w", encoding="utf-8", newline="\n") as f:
        json.dump(out, f, indent=2, sort_keys=True)
        f.write("\n")
    with (bench_root / "indicator_exposure_summary.csv").open("w", encoding="utf-8", newline="\n") as f:
        w = csv.DictWriter(f, fieldnames=keys, extrasaction="ignore")
        w.writeheader()
        w.writerows(out)
...
def write_metrics_extended(bench_root: Path, all_rows: list[dict]) -> None:
    ...
    ext_keys = [
        "variant_id", "horizon", "total_return", "sharpe_ratio", "max_drawdown_pct", "turnover",
        "pct_days_in_market", "median_holding_period", "avg_holding_period", "win_streak_max", "loss_streak_max",
        "var_95", "es_95", "total_cost_pct", "cost_per_turnover", "cost_share_of_return",
        "stability_score", "stability_score_v2", "robustness_score",
        "return_per_turnover", "pct_profitable_months", "median_monthly_return", "worst_month_pct",
    ]
    rows = [{k: r.get(k) for k in ext_keys if k in r} for r in all_rows]
    if not rows:
        return
    keys_used = sorted(set().union(*(set(row.keys()) for row in rows)))
    with (bench_root / "metrics_extended.json").open("w", encoding="utf-8", newline="\n") as f:
        json.dump(rows, f, indent=2, sort_keys=True)
        f.write("\n")
    with (bench_root / "metrics_extended.csv").open("w", encoding="utf-8", newline="\n") as f:
...
def write_ablation(bench_root: Path, all_rows: list[dict], baseline_id: str, ablation_ids: list[str]) -> None:
    ...
    for horizon in ["1y", "3y", "full"]:
        ...
        for aid in ablation_ids:
            ...
            def f(k):
                ...
                try: return round(float(a) - float(b), 6)
            ...
            out.append({
                ...
                "delta_trades": None if base.get("total_trades") is None or alt.get("total_trades") is None else (int(alt.get("total_trades") or 0) - int(base.get("total_trades") or 0)),
            })
    if not out:
        return
    with (bench_root / "ablation_summary.csv").open("w", encoding="utf-8", newline="\n") as f:
...
def write_regime_metrics(
    ...
    for vid, horizon_id, run_dir in variant_horizon_dirs:
        ...
        slope = eq_norm.rolling(roll_win, min_periods=roll_win).apply(lambda x: float((x.iloc[-1] - x.iloc[0]) / roll_win) if len(x) == roll_win else None, raw=False)
        ...
        dd_regime = (eq < peak).astype(str).replace({"True": "in_drawdown", "False": "recovery"}).reindex(ret.index).ffill()
        eq_on_ret = eq.reindex(ret.index).ffill()
        for reg_name, reg_series in [("volatility", vol_regime), ("trend", trend_regime), ("drawdown", dd_regime)]:
            for label in reg_series.dropna().unique():
                ...
                out_rows.append({
                    ...
                    "max_drawdown_pct": round(float(max_dd_pct), 4) if max_dd_pct is not None else None,
                    "bars_count": int(mask.sum()),
                })
    fieldnames = ["variant_id", "horizon", "regime_type", "regime_label", "total_return", "sharpe_ratio", "max_drawdown_pct", "bars_count"]
    with (bench_root / "regime_metrics.json").open("w", encoding="utf-8", newline="\n") as f:
        json.dump(out_rows, f, indent=2, sort_keys=True)
    with (bench_root / "regime_metrics.csv").open("w", encoding="utf-8", newline="\n") as f:
...
def write_attribution_summary(
    ...
    for horizon_id in horizons:
        ...
        for vid in variant_ids:
            ...
            inc_ret_per_turn = (inc_ret / inc_turn) if inc_turn and inc_turn != 0 else None
            out_rows.append({
                "horizon": horizon_id,
                "baseline": baseline_id,
                "variant": vid,
                ...
                "incremental_return_per_turnover": round(inc_ret_per_turn, 6) if inc_ret_per_turn is not None else None,
            })
    attr_fieldnames = ["horizon", "baseline", "variant", "overlap_pct", "incremental_trades", "incremental_return", "incremental_turnover", "incremental_return_per_turnover"]
    with (bench_root / "attribution_summary.json").open("w", encoding="utf-8", newline="\n") as f:
        json.dump(out_rows, f, indent=2, sort_keys=True)
    with (bench_root / "attribution_summary.csv").open("w", encoding="utf-8", newline="\n") as f:
...
def add_robustness_columns(
    ...
    base_1y = rows_by_key.get((baseline_id, "1y")) or rows_by_key.get((baseline_id, "full"))
    ...
    sh_vals = [r.get("sharpe_ratio") for r in valid if r.get("sharpe_ratio") is not None]
    dd_vals = [r.get("max_drawdown_pct") for r in valid if r.get("max_drawdown_pct") is not None]
    to_vals = [r.get("turnover") for r in valid if r.get("turnover") is not None]
    pf_vals = [r.get("profit_factor") for r in valid if r.get("profit_factor") is not None]
    hr_vals = [r.get("hit_rate") for r in valid if r.get("hit_rate") is not None]
    med_vals = [r.get("median_monthly_return") for r in all_rows if r.get("median_monthly_return") is not None]
    ...
    def _rank_pct(vals: list, key_fn, higher_better: bool = True) -> dict:
        ...
        non_none = [(i, key_fn(vals[i])) for i in range(len(vals)) if key_fn(vals[i]) is not None]
    ...
    dd_rank = _rank_pct(all_rows, lambda r: abs(r.get("max_drawdown_pct") or 0), higher_better=False)
    ext_defaults = ("var_95", "es_95", "pct_days_in_market", "median_holding_period", "avg_holding_period", "win_streak_max", "loss_streak_max", "total_cost_pct", "cost_per_turnover")
    out = []
    for i, r in enumerate(all_rows):
        ...
        hr = r.get("hit_rate")
        r["return_per_turnover"] = round(tr / to, 6) if (tr is not None and to is not None and to != 0) else None
        ...
        penalty = 0
        if "cost_2x" in str(r.get("variant_id", "")) and base_1y and r.get("horizon") == "1y":
            if tr is not None and base_return_1y is not None:
                drop = base_return_1y - float(tr)
                penalty = max(0, min(1, drop))
        score = (
            0.20 * _norm(tr, tr_min, tr_max) + 0.20 * _norm(sh, sh_min, sh_max) +
            0.15 * (1 - _norm(abs(dd) if dd else 0, 0, max(dd_abs) if dd_abs else 1)) +
            0.10 * (1 - _norm(to, to_min, to_max)) + 0.10 * _norm(pf, pf_min, pf_max) + 0.10 * _norm(hr, hr_min, hr_max) +
            0.05 * (r.get("pct_profitable_months") or 0) / 100.0 +
            0.05 * _norm(r.get("median_monthly_return"), med_min, med_max) +
            0.05 * (1 - _norm(abs(r.get("worst_month_pct") or 0), 0, 100)) - penalty
        )
        r["robustness_score"] = round(max(0, min(1, score)), 6)
        r["stability_score"] = round((tr_rank.get(i, 0.5) + sh_rank.get(i, 0.5) + dd_rank.get(i, 0.5)) / 3.0, 6)
        out.append(r)
    rows_by_variant = {}
    for r in out:
    ...
    def _stability_v2(r: dict) -> float:
        ...
        returns = [x.get("total_return") for x in same if x.get("total_return") is not None]
        if len(returns) < 2:
...
def write_anomalies(
    ...
    for r in all_rows:
        ...
        if r.get("total_trades") == 0:
            issues.append({"variant_id": vid, "horizon": hor, "type": "zero_trades", "message": "No trades executed"})
        if r.get("total_return") is not None and r.get("total_return") == 0 and (r.get("total_trades") or 0) == 0:
            issues.append({"variant_id": vid, "horizon": hor, "type": "constant_equity", "message": "Zero return and zero trades"})
        tr, to = r.get("total_return"), r.get("turnover")
        if tr is not None and to is not None and to > 0:
            if abs(tr) < 0.001 and to > 100:
                issues.append({"variant_id": vid, "horizon": hor, "type": "turnover_extreme_vs_return", "message": f"Turnover {to} very high vs return {tr}"})
        for k in ["total_return", "sharpe_ratio", "max_drawdown_pct"]:
            v = r.get(k)
            if v is not None and isinstance(v, float) and (v != v):
                issues.append({"variant_id": vid, "horizon": hor, "type": "nan_metric", "message": f"NaN in {k}"})
        dd_dur = r.get("drawdown_duration_max_periods")
        if dd_dur is not None and int(dd_dur) > 200:
            issues.append({"variant_id": vid, "horizon": hor, "type": "extreme_drawdown_duration", "message": f"Drawdown duration {dd_dur} periods very high"})
        ...
        if eq_path.exists():
            try:
                ...
                if "equity" in df.columns and df["equity"].nunique() == 1:
                    issues.append({"variant_id": vid, "horizon": hor, "type": "constant_equity", "message": "Equity curve is constant"})
                if df["equity"].isna().any():
                    issues.append({"variant_id": vid, "horizon": hor, "type": "nan_equity", "message": "NaN in equity curve"})
                ts = pd.to_datetime(df["timestamp"], utc=True)
                if not ts.is_monotonic_increasing and len(ts) > 1:
                    issues.append({"variant_id": vid, "horizon": hor, "type": "inconsistent_timestamps", "message": "Equity timestamps not monotonic"})
            except Exception:
                pass
        req = [run_dir / f"equity_curve_{freq}.csv", run_dir / "metrics_summary.json"]
        for p in req:
            if not p.exists():
                issues.append({"variant_id": vid, "horizon": hor, "type": "missing_required_file", "message": f"Missing {p.name}"})
        qc_path = run_dir / "qc_report.json"
        if qc_path.exists():
            try:
                qc = json.loads(qc_path.read_text(encoding="utf-8"))
                if qc.get("ok") is False and qc.get("summary", {}).get("fail_count", 0) > 0:
                    ...
                    issues.append({"variant_id": vid, "horizon": hor, "type": "data_qc_fail", "message": msg})
            except Exception:
                pass
        ...
        if run_inputs_path.exists():
            try:
                ...
                if ri.get("synthetic_ohlcv") is True:
                    ...
                    issues.append({"variant_id": vid, "horizon": hor, "type": "synthetic_ohlcv", "message": msg})
            except Exception:
                pass
    ...
    if oos_path.exists():
        try:
            ...
            if "large drop = likely overfit" in text:
                issues.append({"variant_id": "OOS", "horizon": "test", "type": "oos_collapse", "message": "OOS report flags large train-to-test drop (possible overfit)"})
        except Exception:
            pass
...
def write_data_quality_summary(
    ...
    import pandas as pd
    summary = {"runs_checked": 0, "missing_values": {}, "duplicate_dates": 0, "non_monotonic_count": 0, "symbol_coverage": [], "date_min": None, "date_max": None}
    for vid, hor, run_dir in variant_horizon_dirs:
        eq_path = run_dir / f"equity_curve_{freq}.csv"
        if not eq_path.exists():
            continue
        try:
            ...
            df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True)
            if df["equity"].isna().any():
                summary["missing_values"][f"{vid}/{hor}"] = int(df["equity"].isna().sum())
            ...
            if summary["date_min"] is None or df["timestamp"].min() < pd.Timestamp(summary["date_min"], tz="UTC"):
                summary["date_min"] = df["timestamp"].min().strftime("%Y-%m-%d")
            if summary["date_max"] is None or df["timestamp"].max() > pd.Timestamp(summary["date_max"], tz="UTC"):
            ...
            if trades_df is not None and "symbol" in trades_df.columns:
                ...
                summary["symbol_coverage"].append({"variant_id": vid, "horizon": hor, "symbol_count": int(n_sym)})
        except Exception:
            pass
    summary["symbol_coverage"] = summary["symbol_coverage"][:50]
    with (bench_root / "data_quality_summary.json").open("w", encoding="utf-8", newline="\n") as f:
...
def run_oos(
    ...
    for v in to_run:
        ...
        run_dir.mkdir(parents=True, exist_ok=True)
        code = _run_backtest(train_path, run_dir, strategy, freq, None, bundle_path, v.get("params"), no_strict_session_gate=no_strict_session_gate)
        if code != 0:
            continue
        _run_analysis(run_dir, freq)
        ...
        if mp.exists():
            with mp.open("r", encoding="utf-8") as f:
                m = json.load(f)
            train_results.append({"variant_id": vid, "total_return": m.get("total_return"), "sharpe_ratio": m.get("sharpe_ratio")})
    # Evaluate top K on test
    top = sorted([r for r in train_results if r.get("total_return") is not None], key=lambda x: (x.get("total_return") or 0), reverse=True)[:5]
    test_results = []
    for r in top:
        vid = r["variant_id"]
        ...
        run_dir.mkdir(parents=True, exist_ok=True)
        code = _run_backtest(test_path, run_dir, v.get("strategy", "trend_baseline"), freq, None, (v.get("params") or {}).get("bundle_path"), v.get("params"), no_strict_session_gate=no_strict_session_gate)
        if code != 0:
            test_results.append({"variant_id": vid, "train_return": r.get("total_return"), "test_return": None, "note": "test_run_failed"})
            continue
        _run_analysis(run_dir, freq)
        mp = run_dir / "metrics_summary.json"
        if mp.exists():
            with mp.open("r", encoding="utf-8") as f:
                m = json.load(f)
            test_results.append({"variant_id": vid, "train_return": r.get("total_return"), "test_return": m.get("total_return"), "test_sharpe": m.get("sharpe_ratio")})
    lines = [
    ...
    for r in top:
        lines.append(f"- {r.get('variant_id')}: return={r.get('total_return')} sharpe={r.get('sharpe_ratio')}")
    lines.append("")
    ...
    for r in test_results:
        ...
        drop = (float(train_ret) - float(tr)) if (train_ret is not None and tr is not None) else None
        stability = " (large drop = likely overfit)" if drop is not None and drop > 0.1 else ""
        lines.append(f"- {r.get('variant_id')}: test_return={tr} test_sharpe={r.get('test_sharpe')}{stability}")
    lines.append("")
    (bench_root / "oos_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def write_benchmark_report(
    ...
    if run_inputs_count > 0:
        ...
        lines.append(f"Data note: OHLCV synthesized for {synth_count}/{run_inputs_count} runs{vol_note}.")
        lines.append("")
    lines.extend([
        "## Dataset",
        "",
    ...
    for r in all_rows:
        if r.get("variant_id") == baseline_id:
            lines.append(f"- {r.get('horizon')}: return={r.get('total_return')} sharpe={r.get('sharpe_ratio')} max_dd={r.get('max_drawdown_pct')} trades={r.get('total_trades')}")
    lines.append("")
    lines.append("## Scoreboard top 10 (by total_return)")
    lines.append("")
    valid = [r for r in all_rows if r.get("total_return") is not None]
    for r in sorted(valid, key=lambda x: (x.get("total_return") or 0), reverse=True)[:10]:
        lines.append(f"- {r.get('variant_id')} {r.get('horizon')}: return={r.get('total_return')} sharpe={r.get('sharpe_ratio')} max_dd={r.get('max_drawdown_pct')}")
    lines.append("")
    ...
    if ab_path.exists():
        with ab_path.open("r", encoding="utf-8") as f:
            ...
            for row in reader:
                lines.append(f"- {row.get('variant')} vs {row.get('baseline')} ({row.get('horizon')}): delta_return={row.get('delta_return')} delta_sharpe={row.get('delta_sharpe')}")
    lines.append("")
    ...
    if oos_sweep_path.exists():
        try:
            ...
            if "Skipped:" in sweep_text:
                lines.append("OOS sweep skipped (insufficient history or split); see benchmark/oos_sweep_report.md.")
            else:
                lines.append("See benchmark/oos_sweep_report.md for filter-sweep OOS (best config on test).")
        except Exception:
    ...
    if (bench_root / "regime_metrics.csv").exists():
        lines.append("See benchmark/regime_metrics.csv. Metrics per volatility/trend/drawdown regime per variant/horizon.")
    else:
    ...
    if anom_path.exists():
        try:
            ...
            if anom:
                for a in anom[:20]:
                    lines.append(f"- [{a.get('variant_id')} {a.get('horizon')}] {a.get('type')}: {a.get('message')}")
                if len(anom) > 20:
                    lines.append(f"- ... and {len(anom) - 20} more (see anomalies.json)")
            else:
    ...
    lines.append("")
    lines.append("Units: total_return as decimal (0.12 = 12%), total_cost_pct in percent (1.5 = 1.5%). gross_total_return_est = total_return + total_cost_pct/100 (estimate). cost_drag_pct = total_cost_pct. return_per_turnover when turnover>0.")
    lines.append("")
    valid_rows = [r for r in all_rows if r.get("total_return") is not None]
    if valid_rows:
        by_ret = sorted(valid_rows, key=lambda x: (x.get("total_return") or 0), reverse=True)[:5]
        by_sharpe = sorted([r for r in valid_rows if r.get("sharpe_ratio") is not None], key=lambda x: (x.get("sharpe_ratio") or 0), reverse=True)[:5]
        by_stability = sorted(valid_rows, key=lambda x: (x.get("stability_score") or 0), reverse=True)[:5]
        lines.append("Top by return: " + ", ".join(f"{r.get('variant_id')}/{r.get('horizon')}" for r in by_ret))
        lines.append("Top by sharpe: " + ", ".join(f"{r.get('variant_id')}/{r.get('horizon')}" for r in by_sharpe))
        lines.append("Top by stability_score: " + ", ".join(f"{r.get('variant_id')}/{r.get('horizon')}" for r in by_stability))
        lines.append("- Cost sensitivity: see trend_baseline_cost_0.5x vs cost_2x; prefer variants that hold up at 2x.")
        by_stability_v2 = sorted(valid_rows, key=lambda x: (x.get("stability_score_v2") or 0), reverse=True)[:5]
        lines.append("Top by stability_score_v2: " + ", ".join(f"{r.get('variant_id')}/{r.get('horizon')}" for r in by_stability_v2))
    else:
        lines.append("No valid runs to summarize.")
    lines.append("")
    lines.append("## Indicator filter impact")
    lines.append("")
    base_rows = {r.get("horizon"): r for r in all_rows if r.get("variant_id") == baseline_id}
    for r in all_rows:
        vid, hor = r.get("variant_id"), r.get("horizon")
        if vid in ("trend_baseline_rsi_filter", "trend_baseline_vol_filter") and base_rows.get(hor):
            b = base_rows[hor]
            ...
            dcost = (r.get("cost_share_of_return") or 0) - (b.get("cost_share_of_return") or 0)
            lines.append(f"- {vid} ({hor}): delta_trades={dt} delta_turnover={dto} delta_max_dd={ddd} delta_cost_share={dcost}")
    if not any(r.get("variant_id") in ("trend_baseline_rsi_filter", "trend_baseline_vol_filter") for r in all_rows):
        lines.append("No filter variants in this run.")
    lines.append("")
    lines.append("## Filter effectiveness")
    lines.append("")
    lines.append("Metrics: pct_days_filtered_out (share of baseline in-market days blocked), pct_entries_blocked (trades), mean/median_indicator_on_entry when backtest exports them.")
    lines.append("")
        ...
        try:
            ...
            for row in exp:
                if row.get("indicator_type") != "baseline":
                    parts = [f"{row.get('variant_id')} ({row.get('horizon')}): indicator={row.get('indicator_type')}"]
                    if row.get("pct_entries_blocked") is not None:
                        parts.append(f"pct_entries_blocked={row.get('pct_entries_blocked')}")
                    if row.get("pct_days_filtered_out") is not None:
                        parts.append(f"pct_days_filtered_out={row.get('pct_days_filtered_out')}")
                    parts.append(f"total_trades={row.get('total_trades')}")
                    lines.append("- " + " ".join(parts))
            if not any(r.get("indicator_type") != "baseline" for r in exp):
                lines.append("See indicator_exposure_summary.csv for pct_days_filtered_out, pct_entries_blocked, mean/median on entry.")
        except Exception:
    ...
    lines.append("")
    if (bench_root / "regime_metrics.csv").exists():
        lines.append("See benchmark/regime_metrics.csv for performance by volatility/trend/drawdown regime. Compare risk_off drawdown across variants.")
    else:
        lines.append("Regime metrics not run (use --regime).")
    lines.append("")
    ...
    if anom_path.exists():
        try:
            ...
            if anom_list:
                ...
                lines.append("Anomaly summary: " + ", ".join(f"{t}={c}" for t, c in sorted(types.items())))
                lines.append("Next actions: fix zero_trades (universe/signal), constant_equity (data/strategy), turnover_extreme (costs/rebalance), oos_collapse (avoid overfit).")
            else:
                lines.append("None detected.")
        except Exception:
            lines.append("See anomalies.json.")
    ...
    lines.append("")
    lines.append("- Use scoreboard and ablation to pick stable variants; prefer lower cost sensitivity if similar return.")
    lines.append("- Prefer strategies with stable Train vs Test when OOS is run.")
    lines.append("- Default trend_baseline remains production default; event/multifactor for diversification experiments.")
    lines.append("")
    if (bench_root / "filter_sweep_results.json").exists():
        try:
            sweep = json.loads((bench_root / "filter_sweep_results.json").read_text(encoding="utf-8"))
            lines.append("## Filter sweep top 10")
            lines.append("")
            for r in sorted(sweep, key=lambda x: (x.get("total_return") or 0), reverse=True)[:10]:
                lines.append(f"- {r.get('variant_id')} {r.get('param_name')}={r.get('param_value')}: return={r.get('total_return')} sharpe={r.get('sharpe_ratio')}")
            lines.append("")
        except Exception:
...
def _filter_sweep_configs() -> list[tuple]:
    ...
    return [
        ("trend_baseline_rsi_filter", {"rsi_entry": 50, "rsi_overbought": 80}, "rsi_entry", 50),
        ("trend_baseline_rsi_filter", {"rsi_entry": 55, "rsi_overbought": 80}, "rsi_entry", 55),
        ("trend_baseline_rsi_filter", {"rsi_entry": 60, "rsi_overbought": 80}, "rsi_entry", 60),
        ("trend_baseline_vol_filter", {"vol_cap": 0.20}, "vol_cap", 0.20),
        ("trend_baseline_vol_filter", {"vol_cap": 0.30}, "vol_cap", 0.30),
        ("trend_baseline_realized_vol_filter", {"rv_cap": 0.20, "rv_window": 20}, "rv_cap", 0.20),
        ("trend_baseline_realized_vol_filter", {"rv_cap": 0.30, "rv_window": 20}, "rv_cap", 0.30),
        ("trend_baseline_realized_vol_filter", {"rv_cap": 0.40, "rv_window": 20}, "rv_cap", 0.40),
        ("trend_baseline_rsi_vol_combo_filter", {"rsi_entry": 55, "rsi_overbought": 80, "vol_cap": 0.20}, "combo", "55_0.20"),
        ("trend_baseline_rsi_vol_combo_filter", {"rsi_entry": 55, "rsi_overbought": 80, "vol_cap": 0.30}, "combo", "55_0.30"),
        ("trend_baseline_rsi_vol_combo_filter", {"rsi_entry": 60, "rsi_overbought": 80, "vol_cap": 0.30}, "combo", "60_0.30"),
    ]
...
def _run_sweep_on_slice(
    ...
    for strategy, params, param_name, param_value in configs:
        ...
        run_dir.mkdir(parents=True, exist_ok=True)
        code = _run_backtest(str(slice_path), run_dir, strategy, freq, DEFAULT_COMMISSION_BPS, None, params, no_strict_session_gate=no_strict_session_gate)
        if code != 0:
            continue
        _run_analysis(run_dir, freq)
        mp = run_dir / "metrics_summary.json"
...
def run_filter_sweep(
    ...
    results = _run_sweep_on_slice(slice_path, sweep_dir, freq, configs, "1y", no_strict_session_gate=no_strict_session_gate)
    for r in results:
    ...
    with (bench_root / "filter_sweep_results.json").open("w", encoding="utf-8", newline="\n") as f:
        json.dump(results, f, indent=2, sort_keys=True)
        f.write("\n")
    with (bench_root / "filter_sweep_results.csv").open("w", encoding="utf-8", newline="\n") as f:
        w = csv.DictWriter(f, fieldnames=keys, extrasaction="ignore")
        w.writeheader()
        w.writerows(results)
...
def run_oos_sweep(
    ...
    if not one_y:
        ...
        (bench_root / "oos_sweep_report.md").write_text(
            "# OOS filter sweep report\n\nSkipped: " + msg + " Run with dataset that has 1y horizon.\n",
            encoding="utf-8",
        )
    ...
    try:
        ...
        if prices.empty or len(prices) < 100:
            ...
            (bench_root / "oos_sweep_report.md").write_text(
                "# OOS filter sweep report\n\nSkipped: " + msg + " Use dataset with more history or run without --quick.\n",
                encoding="utf-8",
            )
            print(msg, file=sys.stderr)
            return
    ...
    if n < MIN_TRADING_DAYS_FOR_OOS_SPLIT:
        ...
        print(f"OOS sweep: {n} trading days < {MIN_TRADING_DAYS_FOR_OOS_SPLIT}; report written, sweep skipped.", file=sys.stderr)
        return
    ...
    train_df = prices[pd.to_datetime(prices["timestamp"]).dt.date <= trading_dates[split_idx - 1]]
    test_df = prices[pd.to_datetime(prices["timestamp"]).dt.date >= trading_dates[split_idx]]
    ...
    code = _run_backtest(str(train_path), baseline_run, baseline_id, freq, DEFAULT_COMMISSION_BPS, None, {}, no_strict_session_gate=no_strict_session_gate)
    baseline_row = {"variant_id": baseline_id, "horizon": "train", "total_return": None, "sharpe_ratio": None, "max_drawdown_pct": None, "turnover": None, "total_trades": None}
    if code == 0:
        ...
        if mp.exists():
            with mp.open("r", encoding="utf-8") as f:
                m = json.load(f)
            baseline_row.update(total_return=m.get("total_return"), sharpe_ratio=m.get("sharpe_ratio"), max_drawdown_pct=m.get("max_drawdown_pct"), turnover=m.get("turnover"), total_trades=m.get("total_trades"))
    sweep_results = _run_sweep_on_slice(train_path, train_sweep_dir, freq, configs, "train", no_strict_session_gate=no_strict_session_gate)
    train_rows = [baseline_row] + [dict(r) for r in sweep_results]
    train_rows = add_robustness_columns(train_rows, [], baseline_id)
    sweep_with_scores = [r for r in train_rows if r.get("variant_id") != baseline_id and r.get("_sweep_strategy")]
    ...
    code_test = _run_backtest(str(test_path), best_dir, best_strategy, freq, DEFAULT_COMMISSION_BPS, None, best_params, no_strict_session_gate=no_strict_session_gate)
...
def main() -> int:
    ...
    ap.add_argument("--synthetic-only", action="store_true", help="Force synthetic data")
    ap.add_argument("--include-synthetic", action="store_true", help="Use real data if found; else generate synthetic and mark as fallback in report")
    ...
    ap.add_argument("--regime", action="store_true", default=True, help="Compute regime metrics (default on)")
    ap.add_argument("--no-regime", action="store_false", dest="regime", help="Skip regime analysis")
    ap.add_argument("--attribution", action="store_true", default=True, help="Compute attribution vs baseline (default on)")
    ap.add_argument("--no-attribution", action="store_false", dest="attribution", help="Skip attribution")
    ap.add_argument("--max-variants", type=int, default=None)
    ap.add_argument("--max-sweep", type=int, default=5, help="Max variants for OOS sweep")
    ap.add_argument("--sweep-filters", action="store_true", dest="sweep_filters", help="Run filter param sweep (1y only): RSI/vol/rv/combo grid")
    ap.add_argument("--oos-sweep", action="store_true", dest="oos_sweep", help="70/30 train/test: pick best filter by stability_score_v2 on train, report on test")
    ap.add_argument("--no-strict-session-gate", action="store_true", dest="no_strict_session_gate", help="Pass --no-strict-session-gate to backtest (recommended for 1d EOD data with 00:00 UTC timestamps)")
    ap.add_argument("--variants-config", type=Path, default=ROOT / "scripts" / "dev" / "benchmark_variants.json")
    args = ap.parse_args()
    ...
    if not dataset_path:
        print("No dataset and not --synthetic-only / --include-synthetic; abort.", file=sys.stderr)
        return 1
    ...
    if args.attribution and variant_ids and horizons_used:
        write_attribution_summary(bench_root, baseline_id, variant_ids, horizons_used, "1d")
    write_anomalies(bench_root, all_rows, variant_horizon_dirs, "1d", list(wiring_anomalies) + list(bar_anomalies))
    write_data_quality_summary(bench_root, all_rows, variant_horizon_dirs, "1d")
    if getattr(args, "sweep_filters", False):
        run_filter_sweep(bench_root, dataset_path, "1d", no_strict_session_gate=getattr(args, "no_strict_session_gate", False))
    if getattr(args, "oos_sweep", False) and dataset_path and not is_synthetic:
        run_oos_sweep(bench_root, dataset_path, "1d", baseline_id=baseline_id, no_strict_session_gate=getattr(args, "no_strict_session_gate", False))
    if args.oos and dataset_path and not is_synthetic:
        run_oos(output_root, dataset_path, "1d", variants, args.max_sweep, no_strict_session_gate=getattr(args, "no_strict_session_gate", False))
    report_label = "synthetic (fallback - no real data)" if synthetic_fallback else dataset_label
    write_benchmark_report(bench_root, report_label, is_synthetic, all_rows, baseline_id, ablation_ids)
    print("Done.")
    return 0
