...
def validate_parquet_file(parquet_path: Path) -> dict:
    ...
    try:
        ...
        if "timestamp" in df.columns:
            ...
            result["date_min"] = date_min.strftime("%Y-%m-%d") if pd.notna(date_min) else None
            result["date_max"] = date_max.strftime("%Y-%m-%d") if pd.notna(date_max) else None
            
            # Check for data gaps (heuristic: less than 2000 rows for 2000-2025 period)
            if result["rows"] < 2000:
                if result["status"] == "ok":
                    result["status"] = "warning"
                result["message"] = f"Low row count ({result['rows']}) - possible data gaps"
        ...
        if "symbol" in df.columns:
            ...
            if len(unique_symbols) > 1:
                ...
                result["status"] = "warning"
                result["message"] = f"Symbol mismatch: file={unique_symbols[0]}, filename={symbol}"
