...
def main() -> None:
    ...
    print(
        f"    Net shares range: {insider_events['net_shares'].min():.0f} to {insider_events['net_shares'].max():.0f}"
    )
    ...
    print(
        f"    Congestion range: {shipping_events['congestion_score'].min():.0f} to {shipping_events['congestion_score'].max():.0f}"
    )
