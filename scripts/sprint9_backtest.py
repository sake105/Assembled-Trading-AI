...
def main():
    ...
    # Load data
    if args.price_file:
        prices = load_prices(args.freq, price_file=args.price_file, output_dir=OUTPUT_DIR)
        print(f"[BT9] Prices: {args.price_file}  rows={len(prices)}  symbols={prices['symbol'].nunique()}")
    else:
        prices = load_prices_with_fallback(args.freq, output_dir=OUTPUT_DIR)
        print(f"[BT9] Prices: loaded  rows={len(prices)}  symbols={prices['symbol'].nunique()}")
    ...
    # Write results
    curve_path, rep_path = write_backtest_report(eq, metrics, args.freq, output_dir=OUTPUT_DIR)
