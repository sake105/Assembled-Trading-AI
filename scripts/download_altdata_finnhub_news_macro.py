...
Examples:
    ...
    if not (args.download_news or args.download_news_sentiment or args.download_macro):
        parser.error("At least one of --download-news, --download-news-sentiment, or --download-macro must be set")
    ...
    if args.download_macro:
        ...
        if not macro_codes:
            print("Warning: No macro codes provided. Use --macro-codes-file or skip --download-macro")
        else:
            try:
                macro_df = fetch_macro_series(macro_codes, start_date, end_date, settings)
                ...
                if macro_df.empty:
                    ...
                    print(f"  Macro codes: {len(unique_codes)} unique codes: {', '.join(unique_codes[:10])}")
