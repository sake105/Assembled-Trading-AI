...
def load_price_data(
    ...
    if symbols is not None:
        ...
        raise ValueError("Either --symbols, --symbols-file, or --universe must be provided")
    
    if not symbol_list:
        raise ValueError("No symbols provided")
    
    logger.info(f"Loading price data: {len(symbol_list)} symbols, freq={freq}, data_source={data_source}")
...
def load_altdata_events(
    ...
    if earnings_path.exists():
        try:
            ...
            logger.info(f"Loaded {len(events_earnings)} earnings events from {earnings_path}")
    ...
    if insider_path.exists():
        try:
            ...
            logger.info(f"Loaded {len(events_insider)} insider events from {insider_path}")
        except Exception as e:
...
def load_altdata_news_macro(
    ...
    if sentiment_path.exists():
        try:
            ...
            logger.info(f"Loaded {len(news_sentiment_daily)} news sentiment daily records from {sentiment_path}")
        except Exception as e:
    ...
    if macro_path.exists():
        try:
            ...
            logger.info(f"Loaded {len(macro_series)} macro series records from {macro_path}")
        except Exception as e:
...
def compute_factors(
    ...
    if factor_set in ("alt_earnings_insider", "core+alt", "core+alt_full"):
        ...
        if not events_earnings.empty:
            ...
            events_earnings = events_earnings[events_earnings["symbol"].isin(price_symbols)]
            ...
            if not events_earnings.empty:
                ...
                earnings_factor_cols = [col for col in earnings_factors.columns 
                ...
                if earnings_factor_cols:
                    ...
                    logger.info(f"Added {len(earnings_factor_cols)} earnings factor columns")
            else:
                logger.warning("No earnings events found after filtering to price data symbols/date range")
        else:
        ...
        if not events_insider.empty:
            ...
            if "timestamp" in events_insider.columns:
                events_insider["timestamp"] = pd.to_datetime(events_insider["timestamp"], utc=True)
            
            # Filter to symbols in prices
            price_symbols = set(factors_df["symbol"].unique())
            events_insider = events_insider[events_insider["symbol"].isin(price_symbols)]
            ...
            if not events_insider.empty:
                ...
                insider_factor_cols = [col for col in insider_factors.columns 
                                      if col.startswith("insider_")]
                if insider_factor_cols:
                    ...
                    logger.info(f"Added {len(insider_factor_cols)} insider factor columns")
            else:
                logger.warning("No insider events found after filtering to price data symbols/date range")
        else:
    ...
    if factor_set in ("alt_news_macro", "core+alt_news", "core+alt_full"):
        ...
        news_sentiment_daily, macro_series = load_altdata_news_macro(output_dir=output_dir)
        ...
        if not news_sentiment_daily.empty:
            ...
            if "timestamp" in news_sentiment_daily.columns:
                news_sentiment_daily["timestamp"] = pd.to_datetime(news_sentiment_daily["timestamp"], utc=True)
            ...
            if not news_sentiment_daily.empty:
                ...
                news_factor_cols = [col for col in news_factors.columns 
                                   if col.startswith("news_sentiment_")]
                if news_factor_cols:
                    merge_cols = ["timestamp", "symbol"] + news_factor_cols
                    factors_df = factors_df.merge(
                        news_factors[merge_cols],
                        on=["timestamp", "symbol"],
                        how="left"
                    )
                    logger.info(f"Added {len(news_factor_cols)} news sentiment factor columns")
            else:
                logger.warning("No news sentiment data found after filtering to price data date range")
        else:
        ...
        if not macro_series.empty:
            ...
            if "timestamp" in macro_series.columns:
                macro_series["timestamp"] = pd.to_datetime(macro_series["timestamp"], utc=True)
            ...
            if not macro_series.empty:
                ...
                macro_factor_cols = [col for col in macro_factors.columns 
                ...
                if macro_factor_cols:
                    ...
                    logger.info(f"Added {len(macro_factor_cols)} macro regime factor columns")
            else:
                logger.warning("No macro series data found after filtering to price data date range")
        else:
...
def write_factor_analysis_report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        f.write(f"# Factor Analysis Report\n\n")
...
def run_factor_analysis_from_args(args) -> int:
    ...
    try:
        ...
        if args.factor_set in ("alt_earnings_insider", "core+alt", "alt_news_macro", "core+alt_news", "core+alt_full"):
        ...
        exclude_cols = {"timestamp", "symbol", "close", "open", "high", "low", "volume", fwd_return_col}
        factor_cols = [col for col in factors_df.columns if col not in exclude_cols and not col.startswith("fwd_")]
        
        if not factor_cols:
            raise ValueError("No factor columns found after computation")
        
        logger.info(f"Analyzing {len(factor_cols)} factors: {factor_cols[:5]}..." + 
                   (f" (showing first 5 of {len(factor_cols)})" if len(factor_cols) > 5 else ""))
        ...
        summary_rank_ic = summarize_ic_series(rank_ic_df) if not rank_ic_df.empty else pd.DataFrame()
        ...
        ls_returns = build_long_short_portfolio_returns(portfolio_returns) if not portfolio_returns.empty else pd.DataFrame()
        
        # Summarize portfolios
        portfolio_summary = summarize_factor_portfolios(ls_returns) if not ls_returns.empty else pd.DataFrame()
        ...
        if not summary_ic.empty:
            display_df = summary_ic[["factor", "mean_ic", "std_ic", "ic_ir", "hit_ratio", "count"]].copy()
        ...
        if not portfolio_summary.empty:
            display_cols = ["factor", "annualized_return", "annualized_vol", "sharpe", "t_stat", "win_ratio", "max_drawdown"]
...
Examples:
  python scripts/run_factor_analysis.py --freq 1d --symbols-file config/macro_world_etfs_tickers.txt --data-source local --start-date 2010-01-01 --end-date 2025-12-03 --factor-set core+vol_liquidity --horizon-days 20
    ...
    parser.add_argument("--freq", type=str, required=True, choices=["1d", "5min"], help="Frequency")
    ...
    parser.add_argument("--universe", type=str, help="Path to universe file (alias for --symbols-file)")
