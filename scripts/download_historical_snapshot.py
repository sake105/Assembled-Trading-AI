...
def download_symbol_history_finnhub(
    ...
    if interval not in resolution_map:
        raise ValueError(
            f"Unsupported interval for Finnhub: {interval}. Supported: 1d, 5min"
        )
    ...
    end_dt = (
        pd.to_datetime(end_date, utc=True) if end_date else pd.Timestamp.now(tz="UTC")
    )
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.info(
                f"Downloaded {len(df)} rows for {symbol} from Finnhub (attempt {attempt}/{max_retries})"
            )
            return df
            ...
            if attempt < max_retries:
                ...
                raise RuntimeError(
                    f"Failed to download {symbol} from Finnhub after {max_retries} attempts: {e}"
                ) from e
        except Exception as e:
            if attempt < max_retries:
                logger.warning(
...
def download_symbol_history_twelve_data(
    ...
    if interval not in interval_map:
        raise ValueError(
            f"Unsupported interval for Twelve Data: {interval}. Supported: 1d, 5min"
        )
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.info(
                f"Downloaded {len(df)} rows for {symbol} from Twelve Data (attempt {attempt}/{max_retries})"
            )
            return df
            ...
            if attempt < max_retries:
                ...
                raise RuntimeError(
                    f"Failed to download {symbol} from Twelve Data after {max_retries} attempts: {e}"
                ) from e
        except Exception as e:
            if attempt < max_retries:
                logger.warning(
...
def download_symbol_history(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.info(
                f"Downloaded {len(hist)} rows for {symbol} (attempt {attempt}/{max_retries})"
            )
    ...
    if "Date" in df.columns:
        ...
        if "timestamp" not in df.columns:
            ...
            if isinstance(df.index, pd.DatetimeIndex):
                ...
                raise RuntimeError(
                    "Could not determine timestamp column from yfinance data"
                )
...
def download_single_symbol_snapshot(
    ...
    if provider == "finnhub":
        if not finnhub_api_key:
            raise ValueError(
                "finnhub_api_key is required when using provider='finnhub'"
            )
        hist = download_symbol_history_finnhub(
            symbol, start_date, end_date, interval, finnhub_api_key
        )
    elif provider == "twelve_data":
        if not twelve_data_api_key:
            raise ValueError(
                "twelve_data_api_key is required when using provider='twelve_data'"
            )
        hist = download_symbol_history_twelve_data(
            symbol, start_date, end_date, interval, twelve_data_api_key
        )
    else:  # yahoo (default)
...
def download_universe_snapshot(
    ...
    logger.info("=" * 60)
    logger.info("Downloading universe snapshot")
    ...
    for idx, symbol in enumerate(symbols, 1):
        ...
        if existing_file.exists():
            try:
                ...
                if size > 1024:  # At least 1KB
                    logger.info(
                        f"[{idx}/{total}] File already exists for {symbol}, skipping download."
                    )
        ...
        for retry_attempt in range(1, max_retries_per_symbol + 1):
            try:
                ...
                if df.empty:
                    logger.warning(
                        f"[{idx}/{total}] Normalized DataFrame is empty for {symbol}, skipping."
                    )
                    results[symbol] = "empty"
                    symbol_success = True
                    break
                ...
                if is_rate_limit:
                    ...
                    if retry_attempt < max_retries_per_symbol:
                        ...
                        logger.error(
                            f"[{idx}/{total}] ✗ Failed to download {symbol} after {max_retries_per_symbol} attempts: {exc}"
                        )
                        results[symbol] = "error"
                        break
        ...
        if idx < total:
            ...
            if results.get(symbol) == "error":
                ...
                last_error_was_rate_limit = (
                    True  # Conservative: assume rate limit if error
                )
            ...
            if last_error_was_rate_limit:
                ...
                logger.info(
                    f"  Waiting {extra_delay:.0f} seconds before next symbol (rate limit protection)..."
                )
...
Examples:
    ...
    if args.provider == "finnhub":
        ...
        if not twelve_data_api_key:
            ...
            exit(1)
        logger.info(
            f"Using Twelve Data provider with API key: {twelve_data_api_key[:8]}..."
        )
