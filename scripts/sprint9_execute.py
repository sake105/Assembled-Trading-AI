...
def run_execution(a: ExecArgs) -> Tuple[Path, pd.DataFrame]:
    ...
    orders = make_orders(
        a.freq, a.ema_fast, a.ema_slow, price_file=a.price_file, output_dir=a.out_dir
    )
...
def parse_args() -> ExecArgs:
    ...
    p.add_argument(
        ...
        help="Output-Ordner (default: from config)",
    )
...
def main() -> None:
    ...
    if a.ema_fast >= a.ema_slow:
        print(
            f"[WARN] ema_fast ({a.ema_fast}) >= ema_slow ({a.ema_slow}) – üblich ist fast < slow."
        )
    run_execution(a)
