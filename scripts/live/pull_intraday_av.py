...
def parse_args():
    ...
    p.add_argument(
        "--apikey", type=str, default=os.environ.get("ALPHAVANTAGE_API_KEY", "")
    )
    return p.parse_args()
...
def _out_dir(a) -> pl.Path:
    ...
    root = (
        pl.Path(a.repo_root) if a.repo_root else pl.Path(__file__).resolve().parents[2]
    )
...
def main():
    ...
    if not a.apikey:
        print(
            "[AV] WARN: Kein API Key gesetzt. Setze $env:ALPHAVANTAGE_API_KEY oder nutze --apikey. (Für Test: 'demo' → MSFT)",
            flush=True,
        )
    ...
    if wrote == 0:
        print(
            f"[AV] DONE but no files written (Limit/kein Key/keine Daten). Empty: {','.join(empty) if empty else '-'}",
            flush=True,
        )
