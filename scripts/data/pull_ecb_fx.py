...
from io_utils import http_get_text, to_parquet
...
BASE = (
"https://sdw.ecb.europa.eu/quickviewexport.do?trans=N&node=2018794&SERIES_KEY={series}&type=csv"
)
...
lines = [ln for ln in csv_txt.splitlines() if ";" in ln and ln.split(";")[0].strip().isdigit() == False or ln[0].isdigit()]
# Simplifizierter Parser: wir suchen die Data-Section – als Fallback lesen wir alles und filtern später
from io import StringIO
df = pd.read_csv(StringIO(csv_txt), sep=",")
# Heuristik: Spaltennamen finden
cand_date = [c for c in df.columns if c.lower().startswith("time") or c.lower().startswith("period") or c.lower().startswith("date")]
cand_val = [c for c in df.columns if c.lower().startswith("obs") or c.lower().startswith("value")]
...
df = df[[cand_date[0], cand_val[0]]].rename(columns={cand_date[0]:"timestamp", cand_val[0]:"close"})
...
df["symbol"] = p.upper()
df["provider"] = "ecb_sdw"
df = df[["timestamp","symbol","open","high","low","close","volume","provider"]].sort_values("timestamp").reset_index(drop=True)
