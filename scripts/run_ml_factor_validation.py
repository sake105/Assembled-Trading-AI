...
def parse_model_params(param_strings: list[str]) -> dict[str, Any]:
    ...
    for param_str in param_strings:
        if "=" not in param_str:
            logger.warning(
                f"Ignoring invalid parameter format: {param_str} (expected 'key=value')"
            )
            continue
...
def write_ml_validation_report(
    ...
    with output_path.open("w", encoding="utf-8") as f:
        f.write("# ML Factor Validation Report\n\n")
        ...
        f.write(
            "- **R²**: Higher is better. R² > 0 indicates model explains some variance.\n"
        )
        f.write(
            "- **Directional Accuracy**: Percentage of correct sign predictions. > 50% indicates predictive power.\n\n"
        )

        f.write("### Factor-Specific Metrics\n")
        f.write(
            "- **IC (Information Coefficient)**: Cross-sectional correlation between predictions and realized returns.\n"
        )
        ...
        if ls_sharpe_raw is not None:
            ...
            f.write(
                "- **L/S Sharpe (Raw)**: Sharpe ratio of long/short portfolio (Top vs. Bottom quintile based on predictions).\n"
            )
            f.write(
                f"  - Current value: {ls_sharpe_raw:.4f} ({'Good' if ls_sharpe_raw > 1.0 else 'Moderate' if ls_sharpe_raw > 0.5 else 'Weak'} signal)\n"
            )
            if ls_sharpe_deflated is not None:
                f.write(
                    f"- **L/S Sharpe (Deflated)**: Deflated Sharpe ratio adjusted for multiple testing (n_tests={n_tests}).\n"
                )
                f.write(
                    f"  - Current value: {ls_sharpe_deflated:.4f} ({'Significant' if ls_sharpe_deflated > 0 else 'May be due to luck/multiple testing'})\n"
                )
            f.write("\n")
        ...
        if feature_importance_df is not None and not feature_importance_df.empty:
            ...
            momentum_features = [
                ...
                if "mom" in f.lower() or "momentum" in f.lower()
            ]
            volatility_features = [
                f
                for f in top_feature_names
                if "vol" in f.lower() or "volatility" in f.lower() or "rv_" in f.lower()
            ]
            ...
            if momentum_features:
                f.write(
                    f"- **Momentum factors** ({', '.join(momentum_features)}) are among the top features. "
                )
                f.write(
                    "This suggests momentum effects are important for return prediction.\n"
                )
            if volatility_features:
                f.write(
                    f"- **Volatility factors** ({', '.join(volatility_features)}) appear in top features. "
                )
                f.write("Volatility characteristics influence predictions.\n")
            if value_features:
                f.write(
                    f"- **Value factors** ({', '.join(value_features)}) are prominent. "
                )
                f.write("Value-based signals contribute to the model.\n")
            if quality_features:
                f.write(
                    f"- **Quality factors** ({', '.join(quality_features)}) are important. "
                )
                f.write("Quality characteristics matter for predictions.\n")

            if not (
                momentum_features
                or volatility_features
                or value_features
                or quality_features
            ):
                f.write(
                    "- Top features are listed above. Review their characteristics to understand model behavior.\n"
                )
        ...
        if (
            permutation_importance_df is not None
            and not permutation_importance_df.empty
        ):
            f.write("## Top 10 Features (Permutation Importance)\n\n")
            f.write(
                "*Note: Permutation importance measures how much model performance decreases when a feature is shuffled.*\n\n"
            )
...
def run_ml_validation(
    ...
    logger.info("ML Factor Validation")
    ...
    try:
        ...
        logger.info(
            f"Loaded factor panel: {len(factor_panel_df)} rows, {len(factor_panel_df.columns)} columns"
        )
    ...
    if last_model is not None and feature_names:
        try:
            ...
            feature_importance_df = compute_model_feature_importance(
                last_model, feature_names
            )
            logger.info(
                f"Feature importance computed: {len(feature_importance_df)} features"
            )

            # Save feature importance CSV
            feature_importance_path = (
                output_dir
                / f"ml_feature_importance_{model_type}_{label_col.replace('fwd_return_', '')}.csv"
            )
        ...
        try:
            ...
            if len(X_full) > 5000:
                logger.info(
                    f"Subsampling to 5000 rows for permutation importance (from {len(X_full)} rows)"
                )
            ...
            logger.info(
                f"Permutation importance computed: {len(permutation_importance_df)} features"
            )

            # Save permutation importance CSV
            perm_importance_path = (
                output_dir
                / f"ml_permutation_importance_{model_type}_{label_col.replace('fwd_return_', '')}.csv"
            )
            ...
            logger.warning(
                f"Failed to compute permutation importance: {e}", exc_info=True
            )
            permutation_importance_df = None
    ...
    try:
        ...
        logger.info(
            f"Used {len(features)} features: {features[:10]}{'...' if len(features) > 10 else ''}"
        )
    ...
    predictions_sample_file = (
        output_dir / f"ml_predictions_sample_{model_type}_{label_col}.parquet"
    )

    predictions_sample = (
        predictions_df.head(10000) if len(predictions_df) > 10000 else predictions_df
    )
    ...
    ls_sharpe_raw = eval_metrics.get("ls_sharpe")
    n_samples = eval_metrics.get(
        "n_predictions", len(predictions_df) if predictions_df is not None else 0
    )
    ...
    portfolio_metrics_file = (
        output_dir / f"ml_portfolio_metrics_{model_type}_{label_col}.csv"
    )
    ...
    predictions_summary = {
        ...
        "y_true_range": [
            float(predictions_df["y_true"].min()),
            float(predictions_df["y_true"].max()),
        ],
        "y_pred_range": [
            float(predictions_df["y_pred"].min()),
            float(predictions_df["y_pred"].max()),
        ],
    }
