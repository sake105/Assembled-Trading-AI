...
def _download_ticker(symbol: str, start: str, end: str, retries: int = 3) -> pd.DataFrame:
    for attempt in range(retries):
        try:
            ...
            date_col = next((c for c in hist.columns if c in ("date", "datetime")), None)
            if date_col is None:
                logger.warning("%s: no date column found in %s", symbol, hist.columns.tolist())
                return pd.DataFrame()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Download but do not write")
    ...
    logger.info("Downloading %d symbols from %s to %s", len(symbols), args.start, args.end)
    ...
    logger.info("Downloaded %d rows (%d symbols)", len(pre2020),
                pre2020["symbol"].nunique())
    ...
    logger.info("Combined: %d rows (%d dropped as duplicates)", len(combined), before - len(combined))
    logger.info("Date range: %s to %s", combined["timestamp"].min().date(),
