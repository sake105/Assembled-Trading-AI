...
def create_signal_and_sizing_fns(args: argparse.Namespace):
    ...
    if args.strategy == "trend_baseline":
        ...
        if not args.bundle_path:
            raise ValueError("--bundle-path is required for multifactor_long_short strategy")
...
def run_walk_forward_analysis(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        logger.info(f"Loaded prices: {len(prices_df)} rows, {prices_df['symbol'].nunique()} symbols")
        ...
        backtest_fn = make_engine_backtest_fn(
            ...
            rebalance_freq=args.rebalance_freq if args.strategy == "multifactor_long_short" else args.freq,
        ...
        logger.info(f"Walk-forward analysis completed: {result.aggregated_metrics['n_successful_splits']}/{result.aggregated_metrics['n_splits']} splits successful")
        
        # Write outputs
        output_dir = Path(args.output_dir) if args.output_dir else ROOT / "output" / "walk_forward"
        ...
        if "mean_test_sharpe" in result.aggregated_metrics:
            logger.info(f"  Mean Test Sharpe: {result.aggregated_metrics['mean_test_sharpe']:.4f}")
        if "std_test_sharpe" in result.aggregated_metrics:
            logger.info(f"  Std Test Sharpe: {result.aggregated_metrics['std_test_sharpe']:.4f}")
...
Examples:
  ...
  python scripts/run_walk_forward_analysis.py \\
    ...
    group.add_argument("--universe", type=Path, help="Path to universe file (one symbol per line)")
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Spread weight (override default)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Impact weight (override default)"
    )
