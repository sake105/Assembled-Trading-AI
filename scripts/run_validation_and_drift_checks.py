...
def load_performance_metrics_from_files(output_dir: Path, freq: str = "1d") -> dict[str, float | None] | None:
...
def run_drift_checks(
        ...
        if not current_features.empty and not reference_features.empty:
            ...
            if not feature_drift_df.empty:
                ...
                results["feature_drift"] = {
                    ...
                    "features_with_drift": drift_features.to_dict("records") if not drift_features.empty else [],
                    "overall_severity": "SEVERE" if (feature_drift_df["drift_flag"] == "SEVERE").any() 
                                      else "MODERATE" if (feature_drift_df["drift_flag"] == "MODERATE").any()
                                      else "NONE"
                }
            else:
                results["feature_drift"] = {
    ...
    reference_labels = extract_labels_from_dataset(reference_dataset) if reference_dataset is not None else None
    ...
    if current_labels is not None and reference_labels is not None:
        ...
        logger.warning("Cannot detect label drift: labels not available in one or both datasets")
    ...
    logger.info("Performance drift detection skipped (requires equity curve, not available from ML dataset)")
...
def write_summary_report(
    ...
    if reference_dataset_path:
        ...
        lines.append("- **Reference Dataset:** Not specified (using sample/reference data)")
    lines.append("")
    ...
    if metadata:
        ...
        if "performance" in validation_details:
            ...
            if perf_meta.get("sharpe_checked"):
                ...
                lines.append(f"- **Sharpe Ratio:** {sharpe:.4f if sharpe is not None else 'N/A'} (threshold: {min_sharpe})")
    ...
    if feature_drift:
        ...
        severity_prefix = "[SEVERE]" if overall_severity == "SEVERE" else "[MODERATE]" if overall_severity == "MODERATE" else "[OK]"
        ...
        if features_with_drift:
            ...
            for feat in features_with_drift[:10]:  # Top 10
                lines.append(f"| {feat['feature']} | {feat['psi']:.4f} | {feat['drift_flag']} |")
            lines.append("")
    else:
        lines.append("**Feature Drift:** [WARNING] Not available (reference dataset missing)")
        lines.append("")
    ...
    if label_drift:
        ...
        severity_prefix = "[SEVERE]" if drift_severity == "SEVERE" else "[MODERATE]" if drift_severity == "MODERATE" else "[OK]"
        lines.append(f"**Label Drift:** {severity_prefix} {drift_severity} (PSI: {psi:.4f})")
        lines.append("")
        ...
        if drift_detected:
            ...
            lines.append(f"- **Base Mean:** {base_mean:.4f if base_mean is not None else 'N/A'}")
            lines.append(f"- **Current Mean:** {current_mean:.4f if current_mean is not None else 'N/A'}")
            lines.append(f"- **Mean Shift:** {mean_shift:+.4f if mean_shift is not None else 'N/A'}")
            lines.append("")
    else:
        lines.append("**Label Drift:** [WARNING] Not available (labels or reference dataset missing)")
        lines.append("")
    ...
    if performance_drift:
        ...
        lines.append("**Performance Drift:** [WARNING] Not available (requires equity curve)")
        lines.append("")
    
    # Footer
    lines.append("---")
    lines.append("")
    lines.append(f"*Report generated by run_validation_and_drift_checks.py at {timestamp.isoformat()}*")
...
def run_validation_and_drift_checks(
    ...
    if reference_dataset_path:
        ...
        if reference_dataset_path.exists():
            ...
            logger.warning(f"Reference dataset not found: {reference_dataset_path}, skipping drift comparison")
    ...
    if performance_metrics is None:
        ...
        performance_metrics = load_performance_metrics_from_files(settings.output_dir, freq=freq)
        if performance_metrics is None:
            logger.warning("Could not load performance metrics from files - validation will be limited")
            performance_metrics = {}
    ...
    drift_results = run_drift_checks(
        ...
        psi_threshold=validation_config.get("psi_threshold", 0.2) if validation_config else 0.2,
        severe_threshold=validation_config.get("severe_threshold", 0.3) if validation_config else 0.3
    )
