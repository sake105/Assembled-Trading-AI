...
def main():
    ...
    micro = df.copy()
    micro["ema20"] = (
        micro.groupby("symbol")["adj_close"].apply(lambda s: ema(s, 20)).values
    )
    micro["ema60"] = (
        micro.groupby("symbol")["adj_close"].apply(lambda s: ema(s, 60)).values
    )
    micro["rsi14"] = (
        micro.groupby("symbol")["adj_close"].apply(lambda s: rsi(s, 14)).values
    )
    micro["mom20"] = (
    ...
    regime = df.copy()
    regime["vol20"] = (
        regime.groupby("symbol")["adj_close"]
        .apply(lambda s: realized_vol(s, 20))
        .values
    )
    regime["vol60"] = (
        regime.groupby("symbol")["adj_close"]
        .apply(lambda s: realized_vol(s, 60))
        .values
    )
    ...
    regime["dd60"] = (
        regime.groupby("symbol")["adj_close"].apply(lambda s: dd(s, 60)).values
    )
