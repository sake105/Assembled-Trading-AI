...
- (e) ⚠️ **OFFEN (F-9.6d-4):** Pipeline-Doublung in `qa/backtest_engine.py` Step 4.5 (~line 1022-1033) + Step 4.6 (~line 1036-1073) ruft `apply_fill_model_pipeline` auf `orders_df` ein zweites Mal auf (ohne `available_cash`), nachdem `simulate_with_costs` intern bereits dieselbe Pipeline MIT cash-gate gelaufen ist. Folge: `orders_df` bekommt status/fill_qty/cost-cols nachträglich, was die test-discrimination der §9.6(d) regression-guard (`test_backtest_engine_no_costs_trades_are_cash_gated`) schwächt — der Test lockt forward-direction (trades_df IS used) korrekt, kann aber nicht zwischen cash-gated trades_df und un-gated-but-fill-pipelined orders_df über Schema unterscheiden. Kein Korrektheitsbug, aber architektonische Doppel-Wahrheit. Cleanup-Auftrag eigener Scope (sensitive zone, qa/).
