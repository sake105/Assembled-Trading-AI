...
- (d) ✅ **DIAGNOSTIZIERT (2026-05-19 spät):** with-costs $91k vs no-costs $27k bei byte-identischen Trades — kein Quirk, sondern ein **Correctness-Bug im `simulate_equity` no-costs-Pfad** (`src/assembled_core/pipeline/backtest.py:319`). Dieser Simulator kennt **keine Cash-Constraint** — `_simulate_fills_per_order` akzeptiert Orders auch bei Cash<0. Konkrete Spur: Equity-Curve no-costs hat Cliff am 2026-03-24 ($103k → $27k an einem Tag), beide Curves dann konstant bis Ende (Simulation produziert keine neuen Fills nach Cash-Erschöpfung, but Equity bleibt eingefroren). `simulate_with_costs` (portfolio.py:20) enforced Cash via cumulative cash_delta-Tracking und stoppt Trades vorher. **Impact:** `--no-costs`-Backtests sind unzuverlässig für Signal-Quality-Aussagen (Test 0 no-costs -73% war übertrieben, signal-only liegt eher bei -7.74% wie with-costs). **Fix-Optionen** (eigener Auftrag, sensitive zone): (i) Cash-Constraint zu `_simulate_fills_per_order` hinzufügen (reject buys bei Cash-Unterdeckung), oder (ii) `simulate_equity` deprecate und nur noch `simulate_with_costs` mit commission_bps=0 für no-costs-Tests anbieten. Variante (ii) ist sauberer (eine Wahrheit, ein Simulator).
