...
- (b) ⚠️ **PHASE 1 IN PROGRESS (2026-05-21):** trend_baseline als Primary, mfv2 als Secondary/regime-bound — strategische Reorganisation. Phase-1 shadow-mode implementiert: `trend_baseline` jetzt registrierte paper-strategy in `paper_runner._prd_make_strategy_fns`. Neue `_prd_run_shadow_strategy()` helper runs trend_baseline parallel zu primary mfv2 (no orders, no broker), persistiert `shadow_signals.json` + `shadow_targets.json` zu run-dir. Dry-run today: mfv2 5 LONGs, trend_baseline 28 LONGs, overlap 2 (auf 195-sym universe, 2026-05-18 bar). Aktiviert via `app.yaml.paper_runner.shadow_strategy.{enabled,name,ma_fast,ma_slow,max_positions}`. Nach N Tagen Vergleichsdaten → Phase 2 Decision (echter switch oder bleiben). **Phase-2 Pre-Conditions (vor activation als primary):** (i) `check_exit_signals` für trend_baseline implementieren (stop-loss / trailing-stop / time-stop — aktuell nur LONG→FLAT via MA-flip), (ii) F-S1-M1 unknown-strategy-name silent-fail in `_prd_run_shadow_strategy` mit explicit WARN + Whitelist fixen, (iii) F-S1-M2 shadow signal_fn bekommt raw prices vs primary bekommt features — Doc oder feature-pipeline-alignment für multifactor_* shadow-Strategien, (iv) Backtest mit Exit-Logik bestätigt > +43.02% Baseline, (v) Risk-Review, (vi) Drawdown-Limit-Check gegen aktuelle Pilot-Equity ($91k = -7.5% Start).
