...
| 3.1 | check_leakage im Orchestrator scharf | ✅ ERLEDIGT (2026-08-17 nach Operator-Deny-Lift): Orchestrator-QA-Step baut das Frame via qa/leakage_frame (gleiches Muster wie run_backtest_strategy); Pflicht-Dry-Run auf dem ECHTEN Frame vor Scharfschaltung: Gate armed auf 1.412 Zeilen, Ergebnis ok (kein qa_block-Risiko). **Reichweite (F-senior-21):** das Gate prueft EXAKT EINE Feature-Spalte pro Lauf (eps_surprise_pct), nicht das ganze Panel; Wiring-Spy-Test verhindert stille Rueck-Regression auf SKIPPED |
| 3.2 | CI-Leakage-No-op ersetzen | ✅ ERLEDIGT (2026-08-17): Phantom-Aufruf (validate_altdata.py existierte nie, `\|\| echo`) ersetzt durch scripts/ci/leakage_detector_selftest.py (fail-drill-Muster: praepariertes Leak MUSS blocken, sauberes Frame ok, kein Frame SKIPPED; lokal 3/3 PASS) — der Runner hat keine Produktionsdaten, der echte Daten-Check laeuft seit 3.1 taeglich lokal |
| 4.1 | Trailing-TEIL-Reduktion wirksam | ✅ ERLEDIGT (2026-08-17): reduction_symbols skalieren jetzt auch target_qty (Faktor 1-reduction; Voll-Stop qty=0 hatte schon gewirkt) — GRADUAL DE-RISK stand vorher nur im Log (E-143); Test-Pin in tests/pipeline/test_tc_sizing_trailing_stops_wiring.py |
| 4.2 | corr-regime-Gate | ✅ ERLEDIGT (2026-08-17): detect_correlation_regime_shift skalierte target_qty UNGATED/ohne Shadow neben dem shadow-gegateten Guard (Audit-§5-Verdacht bestaetigt) — jetzt Part-D-Muster (correlation_regime_shift.shadow_only, Default true = KONSERVATIVE Verhaltensaenderung) + meta["correlation_regime_shift"] wird erstmals geschrieben (der corr_spiked-Rebalance-Trigger las ein nie gesetztes Feld); 3 Tests |
| 4.3 | Factor-Store-Cache-Key (§0.05) | ✅ ERLEDIGT (2026-08-17): compute_universe_key traegt jetzt SHA-256 ueber features/*.py (_feature_code_version) — Code-Aenderung invalidiert automatisch, kein Bump-Vergessen moeglich; §0.05 auf BEHOBEN, Regressionstests (stale Panel wird verworfen); Altbestand output/factors/ startet einmalig COLD |
| 4.4 | per_symbol_cost_bps am Call-Site | ✅ ERLEDIGT (2026-08-17/18): cost_aware-Sizing uebergibt per-Symbol-Kosten (commission aus paper_pilot.cost_model + ADV-Bucket-Spread/2). **Korrektur nach Stage-2-Review (E-186):** die erste Fassung nutzte SpreadModel() OHNE buckets — dessen Default kollabiert JEDES Symbol auf denselben Fallback, es war also nur ein anderer flacher Wert; explizite ADV-Buckets + Varianz-Test stellen die Differenzierung jetzt real her. **EHRLICHE GRENZE:** der Pilot faehrt method=black_litterman — dieser Zweig ist auf dem LIVE-Pfad derzeit inert |
| 4.5 | Earnings-Guard | ✅ ERLEDIGT (2026-08-17): signals/earnings_integration.py existiert erstmals REAL (4 Monate Phantom-Import, still geskippt) — Pre-Earnings-Suppression (score=0 im Fenster, nur ZUKUENFTIGE Termine) + PEAD-Drift (nur OFFENGELEGTE Surprises, disclosure_date<=as_of, Clipping, Suppression-Vorrang); 6 Tests; earnings_guard.enabled wieder true (safe no-op ohne Kalender-Cache); Phantom-Import-Liste bereinigt — der earnings-calendar-refresh-CI hat erstmals einen Konsumenten |
...
| 5.5 | Steuer-Engine-Integration | ✅ KERN ERLEDIGT (2026-08-17 nach Deny-Lift, Plan-Agent-Design): Steuer als PARALLELE, rein lesende FIFO-Schatten-Sicht — accounting/tax_regime_de.py (PrivatDE-Port aus mandat2, bitgleicher Paritaetstest gegen das eingefrorene Original), accounting/tax_view.py (stateless In-Memory-Replay via tax_lots.match_fifo, EUR je Datum, Exit-Fee-Regel, Verlustvortrag, Pauschbetrag, over_close-Sichtbarkeit, fx_source-Ehrlichkeit), Orchestrator-Step 4c (Opt-in --write-tax-view, best-effort, ledger_result-invariant) + scripts/ops/build_tax_report.py (Backfill). position_engine bleibt autoritativ fuers operative PnL. 11 Tests. BEWUSST NICHT: summarize_with_regime-Doppelpfad in compliance (zweite Wahrheit, Rule 50 — tax_view.json ist als authoritative markiert); Dividenden-Zeile leer (keine DIVIDEND-Ledger-Events, als Luecke im Report ausgewiesen); ECB-Kurs-Cache = Follow-up |
