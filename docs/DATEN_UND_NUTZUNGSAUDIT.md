...
| 1.4 | Kill-Switch-Zustand dokumentiert (KNOWN_ISSUES §0.00) | ✅ Doku ERLEDIGT; Disengage = Operator (`OPERATOR_KILL_TOKEN`) OFFEN |
...
| 6.2 | Gratis-Quellen nachziehen (EDGAR/yfinance) | ✅ ERLEDIGT NACH ZWISCHENFALL (2026-08-17): Der ERSTE Alpaca-Nachzug hat den Live-Cache mit RAW-Bars beschaedigt (Alpaca-Default; BKNG +2444 %-Naht, Splits als −90 %-Crashs — Stage-2-Review FAIL, E-165). Saniert: Restore aus .bak + Backfill, adjustment=ALL, adj_close-Invariante am Schreibpunkt, Overlap-Re-Adjustierung (Corporate Actions zwischen Ankern) + fail-closed Naht-Guard. ENDSTAND VERIFIZIERT: 280.460 Zeilen, 0 NaN, KEINE neuen >50 %-Moves vs. Referenz; **193/220 Symbole auf 2026-08-14**, 25 weiter auf 2026-08-05 (Batch-Budget --max-symbols=30 bzw. Overlap-Drop; naechster Scheduler-Lauf zieht sie nach: 27 Targets <= Budget 30), 2 delistet (EXAS/HOLX). CRWD/DD dauerhaft verworfen (Overlap-Ratio inkonstant, geflaggt — Ursachenanalyse offen). Root-Cause 16.08.: prewarm lud .env nie [E-145, gefixt]. GRENZE (F-auditor-8): Naht-Guard + Overlap-Re-Adjustierung sitzen NUR in prewarm — refresh_sector_etf_cache.py (5. Schreiber derselben Datei, 9 ETFs) kann die E-165-Teil-2-Naht dividendengross weiterhin erzeugen; Helper-Extraktion = Follow-up. macro/fundamentals/dividends erneuert; DERA aktuell; earnings/insider = Rest-Offenpunkt |
...
| 6.4 | Toter-Code-Archivierung (~40 Waisen) | ◐ TRANCHE 1 ERLEDIGT (2026-08-17: 9 Module + 3 dedizierte Tests + 2 leere Verzeichnisse nach archive/orphaned_code_2026-08-17/, Kriterium 0 Referenzen + keine Sammeltest-Bindung; Collection sauber (0 Errors; 0 Collection-Errors (Zahl interpreterabhaengig, .venv Py3.11: 9.350 nach allen Pins)). TRANCHE 2 OFFEN: 16 Module an Sammeltests mit harten Imports gebunden [Testdatei-Chirurgie], 9 mit Referenz-Ketten — Details im Archiv-README) |
