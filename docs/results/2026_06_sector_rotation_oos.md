...
> **CORRECTION (2026-06-01) — price-type label.** The honesty note below says prices are "RAW close (not dividend/total-return adjusted)". A later feed-divergence check (`docs/results/2026_06_sector_fullhist_feed_divergence.md`) found this is wrong: the live `daily.parquet` `close` is **total-return (split+dividend) adjusted** — it matches yfinance Adj Close to ~0.00 bps median across all 9 symbols, and diverges from yfinance raw Close by 330–1600 bps (the cumulative dividend adjustment). So this falsification actually ran on **total-return** prices, not raw. This does NOT change the verdict — every book AND the SPY benchmark use the same adjusted basis, so the relative SPY-beat comparison is unaffected and **all books stay REJECTED** — and the "absolute CAGRs understate total return" caveat below is moot (the CAGRs were already total-return). The generating script's honesty prose has since been corrected to state the total-return (adjusted) basis; the doc body below predates that regeneration and is superseded by this note.
