...
def run_trend_baseline_backtest(
    output_dir: Path, price_file: Path | None = None
) -> dict:
...
def run_event_insider_shipping_backtest(
    output_dir: Path, price_file: Path | None = None
) -> dict:
...
def build_ml_dataset(
    output_dir: Path, strategy: str, price_file: Path | None = None
) -> Path | None:
...
def run_validation_and_drift_checks(dataset_path: Path, output_dir: Path) -> dict:
    ...
    from scripts.run_validation_and_drift_checks import (
        run_validation_and_drift_checks as run_checks,
    )
    ...
    try:
        ...
        if report_path.exists():
            ...
            validation_ok = (
                "[PASSED]" in report_content or "Status: [PASSED]" in report_content
            )
...
def print_final_summary(all_results: dict) -> None:
    ...
    if validation_result.get("success"):
        ...
        print(
            f"{status} Validation & Drift Checks: {'PASSED' if validation_ok else 'FAILED'}"
        )
        print(f"    Drift Severity: {drift_severity}")
    else:
    ...
    print("=" * 70)
    print(
        f"System Health Check completed at {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}"
    )
    print("=" * 70)
...
def main() -> int:
    ...
    if not price_file:
        logger.warning(
            "Sample price file not found, backtests may fail if no data is available"
        )
    ...
    if ml_dataset_path and ml_dataset_path.exists():
        ...
        all_results["validation"] = {
            "success": False,
            "error": "ML dataset not available",
        }
        print_section("STEP 4: Validation & Drift Checks")
    ...
    overall_success = (
        trend_result.get("success", False)
        and event_result.get("success", False)
        and ml_dataset_path is not None
        and all_results.get("validation", {}).get("success", False)
    )
